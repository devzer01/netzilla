...
<div class="profile-list">
	...
	<img border="0" src="thumbnails.php?file={$item.picturepath}&amp;w=97&amp;h=98" width="97" height="98" alt="{$item.username}"/>
	</div>
</div>
</a>
...
{if $style eq '2'}
	<a href="javascript:void(0)" class="quick-icon-left del-icon" title="{#Delete#}" onclick="removeFavorite('{$item.username}','favorite-list-container', {$style}); return false;"></a>
{else}
	...
	{if ($nofavorite ne 'true') and ($item.username ne $smarty.session.sess_username)}
		...
		<a href="javascript:void(0)" class="quick-icon-right fav-icon" title="Favorite" onclick="jQuery(this).remove(); return addFavorite('{$item.username}','favorite-list-container');"></a>
		{else}
		<!--ALIGN ICON HERE-->
		<!-- <div class="fav"><img src="images/cm-theme/icon-fav-g.png"/></div> -->
		{/if}
	{elseif $item.username ne $smarty.session.sess_username}
		<a href="javascript:void(0)" class="quick-icon-right del-icon-g" title="{#Delete#}" onclick="return removeFavorite('{$item.username}','favorite-list-container')"></a>
	{/if}
{/if}
</div>
{/if}