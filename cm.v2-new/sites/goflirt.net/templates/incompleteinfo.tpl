...
<div style="line-height:18px; margin-bottom:20px;">{#reg2_headline_intro#}</div>
	...
	<form id="register_form" name="register_form" method="post" action="?action=incompleteinfo{if $smarty.get.action eq 'incompleteinfo_skip'}&nextstep=mobileverify_skip{else}&nextstep=mobileverify{/if}">	
		<label class="text">{#mobile#}:</label><br class="clear"/>
		<span>
			...
			<input type="text" id="phone_number" name="phone_number" value="{$save.phone_number}" class="formfield_01" maxlength="10" onkeypress="return isValidCharacterPattern(event,this.value,3)" onkeyup="checkNullPhone('', document.getElementById('phone_code2').value, document.getElementById('phone_number').value)" onblur="checkNullPhone('', document.getElementById('phone_code2').value, document.getElementById('phone_number').value)" style="width:295px !important;"/>
			<br clear="all"/>
		...
		<span style="margin-bottom:10px; float:left; margin-left:40px;">
			...
			<a href="#" onclick="submitAjaxFormIncompleteInfo(); return false;" class="btn-search" style="width:auto !important; padding:0 10px; width:350px !important;">{#reg2_submit#}</a>
			{if $smarty.get.action eq "incompleteinfo_skip"}
			<a href="./" onclick=""  class="btn-search">{#reg2_skip#}</a>
			{else}
			<a href="#" onclick="jQuery('#mask').hide(); jQuery('.window').hide(); return false;" class="btn-search" style="width:auto !important; padding:0 10px; margin-left:10px; width:150px !important;">{#CANCEL#}</a>
			{/if}
		</span>
