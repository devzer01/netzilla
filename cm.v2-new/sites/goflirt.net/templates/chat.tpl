...
<script type="text/javascript">
		...
		jQuery.get(uri, function(data) {
			if(data != '')
			{
				...
				else
				{
					...
					if(timer2)
						clearTimeout(timer2);
					timer2 = setTimeout(markAsRead, 5000);
					if(total!=0)
						loadMessagesHistory(currentUsername, total, "part1");
				}
			}
			if(timer1)
			{
	...
	{
		loadMessagesHistory(currentUsername, totalMessages, "part2");
	}
	...
	function addAttactmentGifts(id, coins)
	{
		...
		jQuery.ajax({ type: "POST", dataType: "json", url: "?action=attachments&type=gifts", data: "id="+id, success:(function(result){
			if(result.code=="FINISHED")
			{
				...
				jQuery('#attachments-list').css('display', 'none');
				jQuery('#attachments-list').append(result.html);
				jQuery('#mask').hide();
				jQuery('.window').hide();
				jQuery.ajax(
