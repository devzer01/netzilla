...
<!--icon search -->
    	...
    	<ul class="container-icon-search">
        	<li><a href="#" id="search_online_button" onclick="return doSearch('action=search&type=searchOnline');"><img src="images/cm-theme/s-online.png"/><p>Online</p></a></li>
            <li><a href="#" onclick="return doSearch('action=search&type=searchNewestMembers')"><img src="images/cm-theme/s-news.png" /><p>{#Newest#}</p></a></li>
            <li><a href="#" onclick="return doSearch('action=search&type=searchGender&wsex=m&sex=w')"><img src="images/cm-theme/s-man.png"/><p>{#MAN_SEARCH_WOMAN#}</p></a></li>
            <li><a href="#" onclick="return doSearch('action=search&type=searchGender&wsex=w&sex=m')"><img src="images/cm-theme/s-women.png"/><p>{#WOMAN_SEARCH_MAN#}</p></a></li>
            <li><a href="#" onclick="return doSearch('action=search&type=searchGender&wsex=m&sex=m')"><img src="images/cm-theme/s-gay.png"/><p>{#MAN_SEARCH_MAN#}</p></a></li>
            <li><a href="#" onclick="return doSearch('action=search&type=searchGender&wsex=w&sex=w')"><img src="images/cm-theme/s-less.png"/><p>{#WOMAN_SEARCH_WOMAN#}</p></a></li>
        </ul>
...
<!--box search -->    
    <div class="container-box-content-01">
        ...
        <div class="box-content-01-m-m search-height">
        	...
        	<input name="username" type="text" id="username" placeholder="{#Search#} Nickname" class="formfield_01" style="width:130px; margin-bottom:5px;">
			...
			{/if}
			<select name="q_gender" id="q_gender" class="formfield_01" style="width:140px; margin-bottom:5px;">
				<option value="">{#Any#}</option>
				{html_options options=$gender}
			</select>
            <label>{#Age#}:</label>
			<select name="q_minage" id="q_minage" onchange="ageRange('q_minage', 'q_maxage')" class="formfield_01" style="width:50px; margin-bottom:5px;">
			...
			{/if}
			<select name="q_maxage" id="q_maxage" class="formfield_01" style="width:50px; margin-bottom:5px;">
				{html_options options=$age selected=$select_q_maxage}  
			</select>
            <label>{#Have_Photo#}:</label>
			<select name="q_picture" id="q_picture" class="formfield_01" style="width:140px; margin-bottom:5px;">
				{html_options options=$picyesno selected=$smarty.session.right_search.q_picture}
			</select>
            <label>{#Country#}:</label>
			<select id="q_country" name="country" onchange="loadOptionState('#q_state', this.options[this.selectedIndex].value, ''); if((jQuery(this).val()!=1)) jQuery('#state_span').hide(); else jQuery('#state_span').show();" class="formfield_01" style="width:140px; margin-bottom:5px;">
			</select>
			<span id="state_span" style="display: none">
            <label>{#State#}:</label>
			<select id="q_state" name="state" class="formfield_01" style="width:140px; margin-bottom:5px;">
			</select>
			</span>
			<span style="display: none">
            <label>{#City#}:</label>
			<select id="q_city" name="city" class="formfield_01" style="width:140px; margin-bottom:5px;">
			</select>
			</span>
