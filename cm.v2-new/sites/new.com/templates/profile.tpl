...
<div class="bg-box-viewprofile">
	...
	<ul class="container-profile-border my-profile" style="margin-left:5px;">
		<li><a href="thumbnails.php?file={$profile.picturepath}" class="fancybox profile-border{if $profile.approval} approval{/if}"></a><img src="thumbnails.php?file={$profile.picturepath}&w=108&h=108" width="108" height="108" class="profile-img"/></li>
	</ul>
	<!--upload photo profile -->
	<!--end upload photo profile -->
	{include file="profile_detail.tpl"}
	<div style="float:left; padding:0 10px; margin-bottom:10px;">            
		...
		<div class="container-btn-menu-profile"> 
			<form id="profilepic_form" method="post" enctype="multipart/form-data" action="?action=editprofile">
				<input type="file" id="profilepic" name="profilepic" onchange="this.form.submit();" style="width:300px; height:45px; opacity:0; filter:alpha(opacity=0); margin-left:20px; cursor:pointer; position:absolute;"/>
			</form>
