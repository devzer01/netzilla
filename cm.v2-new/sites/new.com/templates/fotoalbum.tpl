...
<div style="margin:0 20px 20px 20px;">
    ...
    {else}
		...
		<a href="#" onclick="jQuery('#upload_foto_form').submit(); return false;" class="btn-login" style="margin-top:10px; width:80px;">Upload</a>
		<br class="clear" />
	{/if}
</div>
{/if}
...
{if count($fotoalbum)}
<ul class="container-photo-border">
{foreach from=$fotoalbum item=item name="fotoalbum"}
	<li><a href="thumbnails.php?file={$item.picturepath}" class="fancybox profile-border {if $item.approval} approval{/if}"></a><img src="thumbnails.php?file={$item.picturepath}&w=108&h=108" width="108" height="108" class="profile-img"/>
	<a href="#" class="q-icon-r q-del" onclick="return deletePhoto({$item.id}{if $item.approval}, 'APPROVAL'{/if})"><span>del</span></a>
	</li>
{/foreach} 
</ul>
{/if}
