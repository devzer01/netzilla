...
//we can use much simpler jquery/css to test this.
		...
		if(eval("data[n][1]"+data[n][2]+"data[n][3]"))
		{
			...
			if((data[n][0] != '') && (data[n][5] != ''))
			{
				if(data[n][5] != "#accept_info")
					$(data[n][5])[0].innerHTML = "<div class='error_info left'>"+data[n][4]+"</div>";
				else
					$(data[n][5])[0].innerHTML = "<div class='error_info position-top'>"+data[n][4]+"</div>";
			}
			status = false;
		}
	}
...
function loadOptionState(state_id, select_country, select_state)
{
	...
	else
	{
		...
		{
			$(state_id)[0].options[n+1] = new Option(state[select_country][n].value, state[select_country][n].id);
			if(select_state == state[select_country][n].id)
				$(state_id)[0].options[n+1].selected = true;
		}
		$(state_id)[0].options.length = state[select_country].length+1;
		if($(state_id)[0].options.length==2)
		{
			$(state_id)[0].selectedIndex=1;
			state_select = $(state_id)[0].options[1].value;
		}
	}
}
...
function loadOptionCity(city_id, select_state, select_city)
{
	...
	{
		$(city_id)[0].options[0].selected = true;
		$(city_id)[0].options.length = 1;
	}
	else
	{
		for(m=0; m<city[select_state].length; m++)
		{
			$(city_id)[0].options[m+1] = new Option(city[select_state][m].value, city[select_state][m].id);
			if(select_city == city[select_state][m].id)
				$(city_id)[0].options[m+1].selected = true;
		}
		$(city_id)[0].options.length = city[select_state].length+1;
		if($(city_id)[0].options.length==2)
		{
			$(city_id)[0].selectedIndex=1;
			city_select = $(city_id)[0].options[1].value;
		}
	}
}

function loadOptionCountry1(data, status, originalRequest)
{
	if(originalRequest.status != 200) return;
		...
		$('#country')[0].options[0] = new Option(select_country, 0);
		$('#state')[0].options[0] = new Option(select_state, 0);
		$('#city')[0].options[0] = new Option(select_city, 0);

		for(n=0; n<country.length; n++)
		{
			$('#country')[0].options[n+1] = new Option(country[n].value, country[n].id);
		}
		
		//whats this for?
		//$('#country')[0].options.length = country.length+1;

		if((country_select == 0) || (country_select == ''))
			$('#country')[0].options[0].selected = true;
		else
		{
			for(var n=1; n<$('#country')[0].options.length; n++)
			{
				if($('#country')[0].options[n].value == country_select)
					$('#country')[0].options[n].selected = true;
			}
		}
...
function q_loadOptionCountry(data, status, originalRequest)
{
	if(originalRequest.status != 200) return;
		...
		$('#q_country')[0].options[0] = new Option(select_country, 0);
		
		//city and state is not available in some search screens
		if ($("#q_state")[0]) $('#q_state')[0].options[0] = new Option(select_state, 0);
		if ($("#q_city")[0])  $('#q_city')[0].options[0] = new Option(select_city, 0);

		for(n=0; n<country.length; n++)
		{
			$('#q_country')[0].options[n+1] = new Option(country[n].value, country[n].id);
			if(q_country_select == country[n].id)
				$('#q_country')[0].options[n+1].selected = true;
		}
		$('#q_country')[0].options.length = country.length+1;

		if ($("#q_state")[0]) loadOptionState('#q_state', q_country_select, q_state_select);
		if ($("#q_city")[0]) loadOptionCity('#q_city', q_state_select, q_city_select);
}
