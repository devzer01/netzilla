...
{
	var agemin = $(idage_min)[0];
	var agemax = $(idage_max)[0];
...
function checkUsername(username)
{
	...
	{
		if((username.val().length>=6) && (username.val().length<=30))
			ajaxRequest('isUsername', 'username='+username.val(), '', 'isUsername', 'reportError');
		else
...
function checkUsername2(username)
{
	...
	{
		if((username.val().length>=6) && (username.val().length<=30))
			ajaxRequest('isUsername', 'username='+username.val(), '', 'isUsername2', 'reportError');
		else
			alert(usernameLength_alert);
	}
}
...
function isPhoneNumber(originalRequest)
{
	if(originalRequest.status == 200)
	{
		if(originalRequest.responseText > 0)
		{
			...
			jQuery("#phone_number_info")[0].innerHTML = '';
		...
		if(eval("data[n][1]"+data[n][2]+"data[n][3]"))
		{
			...
			if((data[n][0] != '') && (data[n][5] != ''))
			{
				if(data[n][5] != "accept_info")
					jQuery(data[n][5])[0].innerHTML = "<div class='error_info left'>"+data[n][4]+"</div>";
				else
					jQuery(data[n][5])[0].innerHTML = "<div class='error_info position-top'>"+data[n][4]+"</div>";
			}
			status = false;
		}
	}
...
function checkNullSelectOption(elm)
{
	...
	if(objVal == 0)
	{
		if((objName != '') && (objMsgElm != ''))
			jQuery(objMsgElm)[0].innerHTML = objMsgErr;
		return false;
	}
	else
	{
		jQuery(objMsgElm).innerHTML = '';
		return true;
	}
}
...
function setCountryCode(originalRequest)
{
	if(originalRequest.status == 200)
	{
		...
		{
			jQuery("country_code").innerHTML = '+' + originalRequest.responseText;
		}
		else
		{
			jQuery("country_code").innerHTML = '+';
...
function checkNullRadioOption(frm, elm, msgErr)
{
	...
	{
		if((objName != '') && (objMsgElm != ''))
			jQuery(objMsgElm).innerHTML = objMsgErr;
...
function checkNullPhone(code, code2, phone)
{
	...
	{
		jQuery("phone_number_info").innerHTML = mobile_alert;
	}
}
	...
	var mbox = new Ajax.Request("ajaxRequest.php",
									{
										...
										onComplete: function(originalRequest){
											if(originalRequest.status == 200)
											{
												...
												{
													jQuery("phone_number_info").innerHTML = country_alert;
													register_step_2_ok = false;
												}
											}
										}
...
function checkNullVerifyCode(mobileVerCode)
{
	...
	{
		jQuery("mobile_ver_code_info").innerHTML = mobile_ver_code_alert;
...
{
	if(originalRequest.status == 200)
		jQuery('message').innerHTML = originalRequest.responseText;
}
...
function checkUsernameSilent(username){
	if(checkNull(username)){
		if(username==old_username){
			...
			jQuery("username_info").innerHTML = "";
			username_ok = true;
		}
		else{
			if((username.length>=6) && (username.length<=30)){
				if(checkFormUserName(username)){
					...
					jQuery("username_info").innerHTML = "<div class='error_info left'>"+username_invalid_alert+"</div>";
				}
			}
			else{
				//jQuery("username_indicator").innerHTML="X";
				jQuery("username_info").innerHTML = "<div class='error_info left'>"+usernameLength_alert+"</div>";
				username_ok = false;
			}
		}
	}
		...
		//jQuery("username_indicator").innerHTML="X";
		jQuery("username_info").innerHTML = "<div class='error_info left'>"+can_not_empty+"</div>";
		username_ok = false;
	}
}
...
function checkUsernameSilentResult(originalRequest){
	if(originalRequest.responseText==0){
		...
		jQuery("username_info").innerHTML = "<div class='error_info left'>"+duplicate_nickname+"</div>";
		username_ok = false;
	}
	hideWaitingBox();
}
...
function checkEmailSilent(email)
{
	if(checkNull(email))
	{
		...
		{
			jQuery("email_info").innerHTML = "<div class='error_info left'>"+emailForm_alert+"</div>";
		}
	}
	else
	{
		jQuery("email_info").innerHTML = "<div class='error_info left'>"+email_alert+"</div>";
	}
}
...
{
	if(originalRequest.responseText==0)
		jQuery("email_info").innerHTML = "";
	else
		jQuery("email_info").innerHTML = "<div class='error_info left'>"+duplicate_email+"</div>";
}

function checkNullPassword(password)
{
	if((password.length>=6) && (password.length<=30))
		jQuery("password_info").innerHTML = "";
	else
		jQuery("password_info").innerHTML = "<div class='error_info left'>"+passwordLength_alert+"</div>";
}

function checkMatching(fstVal,sndVal)
{
	if(fstVal == sndVal)
		jQuery("confirm_password_info").innerHTML = "";
	else
		jQuery("confirm_password_info").innerHTML = "<div class='error_info left'>"+confirmpasswordMatch_alert+"</div>";
}

function checkAcept(acept)
{
	if(acept.checked !== false)
		jQuery("accept_info").innerHTML = "";
	else
		jQuery("accept_info").innerHTML = "<div class='error_info position-top' style='width: auto; margin-top: 5x; line-height:15px'>"+accept_alert+"</div>";
}
...
function setZodiac(originalRequest)
{
	...
	jQuery("zodiac_text").innerHTML = ZodiacArray[originalRequest.responseText];
	jQuery("zodiac_val").innerHTML = ZodiacArray[originalRequest.responseText];
}
...
{
	jQuery('resend_code_info').innerHTML = "";
	if(checkNullSignup3())
	{
...
{
	ajaxRequest("ajaxRequest.php", "action=getCurrentUserMobileNo", "", showVerifyMobileDialogCompleted, "");
}

function showVerifyMobileDialogCompleted(originalRequest)
{
	if((originalRequest.responseText==="Step2") || (originalRequest.responseText==="Step3"))
	{
		switch (originalRequest.responseText)
