...
<br class="clear" />
	...
	<label for="country" class="edit-profile-01">{#Country#}:<font class="request">*</font></label>
	<span>
		<select id="country" name="country" onchange="loadOptionState('#state', this.options[this.selectedIndex].value, '');loadOptionCity('#city', $('#state')[0][$('#state')[0].selectedIndex].value, '')" class="formfield_01"><option></option></select>
	</span>
	<br class="clear"/>
...
<div class="container-edit-profile-group">
	<h3 style="margin-bottom:5px;">{#Your#} {#Description#}:</h3>
	<span>
		<textarea id="description" name="description" columns="20" rows="12" style="width:450px; padding:10px;" onkeyup="checkNullTextArea(this, 'Bitte teilen Sie uns abit mehr über sich.')" onblur="checkNullTextArea(this, 'Bitte teilen Sie uns abit mehr über sich.')" class="description-edit">{$save.description|stripslashes}</textarea>	
	</span>
	<br class="clear"/>
</div>
 
<input type="hidden" name="save_profile" value="{$smarty.session.sess_id}" />
<a href="#" onclick="if(validateEditProfile()) jQuery('#editProfile').submit(); return false;" class="btn-search">{#SUBMIT#}</a>
<br class="clear" />
</form>
