...
<div style="line-height:18px; margin-bottom:20px;">{#reg2_headline_intro#}</div>
	...
	<form id="register_form" name="register_form" method="post" action="?action=incompleteinfo{if $smarty.get.action eq 'incompleteinfo_skip'}&nextstep=mobileverify_skip{else}&nextstep=mobileverify{/if}">	
		<label style="float:left; padding-top:5px;">{#mobile#}:</label>
		<span style="float:left;">
			<div id="country_code" style="float:left;padding-top:5px;width:40px;">+{$save.countrycode}</div>
		...
		<span style="margin-bottom:10px; float:left;">
			...
			<a href="#" onclick="submitAjaxFormIncompleteInfo(); return false;" class="btn-search" style="padding-left:20px; padding-right:20px;">{#reg2_submit#}</a>
			{if $smarty.get.action eq "incompleteinfo_skip"}
			<a href="./" onclick="" class="btn-search">{#reg2_skip#}</a>
			{else}
			<a href="#" onclick="jQuery('#mask').hide(); jQuery('.window').hide(); return false;" class="btn-search" style="padding-left:20px; padding-right:20px;">{#CANCEL#}</a>
			{/if}
		</span>
