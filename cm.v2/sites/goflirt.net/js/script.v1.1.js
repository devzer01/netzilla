...
function isEmailValid()
{
	...
	if (!checkFormEmail(email)) {
		jQuery("#email").validationEngine('showPrompt', emailForm_alert, 'error', 'topRight', true);
		return false;
	}
	return true;
}
...
function isPasswordMatch()
{
	...
	if (password == confirmpassword) {
		jQuery('#password').validationEngine('hide');
	} else {
		jQuery('#password').validationEngine('showPrompt', confirmpasswordMatch_alert, 'error', 'topRight', true);
	}
}
...
function checkNullSignup1()
{
	...
	var data = Array(
					...
					Array('#accept', accept, '==', 0, accept_alert,'#accept_info')
					);
...
function checkActionFocus2(data)
{
	...
	for(var n=0;n<data.length;n++) {
		jQuery(data[n][0]).validationEngine('hide');
	}
	...
	for(var n=0;n<data.length;n++)
	{
		if(eval("data[n][1]"+data[n][2]+"data[n][3]"))
		{
			...
			{
				jQuery(data[n][0]).validationEngine('showPrompt', data[n][4], 'error', 'topRight', true);
			}
			status = false;
		} 
	}
...
function getCountryCode(country)
{
	...
	{
		var mbox = jQuery.ajax({
						url: "ajaxRequest.php",
						type: 'post',
						data: {"action" : "getCountryCode", "country_id":country},
...
function checkNullPhone(code, code2, phone)
{
	...
	if(((code2.length + phone.length)>=9) && ((code2.length + phone.length)<=14))
	{
		...
		var mbox = jQuery.ajax({
									url: "ajaxRequest.php",
									type: 'post',
									data: {"action" : "isPhoneNumber", "phone_number":fullnumber},
									success: function(data)
									{
										isPhoneNumber();
									}
		...
		if(username==old_username){
			...
			if((username.length>=6) && (username.length<=30)){
				if(checkFormUserName(username)){
					ajaxRequest("ajaxRequest.php", "action=isUsername&username="+username, "", checkUsernameSilentResult, "");
				}
				else{
					jQuery('#username').validationEngine('showPrompt', username_invalid_alert, 'error', 'topRight', true);
				}
			}
			else{
				jQuery('#username').validationEngine('showPrompt', usernameLength_alert, 'error', 'topRight', true);
				username_ok = false;
			}
		}
	}
	else{
		jQuery('#username').validationEngine('showPrompt', can_not_empty, 'error', 'topRight', true);
		username_ok = false;
	}
}
...
function checkUsernameSilentResult(originalRequest){
	if(originalRequest.responseText==0){
		...
		jQuery('#username').validationEngine('showPrompt', duplicate_nickname, 'error', 'topRight', true);
		username_ok = false;
	}
}
...
function checkEmailSilent(email)
{
	if(checkNull(email))
	{
		if(checkFormEmail(email))
		{
			ajaxRequest("ajaxRequest.php", "action=isEmail&email="+email, "", checkEmailSilentResult, "");
		}
		else
		{
			jQuery('#email').validationEngine('showPrompt', emailForm_alert, 'error', 'topRight', true);
	...
	{
		jQuery('#email').validationEngine('showPrompt', email_alert, 'error', 'topRight', true);
	}
}
...
function checkEmailSilentResult(originalRequest)
{
	if(originalRequest.responseText==0)
		...
		jQuery('#email').validationEngine('showPrompt', duplicate_email, 'error', 'topRight', true);
}
...
function checkNullPassword(password)
{
	if((password.length>=6) && (password.length<=30))
		...
		jQuery('#password').validationEngine('showPrompt', passwordLength_alert, 'error', 'topRight', true);
}
...
function getZodiac(bDate, bMonth)
{
	...
	if(checkNull(birthDay))
	{
		var mbox = jQuery.ajax({
						url: "ajaxRequest.php",
						type: 'post',
						data: {"action" : "getZodiac", "bdate":birthDay},
						success: function(data)
						{
							setZodiac();
						}
...
function submitAjaxFormIncompleteInfo()
{
	...
	jQuery.ajax({
					url: "ajaxRequest.php",
					type: 'post',
					data: {"action" : "ajaxFormIncompleteInfo", "mobileNo":mobileNo},
					success: function(data)
					{
						if(data === "1")
						{
...
function submitAjaxFormMobileVerify()
{
	...
	if(checkNullSignup3())
	{
		...
		jQuery.ajax({
						url: "ajaxRequest.php",
						type: 'post',
						data: {"action" : "ajaxFormMobileVerify", "verCode":verCode},
						success: function(data)
						{
							if(data === "1")
							{
...
function submitAjaxFormWrongnumber()
{
	jQuery.ajax({
					url: "ajaxRequest.php",
					type: 'post',
					data: {"action" : "setNulCurrentUserMobileNo"},
					success: function(data)
					{
						if(data === "1")
						{
...
function submitAjaxFormResendVerify()
{
	jQuery.ajax({
					url: "ajaxRequest.php",
					type: 'post',
					data: {"action" : "ajaxFormResendVerify"},
					success: function(data)
					{
						if(data === "1")
						{
