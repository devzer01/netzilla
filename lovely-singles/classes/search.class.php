		...
		if($num > 0)
		{
			...
			if(isset($_SESSION['sess_id']))
			{
				if(defined(ENABLE_SMALL_FRANCHISE) && (ENABLE_SMALL_FRANCHISE=="1"))
				{
					...
					$franchisee = DBConnect::retrieve_value("SELECT agent FROM member WHERE username='".$rs['username']."'");
					$userfranchisee = DBConnect::retrieve_value("SELECT agent FROM member WHERE id='".$_SESSION['sess_id']."'");

					if($franchisee && ($userfranchisee==""))
					{
						DBConnect::execute_q("UPDATE member SET agent='".$franchisee."', agent_profile_username='".$rs['username']."' WHERE id=".$_SESSION['sess_id']);
						funcs::sendMessage($_SESSION['sess_id'],$rs['username'],'New customer connected to '.$franchisee.".",'New customer connected to '.$franchisee.".",3,true);
					}
				}
			}
	...
	static function simpleSearch($search){
		...
		foreach($trim_search as $field => $val){
			...
			switch($field){
				case 'city':
					if(is_numeric($val))
						$result_array[] = "city=$val";
					else
						$result_array[] = "city IN(SELECT id FROM xml_cities WHERE name like '$val%')";
					break;
				case 'state':
					if(is_numeric($val))
						$result_array[] = "state=$val";
					else
						$result_array[] = "state IN(SELECT id FROM xml_cities WHERE name like '$val%')";
					break;
				case 'min_age':
