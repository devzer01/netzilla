...
function getAllPayments($args)
{
	...
	return DBConnect::assoc_query_2D("SELECT m.username, l.price, l.currency, l.purchase_finished_date, l.payment_method, l.payment_type, m.agent FROM purchases_log l LEFT JOIN member m ON l.user_id = m.id WHERE l.purchase_finished=1 AND l.purchase_finished_date !='' AND DATE(l.purchase_finished_date)>='".$fromdate."' AND DATE(l.purchase_finished_date)<='".$todate."' ORDER BY l.purchase_finished_date");
}
