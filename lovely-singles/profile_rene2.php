...
for($j=0; $j<= $km_count; $j++){
	...
	$query = "SELECT * FROM user WHERE handynr = '' and pic!='nopic' and pic != '' and LENGTH(nick) > 5";
	...
	/*	$query_test = "SELECT * FROM member WHERE username='".$row['nick']."';";
		...
		if ($row['pic'] != 'nopic' && $row['pic'] != 'nopic.jpg'){
			$picpath = "";
			if(file_exists('/srv/www/htdocs/profiletemp/'.$koma[$j]['db'].'/images/'.$row['sex'].'/'.$row['pic']) && 
					!file_exists('/srv/www/htdocs/profiletemp/new/'.$row['pic'])){
				$check = copy('/srv/www/htdocs/profiletemp/'.$koma[$j]['db'].'/images/'.$row['sex'].'/'.$row['pic'],               '/srv/www/htdocs/profiletemp/new/'.$row['pic']);
	
			}elseif(file_exists('/srv/www/htdocs/profiletemp/'.$koma[$j]['db'].'/images/'.$row['sex'].'/'.$row['pic'].'.jpg') && !file_exists('/srv/www/htdocs/defaultprofile/cms_content/new/images'.$temp_name.'/'.$row['pic'])){
				$check = copy('/srv/www/htdocs/profiletemp/'.$koma[$j]['db'].'/images/'.$row['sex'].'/'.$row['pic'].'.jpg',               '/srv/www/htdocs/profiletemp/new/'.$row['pic'].'.jpg');	
}
...
/*		$plz ='';
		...
		$sql_double = "SELECT username FROM herzoase.member WHERE username = '".$row['nick']."'";
		$result_double = mysql_query($sql_double); 
		$result_double_row = mysql_fetch_row($result_double);
		if (!$result_double_row[0]){
					$sql_double2 = "SELECT username FROM  member WHERE username = '".$row['nick']."'";
		$result_double2 = mysql_query($sql_double2); 
		$result_double_row2 = mysql_fetch_row($result_double);
		if (!$result_double_row2[0]){
		...
		$query2 = "INSERT INTO member (username,password,email,gender,birthday,country,state,city,fake,area,height,weight,eyescolor,haircolor,type,lookmen,lookwomen,description,picturepath,mobileno,ursprung) 
			Values('".$row['nick']."','test721225user','".$zufall_mail."',".(($row['sex']=='m')?1:2).",'".$geb."','".$country."','".$state."','".$city."',1,'".$plz."','".$row['groesse']."','".$row['gewicht']."',".$eyes.",".$hair.",".rand(2, 4).",".(($row['searchSex']=='Mann'||$row['searchSex']=='Egal')?1:0).",".(($row['searchSex']=='Frau'||$row['searchSex']=='Egal')?1:0).",\"".$row['flirttext']."\",'".$picture."','".$zufall."','".$koma[$j]['db']."');";
	$result2 = mysql_query($query2);
		echo "fehlender EINTRAG ($i): ".$row['nick']."<br>";
		echo $query2."<br>";

		$i++;	
		}else {
			//echo "DOPPELTER EINTRAG ($i2): ".$row['nick']."<br>";
			//$i2++;				
		}
		}else {
			//echo "DOPPELTER EINTRAG ($i2): ".$row['nick']."<br>";
