...
function getPackageFromTransactionID($transaction_id,$for_redirect)
{
	//if($_SERVER['REMOTE_ADDR']==PAYMENT_SERVER_IP)
	{
		...
		{
			if($result = DBConnect::assoc_query_1D("SELECT pl.*,m.username, m.email FROM purchases_log pl LEFT JOIN member m ON pl.user_id=m.id WHERE pl.id=".$transaction_id))
			{
...
function finishPurchase($transaction_id, $saleID, $payment_method, $payment_type, $postdata=array())
{
	//if($_SERVER['REMOTE_ADDR']==PAYMENT_SERVER_IP)
	{
		if(DBConnect::retrieve_value("SELECT id FROM purchases_log WHERE id=".$transaction_id." AND purchase_finished=0"))
		{
			...
			if(DBConnect::execute_q("UPDATE purchases_log SET purchase_finished=1, reference_id='".$saleID."', payment_method='".$payment_method."', payment_type='".$payment_type."', purchase_finished_date=NOW(), postdata_serialized='".serialize($postdata)."', postdata_text='".print_r($postdata,true)."' WHERE id=".$transaction_id))
			{
...
function chargeback($transaction_id, $ref)
{
	if(DBConnect::retrieve_value("SELECT id FROM purchases_log WHERE id=".$transaction_id." AND reference_id='".$ref."' AND purchase_finished=1"))
	{
		if(DBConnect::execute_q("UPDATE purchases_log SET purchase_finished=0 WHERE id=".$transaction_id." AND reference_id='".$ref."'"))
		{
			//get transaction detail
			$detail = DBConnect::assoc_query_1D("SELECT * from purchases_log WHERE id=".$transaction_id);
...
function getCountPayments($args, $expect_times, $expect_topup)
{
	...
	//$sql3 = "SELECT m.username FROM purchases_log l LEFT JOIN member m ON l.user_id = m.id WHERE l.purchase_finished=1 AND l.purchase_finished_date !='' AND purchase_finished_date !='0000-00-00 00:00:00' AND DATE(l.purchase_finished_date)>='".$fromdate."' AND DATE(l.purchase_finished_date)<='".$todate."' GROUP BY m.username ";
...
function addCoinsPaypal($post)
{
	if(isset($post['coins']))
	{
		...
		{
			$error .= "Please enter valid coins.";
		}

		if(!$error)
		{
			DBConnect::execute_q("UPDATE member SET coin=coin+".$post['coins']." WHERE username='".$username."'");

			//get current coin value
			$coinVal = funcs::checkCoin($username);

			//insert coin log
			$sqlAddCoinLog = "INSERT INTO coin_log (member_id, send_to, coin_field, coin, coin_remain, log_date) VALUES ('0','".$user_id."','payment',".$post['coins'].",".$coinVal.", NOW())";
			DBconnect::execute($sqlAddCoinLog);
			...
			DBConnect::execute_q("INSERT INTO purchases_log (user_id,package_id,price,coin_amount,currency,purchase_datetime, ip, purchase_finished, payment_method, payment_type, purchase_finished_date) VALUES (".$user_id.",0,50,".$post['coins'].",'EUR',NOW(),'".$_SERVER['REMOTE_ADDR']."', 1, 'Paypal', 'Manual', NOW())");
