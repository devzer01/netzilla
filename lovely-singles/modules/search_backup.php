...
if($username != '')
{
	$profileDetails = Search::searchName($username, $next, MESSAGE_RECORD_LIMIT); //limit 15
...
// print_r($memrec_data);	
	...
	elseif($search_type == 2)//search profile
	{
		...
		//$profileDetails = Search::geProfile($username, $gender, $have_pic, $country, $state, $city, $min_age, $max_age, $next, PROFILE_SEARCH_MAX-$next, $_SESSION[sess_id]); // MESSAGE_RECORD_LIMIT
		...
		$profileDetails = Search::geProfile($username, $gender, $have_pic, $country, $state, $city, $min_age, $max_age, $next, MESSAGE_RECORD_LIMIT, $_SESSION[sess_id]);
		$numProfile = count($profileDetails);
		...
		if ($memrec_total < PROFILE_SEARCH_MIN) //PROFILE_SEARCH_MIN = 15
		{
			...
			if(isset($_SESSION['sess_id']) && $_SESSION['sess_id'] > 0 && count($profileDetails) < MESSAGE_RECORD_LIMIT)
			{
				//login
				if($numProfile > 0 && count($profileDetails) > 0 && is_array($profileDetails))
				{
			...
			{
				//not login
				if(count($profileDetails) < MESSAGE_RECORD_LIMIT)
				{
					//if no result
					$noResult = 1;
