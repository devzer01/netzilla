...
define('MESSAGE_HISTORY_PERIOD', '5 WEEK');
...
function getAllContact($userid)
{
	...
	$inbox = DBconnect::assoc_query_2D("SELECT 'inbox' as type, CASE WHEN i.from_id = 1 THEN 'System Admin' ELSE m.username END AS username , m.id, m.picturepath, i.datetime, i.status FROM message_inbox i LEFT JOIN member m ON i.from_id=m.id WHERE i.to_id=".$userid." $sql_current_username $sql_message_history_preiod_inbox ORDER BY i.datetime DESC");

	$outbox = DBconnect::assoc_query_2D("SELECT 'outbox' as type, CASE WHEN o.to_id = 1 THEN 'System Admin' ELSE m.username END AS username , m.id, m.picturepath, o.datetime, o.status FROM message_outbox o LEFT JOIN member m ON o.to_id=m.id WHERE o.from_id=".$userid." $sql_current_username $sql_message_history_preiod_outbox ORDER BY o.datetime DESC");
...
function getMessages($userid, $username, $part)
{
	...
	$inbox = DBconnect::assoc_query_2D("SELECT CASE WHEN i.from_id = 1 THEN 'System Admin' ELSE m.username END AS username, m.picturepath, i.* FROM message_inbox i LEFT JOIN member m ON i.from_id=m.id WHERE i.from_id=".$from." AND i.to_id=".$userid.$sql_message_history_preiod_inbox);

	//$outbox = array();
	$outbox = DBconnect::assoc_query_2D("SELECT CASE WHEN o.from_id = 1 THEN 'System Admin' ELSE m.username END AS username, m.picturepath, o.* FROM message_outbox o LEFT JOIN member m ON o.from_id=m.id WHERE o.from_id=".$userid." AND o.to_id=".$from.$sql_message_history_preiod_outbox);
