...
function checkNullSignupBox()
{
	...
	return checkActionFocusBox(data);
}
...
function checkActionFocusBox(data)
{
	var status = true;
	for(var n=0;n<data.length;n++)
	{
		if(eval("data[n][1]"+data[n][2]+"data[n][3]"))
		{
			...
			if((data[n][0] != '') && (data[n][5] != ''))
			{
				$(data[n][5]).innerHTML = "<div class='error_info-box'>"+data[n][4]+"</div>";
			}
			status = false;
		}
	}
...
function checkUsernameSilentForBox(username){
	if(checkNull(username)){
		if(username==old_username){
			//$("username_indicator").innerHTML="OK";
			$("username_info").innerHTML = "";
			...
			if((username.length>=6) && (username.length<=30)){
				if(checkFormUserName(username)){
					var mbox = new Ajax.Request("ajaxRequest.php", {method: "post", parameters: "action=isUsername&username="+username, onComplete: checkUsernameSilentResultForBox});
				}
				else{
					$("username_info").innerHTML = "<div class='error_info-box'>"+username_invalid_alert+"</div>";
				}
			}
			else{
				$("username_info").innerHTML = "<div class='error_info-box'>"+usernameLength_alert+"</div>";
				username_ok = false;
			}
		}
	}
		...
		//$("username_indicator").innerHTML="X";
		$("username_info").innerHTML = "<div class='error_info-box'>"+can_not_empty+"</div>";
		username_ok = false;
	}
}
...
function checkUsernameSilentResultForBox(originalRequest){
	if(originalRequest.responseText==0){
		$("username_info").innerHTML = "";
		username_ok = true;
	}
	else{
		$("username_info").innerHTML = "<div class='error_info-box'>"+duplicate_nickname+"</div>";
		username_ok = false;
	}
	hideWaitingBox();
}
...
function checkEmailSilentForBox(email)
{
	if(checkNull(email))
	{
		if(checkFormEmail(email))
		...
		{
			$("email_info").innerHTML = "<div class='error_info-box'>"+emailForm_alert+"</div>";
		}
	}
	else
	{
		$("email_info").innerHTML = "<div class='error_info-box'>"+email_alert+"</div>";
	}
}
...
function checkNullPassword(password)
{
	if((password.length>=6) && (password.length<=30))
		...
		$("password_info").innerHTML = "<div class='error_info-box'>"+passwordLength_alert+"</div>";
}

function checkNullPasswordForBox(password)
{
	if((password.length>=6) && (password.length<=30))
		$("password_info").innerHTML = "";
	else
...
function checkMatching(fstVal,sndVal)
{
	if(fstVal == sndVal)
		...
		$("confirm_password_info").innerHTML = "<div class='error_info-box'>"+confirmpasswordMatch_alert+"</div>";
}

function checkMatchingForBox(fstVal,sndVal)
{
	if(fstVal == sndVal)
		$("confirm_password_info").innerHTML = "";
	else
...
function checkAceptForBox(acept)
{
	if(acept.checked !== false)
		$("accept_info").innerHTML = "";
	else
		$("accept_info").innerHTML = "<div class='error_info-box'>"+accept_alert+"</div>";
}
...
jQuery(function() {
	notificationSoundElement = loadSound("sounds/eyes-on-me.mp3");
});
