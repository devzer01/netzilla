...
$cur_date = $_GET['date']=''?date("Y-m-d"):$_GET['date'];
echo $duration = funcs::dateDiff("-", date("Y-m-d"), $cur_date);

echo "<pre>Herzoase.com => [{$cur_date}]\n";
...
$sql = "SELECT ID, username FROM payment_log WHERE reminder_date = '{$cur_date}' AND recall = '0'";
...
$list = DBConnect::assoc_query_2D("SELECT * from member WHERE DATE(payment) = ('{$cur_date}' - INTERVAL 1 DAY) AND (prolonging_payment_id <> 0 OR prolonging_payment_id <> null)");
if($list)
{
	echo "==================================================================================================================\n\n";
	...
	{
		$subject = 'Mitglieschaft ausgelaufen - keine Zahlung eingegangen';
...
$list = getExpiredMembership(-1-$duration);
if($list)
{
	echo "==================================================================================================================\n\n";
...
$list = getExpiredMembership(6-$duration);
if($list)
{
	echo "==================================================================================================================\n\n";
...
if(!$testing)
{
	if(count($arr1) > 0)
	{
		$subject = '['.$cur_date.'] Herzoase Mitgliedschaft verlängert - Info';
	...
	if(count($arr2) > 0)
	{
		$subject = '['.$cur_date.'] Herzoase Mitgliedschaft verlängert - ELV Buchung vornehmen';
