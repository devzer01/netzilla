...
{else}
	<form id="register_form" name="register_form" method="post" action="?action=incompleteinfo{if $smarty.get.action eq 'incompleteinfo_skip'}&nextstep=mobileverify_skip{else}&nextstep=mobileverify{/if}">	
		...
		<span>
			...
			<a href="javascript: void(0)" onclick="submitAjaxFormIncompleteInfo()" class="button">{#reg2_submit#}</a>
			{if $smarty.get.action eq "incompleteinfo_skip"}
			<a href="./" onclick="" class="button">{#reg2_skip#}</a>
			{/if}
		</span>
	...
	{/if}
{else}
{#phone_number_not_support#}<!--Wir bedauern, aber momentan unterstützt unser System den Versand von SMS aus deinem Land noch nicht. Du kannst aber mit anderen Nutzern direkt über diese SEite in Kontakt treten.--><br/><br/>
{/if}