...
<a href="?action=mymessage&type=inbox" class="butsearch active">{#MESSAGES#}</a>
{else $submenu eq ""}
	<a href="?action=mymessage&type=inbox" class="butsearch">{#MESSAGES#}</a><div id="new_msg" style="position:relative; top:-30px; float:right; height:0px"></div><!--<font style="display:block-inline; width:auto; float:left; margin-left:8px;"></font> -->
{/if}
{if $submenu eq "mymessage"}
<a href="?action=mymessage&type=inbox" class="butsearchsub">{#Inbox#}</a>
<a href="?action=mymessage&type=outbox" class="butsearchsub">{#Outbox#}</a>
<a href="?action=mymessage&type=writemessage" class="butsearchsub">{#Send#}</a>
<a href="?action=mymessage&type=archive" class="butsearchsub">{#Archive#}</a>
<a href="?action=sendcard" class="butsearchsub">{#ECards#}E-Cards</a>
{else}
{/if}
