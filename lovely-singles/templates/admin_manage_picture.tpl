...
<h1>Manage Pictures</h1>
	<div class="result-box-inside-nobg">
		...
		{foreach from=$list item="item"}
			...
			<div class="picture-username"><input type="checkbox" name="id[]" value="{$item.id}"/> <a href="?action=viewprofile&username={$item.username}&from=admin" class="link">{$item.username}</a></div>
			</div>
		{/foreach}
		<br clear="both"/><br/>
		<div class="page">
