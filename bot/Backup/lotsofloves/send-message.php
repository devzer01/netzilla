...
$loginURL = "http://www.lotsofloves.com/profile/login";
$loginRefererURL = "http://www.lotsofloves.com/";
$searchURL = "http://www.lotsofloves.com/search/results/";
$searchReferer = "http://www.lotsofloves.com/search/";
$searchResultsPerPage = 20;
$sendMessageURL = "http://www.lotsofloves.com/mail/compose/";
$sendMessageRefererURL = "http://www.lotsofloves.com/mail/compose/";

$receiverInboxURL = "http://www.lotsofloves.com/mail/";
...
else
{
	...
	$_POST = array(		"start_time" => "15:34:00",
						"end_time" => "18:35:00");
	$command = array(
						"profiles" => array(
												array(
														"username" => "silentlife94",
														"password" => "mi2mi09"
													)
											),
						"messages" => array(
...
elseif(isset($command['action']) && ($command['action']=='check'))
{
	...
	$profiles = DBConnect::assoc_query_2D("SELECT id, username, password FROM user_profiles WHERE site_id =".SITE_ID." AND status!='false'");
	if(is_array($profiles) && count($profiles))
	{
		funcs::savelog(count($profiles)." profiles to check.");
...
}elseif(isset($command['action']) && ($command['action']=='search'))
{
	...
	if($post['country']==1)
		$counties = range(1,58,1);
	elseif($post['country']==2)
		$counties = range(59,71,1);
	elseif($post['country']==3)
		$counties = range(72,140,1);
	else
		$counties = array("");
	...
	foreach($counties as $county)
	{
		if(funcs::memberlogin($command['profiles'][$current_profile]['username'], $command['profiles'][$current_profile]['password'], $loginURL, $loginRefererURL, $current_profile, $total_proflies))
		{
			...
			do
			{
				funcs::savelog("Search country: ".$post['country'].", county: ".$post['county'].", page: ".$page);
...
elseif(isset($command['action']) && ($command['action']=='send'))
{
	...
	if(($_POST['start_time']!="00:00:00") && ($_POST['end_time']!="00:00:00"))
	{
		...
		while($member=funcs::getNextMember($command))
		{
			...
			$randomSubject = $command['messages'][$current_subject]['subject'];
