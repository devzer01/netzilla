...
if($_GET['action']=="view_log")
{
	...
	else
	{
		...
		echo '<p style="text-align: center;">'
			,'<a href="index.php?action=stop&id=',$command['id'],'">STOP Bot</a></p>';
		echo "<pre>";
...
elseif($_GET['action']=="stoppreset")
{
	...
	if($preset != "")
	{
		...
		$arrCommands = array();
		$row = array();
		$data = explode('_',$preset);
		$target = (empty($data[0])) ? '' : $data[0];
		
		if(in_array($target, array('DE','UK','NICK')))
		{
			switch($data[1]) {
				case 1: // Stop ALL
					$sql = "SELECT commands.* FROM commands WHERE commands.site IN (
						SELECT id FROM sites WHERE target = '".$target."'
					) AND commands.status = 'true' AND preset != ''";
					break;
				case 3: // Stop Not Start
					$sql = "SELECT commands.* FROM commands WHERE commands.site IN (
						SELECT id FROM sites WHERE target = '".$target."'
					) AND commands.status = 'true' AND commands.run_count = 0 AND preset != ''";
					break;
...
elseif($_GET['action']=='stopmarket') {
	$stop_target = isset($_GET['stop_target']) ? $_GET['stop_target']:"";
	$data = explode('_',$stop_target);
	$target = $data[0];
	if(in_array($target, array('DE','UK','NICK')))
	{
		switch($data[1]) {
			case 1: // Stop ALL
				$sql = "SELECT commands.* FROM commands WHERE commands.site IN (
					SELECT id FROM sites WHERE target = '".$target."'
				) AND commands.status = 'true'";
				break;
			case 3: // Stop Not Start
				$sql = "SELECT commands.* FROM commands WHERE commands.site IN (
					SELECT id FROM sites WHERE target = '".$target."'
				) AND commands.status = 'true' AND commands.run_count = 0";
				break;
			default: // Stop RUNNING
				$sql = "SELECT commands.* FROM commands WHERE commands.site IN (
					SELECT id FROM sites WHERE target = '".$target."'
	...
	if($result){
		...
		if(count($arr))
		{
			...
			$masklist = '';
						...
						{
							$masklist .= "<option value='$mask_url[name]' ".(($i == 1) ? 'selected="selected"' : '').">$mask_url[name]</option>";
						}
			...
			foreach($arr as $item)
			{
				...
				$grpname = preg_replace('/[^a-zA-Z0-9]/', '', $item['group_name']);
				...
				echo "<td>
						...
						<form onsubmit='runPresetGroup(jQuery(this).serialize()); return false;'>
							...
							<input type='checkbox' name='change_time' checked='checked' value='1'/> change time?<br/>
				...
				for($i=0;$i<24;$i++){ if($i<10) $txt = "0".$i; else $txt = $i;?>
					<option value="<?php echo $txt;?>" <?php if($i == 23){ echo "selected='selected'";} ?>><?php echo $txt;?></option>
				<?php }
				echo		"</select>
							to: <select name='end_h'>";
				for($i=0;$i<24;$i++){ if($i<10) $txt = "0".$i; else $txt = $i;?>
					<option value="<?php echo $txt;?>" <?php if($i == 22){ echo "selected='selected'";} ?>><?php echo $txt;?></option>
				<?php }
				echo	"</select><br />
						Target CM <br />"
						,'<select name="target_cm" id="target_'.$grpname.'" onchange="javascript:bot.mask(\''.$grpname.'\');">';
						...
						echo '</select>'
						,'<br />New URL? <br /><select name="mask_url" id="mask_'.$grpname.'">';
						echo "<option value=''>Not change</option>".$masklist;
			...
			if($return_message == "")
			{
				...
				foreach($servers as $server)
				{
					...
					if(isset($result['eafem']) && ($result['eafem']=="1"))
					{
						...
						if($result['age_to']<24)
						{
							$result['profile_type']=1;
						}
						elseif($result['age_to']<31)
						{
							$result['profile_type']=2;
						}
						else
						{
							$result['profile_type']=3;
						}
					}
					...
					if((int) $result['start_h'] > 23){
						$result['start_h'] = sprintf("%02d",($result['start_h'] - 24));
					}
...
$(document).ready(function() {
	...
	$("#savevcard").click(function (e) {
		e.preventDefault();

		var info = $("#vcard").val();
		var info2 = $("#vcard2").val();
		var site_id = $("#vcard_site_id").val();
		
		$.ajax({
				url: 'json.php?action=savevcard',
				data: {info: info, info2: info2, site_id: site_id},
				type: 'post',
				dataType: 'json',
				success: function (json) {
					if (json.code == 1) {
						alert("Vcards save ok");
					} else {
						alert("Vcards didn't save");
					}
				}
		});
	});
	...
	$("#site").change(function (e) {
		e.preventDefault();
		$.get("json.php?action=vcardh&id=" + $(this).val(), function (data) {
			$("#vcard").val(data);
		});
		$.get("json.php?action=vcardh2&id=" + $(this).val(), function (data) {
			$("#vcard2").val(data);
		});		
		$("#vcard_site_id").val($(this).val());
	...
	$("#resetvcard").click(
		function (e) {
			e.preventDefault();
			var site_id = $("#vcard_site_id").val();
			$("#vcard").load("json.php?action=vcardh&id=" + site_id);
			$("#vcard2").load("json.php?action=vcardh2&id=" + site_id);
		}
	);
});
			...
			for(i=machine_from; i<=machine_to; i++)
			{
				...
				//if($("input[name='profiles[]']:checked").length>0)
				{
					if($('#eafem').attr('checked'))
					{
						...
						if(parseInt($('#age_to').val())<24)
						{
							$('#profile_type').val(1);
						}
						else if(parseInt($('#age_to').val())<31)
						{
							$('#profile_type').val(2);
						}
						else
...
function getMsgArea(){
	site = parseInt($("select[name=site]").val());
	var login_by = $("select[name=login_by]").val();
	var site1 = [5,10,35,39,46,50,51,54,55,56,57,58,59,60,63,66,67,68,69,71,73,78];
	var site2 = [5,10,39,46,50,51,54,55,56,57,58,59,60,63,66,67,68,69,71,73,78];
	// Do not set if site id > 84
	console.log('+++> Site : '+site+' / Index : '+$.inArray(site, site1));
	if($.inArray(site, site1) != -1 || site > 84){
		$("#messages_area").show();

		if($.inArray(site, site2) != -1 || site > 84){
			if(login_by == 1){
				$("#repeat_area").show();
			}else{
				$("#repeat_area").hide();
...
function getRepeatProfile(){
	site = parseInt($("select[name=site]").val());
	var site_ids = [5,10,35,39,46,50,51,54,55,56,57,58,59,60,63,66,67,68,69,71,73,78];
	if(($.inArray(site, site_ids) != -1) || site > 84){
		$("#repeat_area").show();
	}else{
		$("#repeat_area").hide();
	}
...
/*function getRepeatProfile(){
	...
	if(site == 35 || site == 55 || site == 59 || site == 68 || site == 60 || site == 67 || site == 56 || site == 5 || site == 80 || site == 54 || site > 84){
			if(login_by == 1){
				$("#repeat_area").show();
			}else{
				$("#repeat_area").hide();
...
<body>
<div style='width: 100%;'>
<div style="width: 815px; margin: 0px 10px 10px 10px; float:left">
...
<div id='rightside' style='float: left;'>
	<div style='margin-top: 400px; margin-left: 50px; float: left; border: 1px solid black; width: 450px; height: 350px; overflow: auto;'>
		<textarea style='width: 100%; height: 100%;' id='vcard'></textarea>
	</div>
	<div style='margin-top: 400px; margin-left: 20px; float: left; border: 1px solid black; width: 450px; height: 350px; overflow: auto;'>
		<textarea style='width: 100%; height: 100%;' id='vcard2'></textarea>
	</div>
	<input type='hidden' name='vcard_site_id' id='vcard_site_id' value='0' />
	<div style='float: left; clear: both; margin-top: 20px; margin-left: 150px;'>
		<button type='button' name='savevcard' id='savevcard'>Save</button>
		<button type='button' name='resetvcard' id='resetvcard'>Reset</button>
	</div>
</div>
</div>
<div style='width: 100%;'>
...
<script type="text/javascript">
	var bot = {
		mask: function(name) {
			var val = $("#target_"+name).val();
			$.ajax({
				type: "POST",
				url: 'ajax.php',
				data: { section: 'getMaskURL', target: val},
				success: function(data) {
					$('#mask_'+name).find('option').remove().end().append(data);
				}
			});
		}
	}
