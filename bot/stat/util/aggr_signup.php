...
function getSignupStats($start = null, $finish = null) {
	...
	foreach ($servers as $server) {
		try
		{
			...
			if (isset($statisticsResult['signupRef'])) {
				$json = json_decode($statisticsResult['signupRef'], true);
				
				$sql = "DELETE FROM signup_redirect WHERE report_date = :report_date AND portal_id = :portal_id ";
				$sth = $pdo->prepare($sql);
				$sth->execute(array(':report_date' => $fromdate, ':portal_id' => $server['ID']));
				
				foreach ($json as $j) {
					$sql = "INSERT INTO signup_redirect (report_date, portal_id, count, host, mobile) "
					     . "VALUES (:report_date, :portal_id, :count, :host, :mobile) ";
					$sth = $pdo->prepare($sql);
					$sth->execute(array(':report_date' => $fromdate, ':portal_id' => $server['ID'], ':count' => $j['cnt'], ':host' => $j['ref'], ':mobile' => $j['mobile']));
				}
			}
		}
		catch (SoapFault $e)
