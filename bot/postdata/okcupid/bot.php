	...
	public function login()
	{
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				$content = $this->getHTTPContent($this->loginURL, $this->loginRefererURL, $cookiePath, $this->loginArr[$this->currentUser], array(
					'X-Requested-With: XMLHttpRequest'
				));
				$json = json_decode($content);
				
				$this->savelog('Bot ID : '.$this->commandID);
				switch($json->status) {
					case 0:
						$login = $this->loginArr[$this->currentUser];
						unset($login['okc_api']);
						$content = $this->getHTTPContent($this->loginURL, $this->loginRefererURL, $cookiePath, $login);
						...
						break;
					case 104:
						$this->savelog("Profile ".$username." failed to login incorrect username/password");
						$this->setProxy();
						$this->getNewProfile(TRUE);
						return false;
						break;
					case 107:
						$this->savelog("IP Address Blocked");
						$this->setProxy();
						$this->getNewProfile(TRUE);
						return false;
						break;
					default:
						$this->savelog("Unidentified Error");
						return false;
						break;
				}
			}
	...
	public function getProxyip() {
		switch($this->command['proxy_type']){
			case 1:
				...
				break;
			case 2:
				$data = json_decode(file_get_contents('http://192.168.1.253/bot/proxy/country/uk'));
				return array(
					'proxy_ip' => $data->host,
