...
class Ahano extends bot
{
	public function ahano($post)
	{
		...
		$this->databaseName = "ahano";
		$this->userhash = "";

		//Login
			$this->usernameField = "s_nickname";
			$this->indexURL = "http://www.ahano.de/";
			$this->indexURLLoggedInKeyword = "Logout";
			$this->loginURL = "http://www.ahano.de/?kom=15&u_kom=31&loggin=1";
			$this->loginRefererURL = "http://www.ahano.de/?kom=15&u_kom=31";
			$this->loginRetry = 3;
			$this->logoutURL = "http://www.ahano.de/?kom=15&u_kom=31&logout=1";
		
		//Search
			$this->searchIndex = "http://www.ahano.de/?kom=18&u_kom=138";
			$this->searchURL = "http://www.4crazy.de/mitglieder/suche/";
			$this->searchRefererURL = "http://www.ahano.de/?kom=18&u_kom=138";			
			
			$this->searchResultsPerPage = 10;		
		
		//Message
			$this->messageURL = "http://www.ahano.de//inc/profile/aktivescript/nachricht_auswerten.inc.php";
	...
	public function addLoginData($users)
	{
		foreach($users as $user)
		{
			$login_arr = array(
				...
				"a_u_u_kom" => "",
				"beitrag_posie" => 1,
				"s_nickname" => $user["username"],
				"s_passwort" => $user["password"]
			);	
	...
	{		
		if($this->countMessage < 20){
			...
			while($endloop == false)
			{
				$content = $this->getHTTPContent("http://www.ahano.de/?kom=18&u_kom=138&mitliste_suchen=1&suche_geschlecht=1&suche_bild=0&interessen_schlagwoerter=&interessen_und=1&geo_id=&liste_hobbies=&Sozio_4_Filter=&Sozio_6_Filter=0&Sozio_1_Filter=0&Sozio_9_Filter=&suche_geordnet=".$this->command['gender']."&Anzahl=20&seite_liste=".$pages."#sefach", $this->searchRefererURL, $cookiePath);

				if(strpos($content, 'keine Mitglieder gefunden') !== false){
					$endloop = true;
				}else{
	...
	private function getMembersFromSearchResult($content){
		...
		if($html = str_get_html($content)){
			$nodes = $html->find("p.listeueber a");
			foreach ($nodes as $node) {								
							
				$vowels = array("&nbsp;");
				$name = trim(str_replace($vowels,"",$node->plaintext));
							
				$uid = $node->innertext;
				$uid = substr($uid, strpos($uid, 'favoausgabe_')+12);
	...
	private function work_visitProfile($username, $item, $cookiePath)
	{
		...
		$content = $this->getHTTPContent("http://www.ahano.de/inc/forum/profile.php?userid=".$item["uid"], $this->searchIndex, $cookiePath);
		$this->sleep(5);
		return $content;
	}
			...
			if($this->command['msg_type'] == "pm")
			{
				...
				{				
					...
					{				
						...
						$content = $this->getHTTPContent($this->messageURL, "http://www.ahano.de/community/mitglieder/43812/geli_huber/", $cookiePath, $param, $headers);

						$url_log = "URL => ".$this->messageURL."\nREFERER => http://www.ahano.de/community/mitglieder/43812/geli_huber/\n";
				...
				if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
				{
					...
					if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
					{
						...
						$headers = array('Content-Type: application/x-www-form-urlencoded','X-Requested-With: XMLHttpRequest');
						$content = $this->getHTTPContent("http://www.ahano.de//inc/profile/aktivescript/nachricht_auswerten.inc.php", "http://www.ahano.de/community/mitglieder/43812/geli_huber/", $cookiePath, $param);							
						file_put_contents("sending/gb-".$username."-".$item['username'].".html",$content);
							
						if(strpos($content, '"fehlermeldung":""') !== false)
						{
							$this->newMessage=true;
	...
	{
		$content = $this->getHTTPContent("http://www.ahano.de/?kom=16&u_kom=135", $this->loginRefererURL, $cookiePath);

		if($html = str_get_html($content)){					
					
			$nodes = $html->find("a.admin");	
					
			foreach($nodes as $node){
				if($node->innertext == "l&ouml;schen"){
					$msgArr = substr($node->outertext, strpos($node->outertext, "postaus(")+8);
					$msgArr = substr($msgArr, 0, strpos($msgArr, ")"));
					
					list($aktion,$msgid) = @explode(",",$msgArr);
					
					$vowels = array("'");
					$msgid = trim(str_replace($vowels,"",$msgid));
					$aktion = trim(str_replace($vowels,"",$aktion));
					...
					if($msgid != "")
					{
						...
						$content = $this->getHTTPContent("http://www.ahano.de/inc/postfach_2/postfach_2_loeschen_ajax.inc.php", "http://www.ahano.de/?kom=16&u_kom=135", $cookiePath, $param, $headers);
						$this->savelog("Delete message: ".$msgid);
					}
				}
			}
		}
