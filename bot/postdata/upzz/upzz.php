...
class Upzz extends bot
{
	public function upzz($post)
	{
		...
		$this->databaseName = "upzz";
		$this->userhash = "";

		//Login
			$this->usernameField = "nickname";
			$this->indexURL = "http://www.upzz.com/portal/index.html";
			$this->indexURLLoggedInKeyword = "Logout";
			$this->loginURL = "http://www.upzz.com/portal/login.html";
			$this->loginRefererURL = "http://www.upzz.com/portal/index.html";
			$this->loginRetry = 3;
			$this->logoutURL = "http://www.upzz.com/portal/logout.html";
		
		//Search
			$this->searchIndex = "http://www.upzz.com/index.php?s=online&p=0&ajax=Y";
		...
		//Message
			$this->messageURL = "http://www.upzz.com/portal/online.html";
			$this->messageReferer = "http://www.goolive.de/messenger/nachricht-schreiben-688304";
			$this->inboxURL = "http://www.upzz.com/portal/nachrichten/inbox.html";
			$this->outboxURL = "http://www.upzz.com/portal/nachrichten/outbox.html";
		...
		if($this->countMessage < $this->limitMessage){
			...
			while($endloop == false){				
				...
				if($this->countMessage < $this->limitMessage){
					...
					$content = $this->getHTTPContent("http://www.upzz.com/portal/search.html?do=1&land=".$this->command['land']."&geschlecht_s=".$this->command['gender']."&alter_von=".$this->command['ageStart']."&alter_bis=".$this->command['ageEnd']."&plz=&x=59&y=13", "http://www.upzz.com/portal/online.html?fbqp=1", $cookiePath);
					//$content = $this->getHTTPContent("http://www.upzz.com/index.php?s=search&do=1&c=1&p=".$pages."&ajax=Y", $this->loginRefererURL, $cookiePath);
					
					//$content = $this->getHTTPContent("http://www.upzz.com/portal/search.html?do=1&land=de&geschlecht_s=w&alter_von=18&alter_bis=99&plz=01067&x=55&y=11", "http://www.upzz.com/portal/online.html?fbqp=1", $cookiePath);
					$search_arr = array(
						"ajax" => "Y",
						"c" => 1,
						"do" => 1,
					...
					$content = $this->getHTTPContent("http://www.upzz.com/index.php?s=online&do=1&c=1&g=&p=".$pages."&ajax=Y", "http://www.upzz.com/portal/online.html", $cookiePath, $search_arr, $headers);

					//echo $content;exit;

					if(strpos($content, 'Es wurden leider keine Mitglieder gefunden') !== false)
					{
	...
	private function getMembersFromSearchResult($content){
		...
		if($html = str_get_html($content)){
			$nodes = $html->find("div.opcntun a");

			foreach ($nodes as $e) {
									
				$name = trim($e->innertext);
				$vowels = array("/portal/profil/","/$name.html");
				$uid = trim(str_replace($vowels,"",$e->href));
	...
	private function work_visitProfile($username, $item, $cookiePath)
	{
		...
		$content = $this->getHTTPContent("http://www.upzz.com".$item["link"], $this->searchIndex, $cookiePath);
		$this->sleep(5);
		return $content;
	}
		...
		if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
		{
			...
			{				
				...
				if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
				{
					...
					$param = array(
						"do" => 1,
						"empfaenger" => $item["username"],
						"empfaenger_id" => $item["uid"],
						"empfaenger_geschlecht" => "m",
						"antwort_id" => 0,
						"nachricht" => $message,
						"anhang[]" => "",	
						"x" => 76,
					...
					$content = $this->getHTTPContent("http://www.upzz.com/portal/nachrichten/senden_ajax.html#box", "http://www.upzz.com/portal/profil/".$item["uid"]."/".$item["username"].".html", $cookiePath, $param);

					$url_log = "URL => http://www.upzz.com/portal/nachrichten/senden_ajax.html#box\nREFERER => http://www.upzz.com/portal/profil/".$item["uid"]."/".$item["username"].".html\n";
					file_put_contents("sending/pm-".$username."-".$item["username"].".html",$url_log.$content);

					if(strpos($content, 'Deine Nachricht an') !== false)
					{	
						$this->newMessage=true;
	...
	private function getInboxMessages($username, $cookiePath)
	{
		...
		$content = $this->getHTTPContent($this->inboxURL, $this->loginURL, $cookiePath);
