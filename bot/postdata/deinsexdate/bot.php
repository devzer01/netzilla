	...
	public function login()
	{
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				// Log
				$this->nullPost = 1;
				$content = $this->getHTTPContent('http://www.deinsexdate.com/index.php?main=page&view=ajax_result&fetchview=guestAjax', 'http://www.deinsexdate.com/', $cookiePath);
				$json = json_decode($content);
				$data = json_decode($json->view);
				
				if(!empty($data->pcTrackingData->uuid)) {
					$this->loginArr[$this->currentUser]['login-uuid'] = $data->pcTrackingData->uuid;
				}
