...
{
	private $_table_prefix = 'medicsingles_';
	private $_searchResultId = 0;
	private $nextSearchPage = '';
	public $sendmsg_total = 0;
	public $rootDomain = 'http://www.medicsingles.de/';
	...
	public function __construct($post)
	{
		...
		$this->loginURL = "";
		$this->loginActionURL = 'http://www.medicsingles.de/midlogin.php';
		$this->loginRefererURL = "";
		$this->loginRetry = 3;
		$this->logoutURL = 'http://www.medicsingles.de/logout.php';
		$this->indexURL = 'http://www.medicsingles.de/index.html';
		$this->indexURLLoggedInKeyword = 'logout.php';
		$this->searchURL = 'http://www.medicsingles.de/newmemberslist.php';
		$this->searchNextURL = '';
		$this->searchRefererURL = "";
		$this->searchResultsPerPage = 1;
		$this->profileURL = "";
	...
	private function sendUserMessage($item, $username, $cookiePath) {
		...
		if($this->reserveUser($item['username']))
		{
			...
			if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
			{
				...
				// Deine Nachricht ist gesendet worden
			...
			do
			{
				...
				if($page == 1) {
					$content = $this->getHTTPContent(
						'http://www.medicsingles.de/newmemberslist.php', 
						$this->rootDomain, 
						$cookiePath);
				} else {
					$content = $this->getHTTPContent(
						'http://www.medicsingles.de/newmemberslist.php?page='.$page, 
						$this->rootDomain, 
						$cookiePath);
				}
	...
	**/
	private function getMembersFromSearchResult($username, $page, $content, $age)
	{
		...
		if(!empty($content)) {
			...
			if(!empty($html->find("table.module_detail_inside tr"))) {
				foreach($html->find("table.module_detail_inside tr") as $tr) {
					if(!empty($tr->find('td',1))) {
						if($tr->find('td',1)->plaintext == $this->command['gender']) {
							$uid = str_replace('.htm', '', $tr->find('a',0)->href);
							$uid = str_replace('/', '', $uid);
							$list[] = array(
								'username' => $tr->find('a',0)->plaintext,
								'profile_url' => $this->rootDomain . $uid .'.html',
								'message_url' => 'http://www.medicsingles.de/compose.php?recipient='.$uid,
								'user_id' => $uid
							);
						}
					}
