...
class bot
{
	...
	public function login()
	{
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				/**
					PRE HACK BEFORE LOGIN FOR PLANETROMEO.COM 
				**/
				$this->getHTTPContent($this->rootDomain,$this->loginURL, $cookiePath);
				$content = $this->getHTTPContent($this->loginURL, $this->loginRefererURL, $cookiePath, NULL);
				$html = str_get_html($content);

				$this->loginArr[$this->currentUser]['PHPSESSID'] = $html->find("input[name=PHPSESSID]",0)->value;
				$this->loginArr[$this->currentUser]['salt']= $html->find("input[name=salt]",0)->value; 
	...
	protected function getHTTPContent($url, $referer, $cookiePath=null, $postContent=null, $get_info = FALSE)
	{
		$ch = curl_init();
