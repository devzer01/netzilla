...
class Rubensfan extends bot
{
	public function rubensfan($post)
	{
		...
		else
		{
			$this->command = array(
									"profiles" => array(
												array(
													"username" => "kathi1989herrmann@hotmail.com",
													"password" => "liebKath"						
												)
											),
									"messages" => array(
		...
		$this->databaseName = "rubensfan";
		$this->userhash = "";
		$this->token = "";
		$this->usernameField = "login_mail";
		$this->loginURL = "http://www.rubensfan.de/login/";
		$this->loginRefererURL = "http://www.rubensfan.de/";
		$this->loginRetry = 3;
		$this->logoutURL = "http://www.rubensfan.de/logout/";
		$this->indexURL = "http://www.rubensfan.de/";
		$this->indexURLLoggedInKeyword = 'href="logout/"';
		
		$this->searchIndex = "http://www.rubensfan.de/singleboerse_fuer_mollige/";
		$this->searchURL = "http://www.rubensfan.de/singleboerse_fuer_mollige/search/";
		$this->searchRefererURL = "http://www.rubensfan.de/";
	...
	public function work()
	{
		...
		$content = $this->getHTTPContent("http://www.rubensfan.de/profiles/", "http://www.rubensfan.de/", $cookiePath);		
		$pages = 0;$endloop = false;$this->count_msg = 0;
	/*	
		if($this->command['online'] == 1){
			
			$content = $this->getHTTPContent("http://www.rubensfan.de/profiles/online", "http://www.rubensfan.de/", $cookiePath);

			if($this->command['gender'] == "f"){
				
				$content = substr($content, strpos($content, '<h2>Weibliche Mitglieder</h2>'));
				$content = substr($content, 0, strpos($content, '<h2>Männliche Mitglieder</h2>'));				

			}elseif($this->command['gender'] == "m"){

				$content = substr($content, strpos($content, '<h2>Männliche Mitglieder</h2>'));
				$content = substr($content, 0, strpos($content, '<div class="container topline">'));
...
*/
			while($endloop == false){
				...
				if($pages == 0){
					...
					$content = $this->getHTTPContent("http://www.rubensfan.de/profiles/", "http://www.rubensfan.de/profiles/", $cookiePath, $search_arr);
				}else{				
					$content = $this->getHTTPContent("http://www.rubensfan.de/profiles/".$pages, "http://www.rubensfan.de/profiles/", $cookiePath);

				}			

				$result = $this->getMembersFromSearchResult($content,$cookiePath);
				if(count($result) > 0){
					$this->savelog("Go to Page: ".($pages+1));
	...
	private function getOnlineMembersFromSearchResult($content){
		...
		if($html = str_get_html($content)){
			...
			$nodes = $html->find("a.profileonline");
					
			foreach($nodes as $node){				
				array_push($list,array("username" => trim($node->innertext),"links" => $node->href));
			}
	...
	private function getMembersFromSearchResult($content,$cookiePath){
		$list = array();
		
		$content = substr($content, strpos($content, '<div class="profileresult">'));
		$content = substr($content, 0, strpos($content, '<div class="pagination">'));
		...
		if($html = str_get_html($content)){
			...
			foreach($nodes as $node){
				...
				if($html = str_get_html($node->innertext)){
				
					$nodes2 = $html->find(".right");

					foreach($nodes2 as $e){
						$uname = substr($e->innertext, strpos($e->innertext, '<h4>'));
						$uname = substr($uname, 0, strpos($uname, '</h4>'));

						$vowels = array('<br>','</h4>','<img src="img/icons/male.png">','<h4>','<img src="img/icons/female.png">');
						$uname = trim(str_replace($vowels,"",$uname));
					}

				
					if($uname != "in manu feminine"){
						array_push($list,array("username" => $uname,"links" => $node->href));
					}
	...
	private function work_visitProfile($username, $item, $cookiePath)
	{
		...
		$content = $this->getHTTPContent($this->indexURL.$item["links"], $this->searchIndex, $cookiePath);
		$this->sleep(5);
		return $content;
	}
	...
	private function work_sendMessage($username, $item, $cookiePath, $enableMoreThanOneMessage=false){
		...
		if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
		{
			...
			if($this->reserveUser($item['username']))
			{
				...
				{
					$content = $this->getHTTPContent($this->indexURL.$item["links"]."/message/", $this->indexURL.$item["links"], $cookiePath);
					
					$message_arr = array(
						"fparser_files[]" => "image",
						"form" => "singleboersemessage_view::form",
						"text" => $message,
						"image" => ""
					);
					...
					if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
					{						
						$content = $this->getHTTPContent($this->indexURL.$item["links"]."/message/", $this->indexURL.$item["links"], $cookiePath, $message_arr);
						file_put_contents("sending/pm-".$username."-".$item['username'].".html",$content);

						if(strpos($content, 'Nachricht gesendet') !== false)
						{
							$this->newMessage = true;
	...
	public function checkTargetProfile($profile = '') {
		...
		if($profile != '') {
			$content = $this->getHTTPContent('http://www.rubensfan.de/profiles/', $this->indexURL, $cookiePath, array(
				'view' => 'main',
				'form' => 'profiles',
				'gender' => 'n',
				'search' => 0,
			...
			if(!strpos($content,'Es wurde leider kein Profil gefunden')) {
				return TRUE;
			} else {
				return FALSE;
			}
