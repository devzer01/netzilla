...
// 		$this->setNextLoginIndex();
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				**/
				...
				if(!empty($content)){
					...
					if($html->find('form',0)) {
						$signup_url = $html->find('form',0)->action;
						if(strpos($signup_url, 'signin') !== false) {
							$this->loginActionURL = $signup_url;
						}
	...
	protected function isLoggedIn($username)
	{
		...
		$content = $this->getHTTPContent($this->indexURL.'/de', $this->loginRefererURL, $cookiePath);
		if(empty($content)) {
	...
	protected function getHTTPContent($url, $referer, $cookiePath=null, $postContent=null, $get_info = TRUE)
	{
		$ch = curl_init();
