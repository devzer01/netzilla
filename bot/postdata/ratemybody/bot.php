...
class bot
{
	...
	{
		$v = rand(25, 28);
		$this->userAgent = 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:'.$v.'.0) Gecko/20100101 Firefox/'.$v.'.0';		
		$this->currentUser = 0;
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				 ...
				 * PRE Login
				 */
				$content = $this->getHTTPContent($this->loginURL, $this->rootDomain, $cookiePath);
				if(!empty($content)){
					$html = str_get_html($content);
					if(!empty($html->find("input[id='__VIEWSTATE']",0))){
						$this->loginArr[$this->currentUser]['__VIEWSTATE'] = $html->find("input[id='__VIEWSTATE']",0)->value;
						$this->loginArr[$this->currentUser]['__EVENTVALIDATION'] = $html->find('input[id=__EVENTVALIDATION]',0)->value;
						$this->loginArr[$this->currentUser]['referer'] = '(HTTP_REFERER)';
						$this->loginArr[$this->currentUser]['autoLogon'] = 'on';
						$this->loginArr[$this->currentUser]['btnSubmit'] = 'Submit';
					}
				}
				
				
				$content = $this->getHTTPContent($this->loginActionURL, $this->loginActionURL, $cookiePath, $this->loginArr[$this->currentUser]);
