...
class bot
{
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				if($this->command["proxy_type"] == 1){
					...
					while($tor_loop == false)
					{
						$content = $this->getHTTPContent("http://mingle2.com/", "http://mingle2.com/", $cookiePath);

						if(strpos($content, 'Why do I have to complete a CAPTCHA?') !== false)
						{
							$this->savelog("Found Captcha.");
						}else{
	...
	protected function isLoggedIn($username)
	{
		...
		$content = $this->getHTTPContent("http://mingle2.com/welcome/home", $this->loginRefererURL, $cookiePath);
