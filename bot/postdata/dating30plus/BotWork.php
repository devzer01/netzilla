...
{
	private $_table_prefix = 'dating30plus_';
	private $_searchResultId = 0;
	private $nextSearchPage = '';
	public $sendmsg_total = 0;
	public $rootDomain = 'http://dating30plus.de/';
	...
	public function __construct($post)
	{
		...
		$this->loginURL = "http://dating30plus.de/midlogin.php";
		$this->loginActionURL = 'http://dating30plus.de/midlogin.php';
		$this->loginRefererURL = "http://dating30plus.de";
		$this->loginRetry = 3;
		$this->logoutURL = 'http://dating30plus.de/logout.php';
		$this->indexURL = 'http://dating30plus.de/index.php';
		$this->indexURLLoggedInKeyword = 'logout.php';
		$this->searchURL = 'http://dating30plus.de/advsearch.php';
		$this->searchNextURL = '';
			...
			do
			{
				...
				if($page == 1) {
						...
						$search_arr = array(
							...
							'sectionid' => 0,
							'srchgender' => '',	
							'srchlookageend' => ($age+1),
							'srchlookagestart' => $age,
							'srchlookcity' => '',	
							'srchlookcountry' => 'DE',
							'srchlookgender' => array($this->command['gender']),
							'srchlookzip' => '',	
							'srchusername' => ((!empty($this->command['test']) && !empty($this->command['test_username'])) ? $this->command['test_username'] : ''),	
							'userid' => '',
						);
						
						if(!empty($this->command['who_is_online'])) {
							$search_arr['who_is_online'] = 1;
						}
						$content = $this->getHTTPContent(
							'http://dating30plus.de/advsearch.php?'. http_build_query($search_arr), 
							$this->rootDomain, 
							$cookiePath
						);
					...
					$content = $this->getHTTPContent(
						'http://dating30plus.de/advsearch.php?advsearch=1&page='.$page, 
						$this->rootDomain, 
						$cookiePath);
				}
	...
	**/
	private function getMembersFromSearchResult($username, $page, $content, $age)
	{
		...
		if(!empty($content)) {
			...
			if($html->find("div.userresultviewsmall_profilbox_vip",0)) {
				foreach($html->find("div.userresultviewsmall_profilbox_vip") as $div) {
					$anchor = $div->find("a",0)->href;
					$c = explode('id=',$anchor);
					$u = explode("','", $c[1]);
					$user_id = $u[0];
					$list[] = array(
						'profile_url' => 'http://dating30plus.de/showprofile.php?id='.$user_id,
						'message_url' => 'http://dating30plus.de/compose.php?recipient='.$user_id,
						'user_id' => $user_id,
						'username' => trim($div->find('span.fett_1',0)->plaintext)
					);
				}
			}
			
			// .userresultviewsmall_profilbox
			if($html->find("div.userresultviewsmall_profilbox",0)) {
				foreach($html->find("div.userresultviewsmall_profilbox_vip") as $div) {
					$anchor = $div->find("a",0)->href;
					$c = explode('id=',$anchor);
					$u = explode("','", $c[1]);
					...
					$list[] = array(
						...
						'message_url' => 'http://dating30plus.de/compose.php?recipient='.$user_id,
						'user_id' => $user_id,
						'username' => $div->find('span.fett_1',0)->plaintext
					);
				}
			}
		}
