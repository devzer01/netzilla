...
class Flirttown extends bot
{
	public $sendmsg_total = 0;
	public function flirttown($post)
	{
		...
		$this->databaseName = "flirttown";
		$this->userhash = "";

		//Login
			$this->usernameField = "username";
			$this->indexURL = "http://www.flirttown.net";
			$this->indexURLLoggedInKeyword = "action=logout";
			$this->loginURL = "http://www.flirttown.net/content/login.php";
			$this->loginRefererURL = "http://www.flirttown.net/content/left.php";
			$this->loginRetry = 3;
			$this->logoutURL = "http://www.flirttown.net/index.php?action=logout";
	...
	{		
		if($this->countMessage < $this->limitMessage){
			...
			if($this->command['online'] == 1){
				...
				$content = $this->getHTTPContent($this->indexURL."/content/ajax/user_online.php", $this->indexURL."/content/right.php", $cookiePath, $search_arr, $headers);
				$content = stripslashes($content);

				$content = substr($content, strpos($content, '<div class="uoBox">'));
				$content = substr($content, 0, strpos($content, '"}'));

				if($html = str_get_html($content)){
							
					$nodes = $html->find(".uoBox a");
					
					$result = array();

					foreach ($nodes as $node) {	

						array_push($result,array("username" => trim($node->innertext),"id" => str_replace("profil.php?id=","",$node->href),"link" => $node->href));
					}
				...
				while($endloop == false){
					...
					$content = $this->getHTTPContent($this->indexURL."/content/search.php?".http_build_query($search_arr), $this->indexURL."/content/search.php", $cookiePath);			

					$total = 0; $result = array();

					if($html = str_get_html($content)){
						
						$table = $html->find("table.search",0);
						foreach ($table->find("a.usrLink") as $node) {					
							array_push($result,
								array(
									"username" => trim($node->innertext),
									"id" => str_replace("profil.php?id=","",$node->href),
									"link" => $node->href)
								);
							$total++;
						}
			...
			{				
				...
				if($this->command['msg_type']=="pm")
				{
					...
					{						
						...
						$param = array(
							"addbbcode20" => 100,
							"message" => $message,
							"msg_to_userid" => $item["id"],
							"post" => "Nachricht absenden"
						);

						$headers = array('Content-Type: application/x-www-form-urlencoded');
						$content = $this->getHTTPContent($this->indexURL."/content/message.php?id=".$item["id"], $this->indexURL."/content/inbox.php?action=sent", $cookiePath, $param, $headers);
						...
						$content = $this->getHTTPContent($this->indexURL."/content/inbox.php?action=sent", $this->indexURL, $cookiePath);
				...
				{				
					...
					if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
					{
						...
						$headers = array('Content-Type: application/x-www-form-urlencoded');
						$content = $this->getHTTPContent($this->indexURL."/content/profil.php?id=".$item["id"]."&module=guestbook&new_entry=1", $this->indexURL."/content/profil.php?id=".$item["id"]."&module=guestbook&new_entry=1", $cookiePath, $param,$headers);
						file_put_contents("sending/gb-".$username."-".$item['username'].".html",$content);
							
						if(strpos($content, 'Eintrag erfolgreich verschickt'))
						{
							$this->newMessage=true;
	...
	private function deleteAllOutboxMessages($username,$cookiePath)
	{
		...
		$content = $this->getHTTPContent($this->indexURL."/content/inbox.php", $this->indexURL, $cookiePath);

		if($html = str_get_html($content)){					
					
			$nodes = $html->find("#inbox_total");	
		...
		for($i=1;$i<=$totalPages;$i++){
			...
			$content = $this->getHTTPContent($this->indexURL."/content/inbox.php?action=sent&p=".$i, $this->indexURL, $cookiePath);

			if($html = str_get_html($content)){					
					
				foreach($html->find('.inbox  a') as $e) {	

					if(strpos($e->href, 'delete=') !== false)
					{

					// ID
						$id = substr($e->href, strpos($e->href, "delete=")+7);
						$id = substr($id, 0, strpos($id, "&amp;p="));

					//Secure
						$secure = substr($e->href, strpos($e->href, "secure=")+7);

						$content = $this->getHTTPContent($this->indexURL."/content/inbox.php?action=sent&delete=".trim($id)."&p=".$i."&secure=".trim($secure), $this->indexURL."/content/inbox.php", $cookiePath);											
					}
				}
			}
		}
	...
	private function getInboxMessages($username, $cookiePath)
	{
		...
		for($i=1;$i<=$totalPages;$i++){
			$content = $this->getHTTPContent($this->indexURL."/content/inbox.php?action=new&p=".$i, $this->indexURL, $cookiePath);

			if($html = str_get_html($content)){					
					
				foreach($html->find('.userlink') as $e) {					
	...
	public function checkTargetProfile($profile = '') {
		...
		if($profile != '') {
			$content = $this->getHTTPContent('http://www.flirttown.net/content/search.php?user_name_id='.$profile.'&search=suchen&name_find=false&sex=0&age_from=&age_to=&pic=0&real=0&status=0&rubrik=0&lang=0&plz=&radius=egal', $this->indexURL, $cookiePath);
