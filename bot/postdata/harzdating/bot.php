...
class bot
{
	...
	public function logout()
	{
		...
		$content = $this->getHTTPContent("http://harzdating.de/design08/log/logout_1.php?PHPSESSID=".$this->session."&alias=".$username, $this->loginRefererURL, $cookiePath);
		return true;
	}
	...
	public function returnUsers(){
		...
		if(count($result) > 0){
			...
			$login_arr = array(
				"alias_log" => $result[0]['username'],			
				"passw_log" => $result[0]['password'],
				"submit" => "einloggen"
			);
	...
	protected function isLoggedIn($username)
	{
		...
		$content = $this->getHTTPContent("http://harzdating.de/design08/content/meldungen.php", $this->loginRefererURL, $cookiePath);
		
		if(strpos($content, $this->indexURLLoggedInKeyword)!==false)//If logged in
		{
			$content = substr($content,strpos($content,'/log/logout_1.php?PHPSESSID=')+28);
			$content = substr($content,0,strpos($content,'&alias'));
