...
class botwork extends bot
{
	...
	public function __construct($post)
	{
		...
		$this->name = DBConnect::row_retrieve_2D_conv_1D("SELECT message FROM messages_part WHERE part='4' and target='".$target."'");
		...
		if($this->command['version']==1)
		{
			...
			$this->postMessage = array(
				"Search for my nick, [USERNAME], ok?",
				"check out my Username over there, [USERNAME], ok?",
				"look after PattyR over there, c u soon!",
				"Simply use the nickname search and enter my nick: [USERNAME], ok?",
				"You'll find me by using the nick search and entering [USERNAME], ok?",
				"Just look for my Nick, [USERNAME] if you're there! Cheers!",
				"I'm curiously awaiting your reply, simply search Patty Rover there, ok?",
				"Hopefully see u soon over there :P Cheers! [USERNAME]",
				"Check out my Nick over there, it's [USERNAME], ok?",
				"Simply drop [USERNAME] a message over there, ok?",
			);
		}
		else
		{
			...
			$this->postMessage = array(
				"Search for my nick, [USERNAME], ok?",
				"check out my Username over there, [USERNAME], ok?",
				"look after [USERNAME] over there, c u soon!",
				"Simply use the nickname search and enter my nick: [USERNAME], ok?",
				"You'll find me by using the nick search and entering [USERNAME], ok?",
				"Just look for my Nick, [USERNAME] if you're there! Cheers!",
				"I'm curiously awaiting your reply, simply search [USERNAME] Rover there, ok?",
				"Hopefully see u soon over there :P Cheers! [USERNAME]",
				"Check out my Nick over there, it's [USERNAME], ok?",
				"Simply drop [USERNAME] a message over there, ok?",
			);
	...
	private function getPostMessage()
	{
		...
		$i = (count($this->name)-1);
		$rand = rand(0, $i);
		return str_replace('[USERNAME]', $this->name[$rand], $msg);
	}
