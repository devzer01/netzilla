	...
	if(!empty($profiles))
	{
		...
		foreach($profiles as $profile) {
			if($bot->testLogin($profile) == TRUE) {
				...
				if($bot->testSuspended($profile['username']) == TRUE) {
					$bot->savelog($i.'/'.$count.' => Test failed : '.$profile['username'].' will be deleted. [Account Suspended]');
				} else {
				...
				DBConnect::execute_q("UPDATE user_profiles SET status='false' WHERE id=".$profile['id']);
				$bot->savelog($i.'/'.$count.' => Test failed : '.$profile['username'].' will be deleted. [Profile Incorrect]');
			}
			$i++;
		}
