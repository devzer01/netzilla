...
class werKenntWen extends bot
{
	...
	public function werKenntWen($post)
	{
		...
		else                  
		{
			$this->command = array(
									"profiles" => array(
															array(
																	"username" => "Angsthase@hotmail.com",
																	"password" => "22Erkner"
																	)
														),
									"messages" => array(
		...
		$this->loginRetry = 3;
		$this->logoutURL = "http://www.wer-kennt-wen.de/login/logout/";
		$this->indexURL = "http://www.wer-kennt-wen.de";
	...
	{		
		...
		if(empty($this->command['gender'])){
			$this->savelog("[failed] - Not set target gender ... system set default target gender to Male");
			$this->command['gender'] = 2;
		}
		...
		if(isset($this->command['online']) && ($this->command['online']=="1"))
		{
			...
			while($endloop == false){
				...
				$content = $this->getHTTPContent("http://www.wer-kennt-wen.de/online/users/sort/known/show/list/page/".$pages, $this->indexURL, $cookiePath);
				
				
				if(strpos($content, 'Bitte gib die beiden Buchstabenfolgen in das Eingabefeld')) {
					$this->_enabled = FALSE;
					$this->savelog("failed : Bitte gib die beiden Buchstabenfolgen in das Eingabefeld im Kasten ein. Solltest Du die W&ouml;rter nicht lesen k&ouml;nnen, kannst Du Dir &uuml;ber den ReCaptcha-Button zwei neue Buchstabenfolgen generieren lassen.");
					break;
				}
...
// 
					...
					if(!empty($html)) {
						if(!empty($html->find('div.m-s',1))) {
							$allpages = str_replace('<span>...</span>', '', $html->find('div.m-s',1)->plaintext);
						}
					}
				}
				
				$this->savelog("Go to Page: ".$pages." of ".$allpages);
				$list = $this->getMembersFromSearchResult($content);
				
				if(count($list) > 0)
				{
					
					// Checking matching target ?
					$list2 = array();
					foreach($list as $item) {
						$content = $this->work_visitProfile($username, $item, $cookiePath);
						if(strpos($haystack, 'search/users/field/gender/'.$this->command['gender'])) {
							$list2[] = $item;
						}
					}
										
					
					$this->savelog("Found ".count($list2)." member(s)");
					$this->sleep(5);
		...
		if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
		{
			...
			{				
				...
				if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
				{
					...
					/*$param = array(
						hash	a58950761f73ed076e3ff8c2580ad18d
						hashKey	1440393075698
						body	Alles Gute zum Geburtstag wünscht Dir!
						att_nothumb	on
						att_id	
						att_url	
						att_hash	
					...
					$content = $this->getHTTPContent("http://www.wer-kennt-wen.de/guestbook/addentry", "http://www.wer-kennt-wen.de/person/".$item['userid']."/tab/guestbook", $cookiePath, $param, $headers);							
			...
			if($this->command['logout_after_sent'] == "Y"){
				if($this->sendmsg_total >= $this->command['messages_logout']){
					...
					$this->logout();
					$this->savelog('Logout after sent '.$this->command['messages_logout'].' messages(s)');
