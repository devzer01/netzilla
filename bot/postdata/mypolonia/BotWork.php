...
{
	private $_table_prefix = 'mypolonia_';
	private $_searchResultId = 0;
	private $nextSearchPage = '';
	public $sendmsg_total = 0;
	public $rootDomain = 'http://www.mypolonia.de/';
	...
	public function __construct($post)
	{
		...
		$this->usernameField = 'log_nickname';
		$this->loginURL = "http://www.mypolonia.de/login/";
		$this->loginActionURL = 'http://www.mypolonia.de/login/';
		$this->loginRefererURL = "http://www.mypolonia.de/";
		$this->loginRetry = 3;
		$this->logoutURL = 'http://www.mypolonia.de/login/logout.php';
		$this->indexURL = 'http://www.mypolonia.de/account/';
		$this->indexURLLoggedInKeyword = 'login/logout.php';
		$this->searchURL = 'http://www.mypolonia.de/members/search/';
		$this->searchNextURL = '';
	...
	private function searchMember($online = 0) {
		...
		for($age = $this->command['age_from']; $age <= $this->command['age_to']; $age++ )
		{
			...
			$this->getHTTPContent('http://www.mypolonia.de/members/search/', $this->rootDomain, $cookiePath);
			...
			do
			{
				...
				$search_arr = array(
					'sea_agefrom' => $age,
					'sea_ageto' => $age,
					'sea_askedmale' => $this->command['gender'],
					'sea_country' => 'D',
					'sea_distance' => '100',
					'sea_firstname' => '',
					'sea_hometown' => '',	
				...
				if(!empty($this->command['online'])) {
					$search_arr['sea_online'] = 'on';
				}
				
				if(!empty($this->command['photo'])) {
					$search_arr['sea_picture'] = 'on';
				}
				
				if($page == 1) {
					 $content = $this->getHTTPContent(
						'http://www.mypolonia.de/members/search/index.php', 
						'http://www.mypolonia.de/members/search/', 
						$cookiePath,
						$search_arr,
						NULL,
						array(
							'Content-Length: '.mb_strlen(http_build_query($search_arr)),
							'Content-Type: application/x-www-form-urlencoded'
						)
					);
				} else {
					$content = $this->getHTTPContent(
						'http://www.mypolonia.de/members/search/index.php?page='.$page.'&'.http_build_query($search_arr), 
						$this->rootDomain, 
						$cookiePath);
				}
	...
	**/
	private function getMembersFromSearchResult($username, $page, $content, $age)
	{
		...
		if(!empty($content)) {
			...
			// .userresultviewsmall_profilbox_vip
			if($html->find("div.profilebox_tab",0)) {
				foreach($html->find("div.profilebox_tab a") as $anchor) {
					$user_id = str_replace('/profile/index.php?profile_id=','',$anchor->href);
					$list[] = array(
						'profile_url' => 'http://www.mypolonia.de/profile/index.php?profile_id='.$user_id,
						'message_url' => 'http://www.mypolonia.de/account/mail/new/index.php?profile_id='.$user_id,
						'user_id' => $user_id,
						'username' => trim($anchor->title)
					);
				}
			}
		}
	...
	public function checkTargetProfile($profile = '') {
		...
		if($profile != '') {
			$content = $this->getHTTPContent('http://www.mypolonia.de/members/search/', $this->rootDomain, $cookiePath,array(
				'sea_nickname' => $profile,
				'send' => 'send'	
			));
			if(!strpos($content,'Bitte mache weniger oder andere Angaben')) {
				return TRUE;
			} else {
				return FALSE;
			}
