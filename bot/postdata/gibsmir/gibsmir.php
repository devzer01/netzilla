...
class Gibsmir extends bot
{
	public function gibsmir($post)
	{
		...
		else
		{
			$this->command = array(
									"profiles" => array(
															array(
																	"username" => "michelleegger69@outlook.com",
																	"password" => "Be4aehpEQwzy"
																	)
														),
									"messages" => array(
															array(
		...
		$this->databaseName = "gibsmir";
		$this->userhash = "";

		//Login
			$this->usernameField = "txtLogin";
			$this->indexURL = "http://www.gibsmir.de/mypage.php";
			$this->indexURLLoggedInKeyword = "Logout";
			$this->loginURL = "https://www.gibsmir.de/login.html";
			$this->loginRefererURL = "http://www.4crazy.de";
			$this->loginRetry = 3;
			$this->logoutURL = "http://www.gibsmir.de/logout.asp";
		
		//Search
			$this->searchIndex = "http://www.gibsmir.de/find.php";
	...
	{		
		...
		$content = $this->getHTTPContent("http://www.gibsmir.de/mypage.php?source=login&chapter=mypage", "http://www.gibsmir.de", $cookiePath);
		$content = $this->getHTTPContent("http://www.gibsmir.de/my_photos.php?source=login", "http://www.gibsmir.de", $cookiePath);
		$content = $this->getHTTPContent("http://www.gibsmir.de/find.php", "http://www.gibsmir.de", $cookiePath);
		...
		while($endloop == false){
			$search_array = array(
				...
				"go" => "Find",
				"location" => $this->command['city'],
				"online_now" => $this->command['online'],
				"page" => $pages,				
				"sort" => "default",
				"spo_s" => 1,
				"tab" => "all",
			...
			$content = $this->getHTTPContent("http://www.gibsmir.de/find.php", "http://www.gibsmir.de/find.php", $cookiePath, $search_array, $headers);
		
			$results = json_decode($content, true);
			
			if(count($results["profiles"]) > 0){
				
				$this->savelog("Go to Page: ".$pages);

				$result = $this->getMembersFromSearchResult($results["profiles"]);
	...
	private function getMembersFromSearchResult($content){
		...
		foreach($content as $k => $v){
			...
			if($html = str_get_html($v)){						
				$nodes = $html->find("a.b-scname");
	...
	private function work_visitProfile($username, $item, $cookiePath)
	{
		...
		$content = $this->getHTTPContent("http://www.gibsmir.de".$item["link"], $this->searchIndex, $cookiePath);
		$this->sleep(5);
		return $content;
	}
		...
		if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
		{
			...
			if($this->reserveUser($item['username']))
			{
				if($this->command['msg_type']=="pm")
				{
					...
					$content = $this->getHTTPContent("http://www.gibsmir.de/page.php?name=imb_form&template=profile&xid=".$item['uid'], "http://www.gibsmir.de".$item["link"], $cookiePath,"",$headers);
					$this->sleep(5);
					...
					if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
					{
						...
						$content = $this->getHTTPContent("http://www.gibsmir.de/mailbox/write/-/".$item['uid'].".html", "http://www.gibsmir.de".$item['link'], $cookiePath, $param, $headers);

						$url_log = "URL => http://www.gibsmir.de/mailbox/write/-/".$item['uid'].".html \nREFERER => http://www.gibsmir.de".$item['link']."\n";
						file_put_contents("sending/pm-".$username."-".$item["username"].".html",$url_log.$content);

						if(strpos($content, 'Message sent OK') !== false)
						{
							$this->newMessage = true;
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$loginRetry; $count_login++)
			{
				...
				$content = $this->getHTTPContent($this->loginURL, $this->loginRefererURL, $cookiePath, array(
					"frmPassword" => $profile["password"],
					"login" => "Login",
					"txtLogin" => $profile["username"]
				),$headers);		
				$content = $this->getHTTPContent("http://www.gibsmir.de/mypage.php?source=login&chapter=mypage", $this->loginRefererURL, $cookiePath);
				$content = $this->getHTTPContent("http://www.gibsmir.de/my_photos.php?source=login", $this->loginRefererURL, $cookiePath);

				if(empty($content))
				{
					// $this->savelog("==[TEST]==> No response from server.");
					$this->loginRetry++;
				}
	...
	public function checkTargetProfile($profile = '') {
		...
		if($profile != '') {
			$content = $this->getHTTPContent('http://www.gibsmir.de/find.php?tab=screenname&screenname='.$profile.'&go=', $this->indexURL, $cookiePath);
