...
{
	private $_table_prefix = 'romantik_';
	private $_searchResultId = 0;
	private $nextSearchPage = '';
	public $sendmsg_total = 0;
	public $rootDomain = 'http://www.romantik-50plus.de/';
	public $searchActionURL = 'http://www.romantik-50plus.de/search';
	...
	public function __construct($post)
	{
		...
		$this->loginURL = "http://www.romantik-50plus.de/";
		$this->loginActionURL = 'http://www.romantik-50plus.de/account/login';
		$this->loginRefererURL = "http://www.romantik-50plus.de/";
		$this->loginRetry = 3;
		$this->logoutURL = 'http://www.romantik-50plus.de/account/logout/';
		$this->indexURL = 'http://www.romantik-50plus.de/';
		$this->indexURLLoggedInKeyword = 'account/logout';
		$this->searchURL = 'http://www.romantik-50plus.de/search';
		$this->searchNextURL = '';
	...
	private function sendUserMessage($item, $username, $cookiePath) {
		...
		if($this->reserveUser($item['username']))
		{
			...
			if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
			{
				...
				if(!empty($html->find("li.message",0))) {
					
					
					$li = $html->find("li.message",0);
					$message_url = $li->find("a",0)->href;
					...
					// Go to Message Form
					$this->getHTTPContent($message_url, $item['profile_url'], $cookiePath);
					
					// POST
					$content = $this->getHTTPContent($message_url, $item['profile_url'], $cookiePath, $message_arr);
					
					// Nachricht wurde erfolgreich gesendet
					if(strpos($content, 'gesendet')) {
			...
			do
			{
				...
				if($page == 1) {
						...
						$search_arr = array(
							...
							'age_to' => $age,
							'country' => 200,
							'gender1' => $this->command['gender'], // 2 : Woman , 1 : Man
							'gender2' => (($this->command['gender'] == 2) ? 4 : 3 ), // 3 : Woman , 4 : Man
							'group_type_label' => 'members',
							'issearch' => 1,
							'submit' => 'Suche'
						);
						...
						// Picture only
						if(!empty($this->command['pictures_only'])) {
							$search_arr['pictures_only'] = 1;
						}
						
						$content = $this->getHTTPContent(
							'http://www.romantik-50plus.de/search', 
							$this->rootDomain, 
							$cookiePath,
							$search_arr);
						$html = str_get_html($content);
						if(!empty($html->find('input[id="field_hash"]',0))){
							$hash = $html->find('input[id="field_hash"]',0)->value;
						}
						
				} else {
					$content = $this->getHTTPContent(
						'http://www.romantik-50plus.de/search/'.$hash.'/'.$page, 
						$this->rootDomain, 
						$cookiePath);
				}
	...
	**/
	private function getMembersFromSearchResult($username, $page, $content, $age)
	{
		...
		if(!empty($content)) {
			$html = str_get_html($content);
			if(!empty($html->find("div.typemembers",0))) {
				$div = $html->find("div.typemembers",0);
				foreach($div->find("div.datainfo") as $info){
					if(!empty($info->find('a',0))){
						$anchor = $info->find('a',0);
						$list[] = array(
