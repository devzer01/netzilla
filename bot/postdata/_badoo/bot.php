...
class bot
{
	...
	protected $userAgent = "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:25.0) Gecko/20100101 Firefox/25.0";
	...
	public function login()
	{
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				// $login_arr['rt'] = $this->token;

				/**
					PRE HACK BEFORE LOGIN FOR BADOO.COM 
				**/
				$content = $this->getHTTPContent($this->loginURL, $this->loginRefererURL);
				if(!empty($content)){
					$html = str_get_html($content);
					$login_arr['rt'] = $html->find("input[name=rt]",0)->value;
				}
	...
	public function logout()
	{
		...
		$content = $this->getHTTPContent($this->logoutURL.$this->token, $this->loginRefererURL, $cookiePath);
		return true;
	}
	...
	protected function isLoggedIn($username)
	{
		...
		$content = $this->getHTTPContent("https://badoo.com", $this->loginRefererURL, $cookiePath);
	...
	protected function parse_curl_cookie($cookie_file)
    {
		if(file_exists($cookie_file))
		{
			...
			{
				if(strpos($line, $this->databaseName)>-1)
				{
