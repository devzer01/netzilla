...
class badoo extends bot
{
	public function badoo($post)
	{
		...
		$this->indexURL = "https://badoo.com/search/";
		$this->indexURLLoggedInKeyword = "signout";
		$this->loginURL = "https://badoo.com/signin/";
		$this->loginRefererURL = "http://badoo.com/de";
		$this->loginRetry = 3;
		...
		$this->userAgent = "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:24.0) Gecko/20100101 Firefox/25.0";
	...
	public function addLoginData($users)
	{
		...
		{
			array_push($this->loginArr, array(
					"rt" => $this->token,
					"email" => $user['username'],
	...
	public function work()
	{
		...
		if($this->countMessage < $this->limitMessage){
			...
			{				
				$location = $this->getLocation($city, $token, $cookiePath);				
				if(!$location)
				{
					...
					for($i=0;$i<count($location);$i++){
						...
						while($endloop == false){
							...
							if($this->command['distance'] != ""){
								$area_id = "a_".$location[$i]["id"]."_".$this->command['distance']."_Km";
								$distance = ", distance: ".$this->command['distance']." Km";
							}else{
								$area_id = $location[$i]["id"];
							}
							...
							$search_arr = array(								
											...
											"page" => $pages,
											"gid" => "",
											"age_f" => $this->command['ageStart'],
											"age_t" => $this->command['ageEnd'],
											"r" => $this->r,									
											"rt" => $token,
							...
							$this->savelog("Search for gender: ".$this->command['gender'].", age: ".$this->command['ageStart']."-".$this->command['ageEnd'].", city: ".$location[$i]["name"].$distance);
							$content = $this->getHTTPContent("https://badoo.com/search/?".http_build_query($search_arr), "http://badoo.com/search/", $cookiePath);
									
							if(strpos($content, 'Wir konnten keine Leute finden, die Deinen Suchkriterien entsprechen') !== false)
							{
								$endloop = true;
								$this->savelog("Not Found Profile.");
							
							}else{
								
								$this->savelog("Go to page: ".$pages);
										
								$list = $this->getMembersFromSearchResult($username, $content);
	...
	private function getMembersFromSearchResult($username, $content){
		...
		{
			if($html = str_get_html($data->html)){
						
				$nodes = $html->find("div.badge");

				foreach ($nodes as $node) {	
					
					//Name
						$u_name = substr($node->innertext,strpos($node->innertext,'<span class="face__name-txt">')+29);
						$u_name = substr($u_name,0,strpos($u_name,'</span>'));

					//Id
						$u_id = substr($node->innertext,strpos($node->innertext,'<a data-ga-ev='));
						$u_id = substr($u_id,0,strpos($u_id,'id="'));

						$u_id = substr($u_id,strpos($u_id,'href="http://badoo.com/')+23);
						$u_id = substr($u_id,0,strpos($u_id,'"'));
						$url = $this->profileURL.trim($u_id);
						$u_id = substr($u_id,0,strpos($u_id,'/?r'));
	...
	private function getLocation($city, $token, $cookiePath)
	{
		$result = array();
		...
		$url = "http://badoo.com/ws/suggest-ws.phtml?show=no_regions&ep=2&no_region_name=1&format=full&s_lang_id=5&q=".$city."&ws=1&rt=".$token;		
		$content = $this->getHTTPContent($url, "http://badoo.com/search/", $cookiePath);
	...
	public function checkLogin($username, $password){
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				/**
					PRE HACK BEFORE LOGIN FOR BADOO.COM 
				**/
				$content = $this->getHTTPContent($this->indexURL, $this->indexURL, $cookiePath, NULL);
				$html = str_get_html($content);
				$this->loginArr[$this->currentUser]['rt'] = $html->find("input[name=rt]",0)->value;
				/**
					END OF HACK
				*/
