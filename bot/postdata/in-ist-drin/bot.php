...
class bot
{
				...
				if(!empty($content)){
					...
					**/
					...
					if($html->find('form',0)) {
						...
						// Fill form
						$this->loginArr[$this->currentUser][$nickname] = $this->loginArr[$this->currentUser]['username'];
						$this->loginArr[$this->currentUser][$passwort] = $this->loginArr[$this->currentUser]['password'];
						$this->loginArr[$this->currentUser]['token'] = $html->find('input[name=token]',0)->value;
					}
					/**
						END OF HACK
					*/

					$content = $this->getHTTPContent($this->rootDomain . $formURL, $this->indexURL, $cookiePath, $this->loginArr[$this->currentUser]);
					if(!empty($content)) {
						$html = str_get_html($content);
						if((!strpos($content,"Ausloggen")) && (!empty($html->find('h1',0))) ) {
							...
							if($html->find('div[id=headerMenu] a')){
								$i = 0;
								foreach($html->find('div[id=headerMenu] a') as $anchor) {
									switch($i) {
										case 0: // Index
											$this->indexURL = $this->rootDomain.'bereich_mitglieder/'.$anchor->href;
											break;
										case 1: // Online
											$this-> onlineSearchURL = $this->rootDomain.'bereich_mitglieder/'.$anchor->href;
											break;
										case 2: // Search
											$this->searchURL = $this->rootDomain.'bereich_mitglieder/'.$anchor->href;
											break;
									}
									$i++;
								}
	...
	protected function isLoggedIn($username)
	{
		...
		if(empty($this->_session_id)) {
			...
			$content = $this->getHTTPContent($this->pageURL('vaqrk'), $this->indexURL, $cookiePath);
		}
	...
	protected function getHTTPContent($url, $referer, $cookiePath=null, $postContent=null, $get_info = FALSE, $liveSession = TRUE)
	{
		$ch = curl_init();
		...
		if($this->logged == TRUE && $liveSession == TRUE) {
			if(!empty($content)){
				$html = str_get_html($content);
				if(!empty($html->find('iframe.versteckterIframe',0)) && !empty($html->find('iframe.versteckterIframeChat11',0))) {
					$iframe1 = $html->find('iframe.versteckterIframe',0);
					$iframe2 = $html->find('iframe.versteckterIframeChat11',0);
					
					if(!empty($iframe1) && !empty($iframe2)) {
						$this->getHTTPContent($this->rootDomain.'bereich_mitglieder/'.$iframe1->src, $url, $cookiePath, null, FALSE, FALSE);
						$this->getHTTPContent($this->rootDomain.'bereich_mitglieder/'.$iframe2->src, $url, $cookiePath, null, FALSE, FALSE);
					}
				}
			}
		}
