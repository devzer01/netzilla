...
{
	private $_table_prefix = 'daten_';
	private $_searchResultId = 0;
	private $nextSearchPage = '';
	public $sendmsg_total = 0;
	public $rootDomain = 'http://daten.de/';
	public $searchActionURL = 'http://daten.de/navigation/suche.php?search=1';
	...
	public function __construct($post)
	{
		...
		$this->usernameField = 'field1';
		$this->loginURL = "http://daten.de/login.html";
		$this->loginActionURL = 'http://daten.de/login.html';
		$this->loginRefererURL = "http://daten.de/";
		$this->loginRetry = 3;
		$this->logoutURL = 'http://daten.de/logout.html';
		$this->indexURL = 'http://daten.de/';
		$this->indexURLLoggedInKeyword = 'logout.html';
		$this->searchURL = 'http://daten.de/navigation/suche.php?search=1';
		$this->searchNextURL = '';
	...
	private function sendUserMessage($item, $username, $cookiePath) {
		...
		if($this->reserveUser($item['username']))
		{
			...
			if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
			{
				...
				if($html->find("input.button1",0)) {
					$uid = 0;
					foreach ($html->find("input.button1") as $input) {
						echo '#'.$input->value.'#<br />';
						if($input->value == 'Kontakt aufnehmen') {
							$data = explode('php?uid=',$input->onclick);
							$uid = str_replace("')", '', $data[1]);
						}
					}
					
					
					$this->savelog("Sending message to ".$item['username'].' / uid : '.$uid);
						
					// Crawl
					$content = $this->getHTTPContent('http://daten.de/navigation/iman.php?uid='.$uid, $this->rootDomain, $cookiePath);
					$message_arr = array(
						'Ziel' => '',
						'Ziel2' => '',
						'message' => $message,
					...
					// POST = http://daten.de/navigation/iman.php?uid=227693&send=1
					$content = $this->getHTTPContent('http://daten.de/navigation/iman.php?uid='.$uid.'&send=1', 'http://daten.de/navigation/iman.php?uid='.$uid, $cookiePath, $message_arr);
			...
			do
			{
				...
				if($page == 1) {
						...
						$search_arr = array(
							'abis' => $age,
							'avon' => $age,
							'is' => (empty($this->command['gender'])) ? 1 : $this->command['gender'],
							'plzb' => 'alle',
							'sland' => 'alle',
							'sumkreis' => 'alle',
							// 'sonline' => '',
						...
						$content = $this->getHTTPContent(
							'http://daten.de/navigation/suche.php?search=1', 
							$this->rootDomain, 
							$cookiePath,
							$search_arr);
					...
					$content = $this->getHTTPContent(
						'http://daten.de/navigation/suche.php?seite='.$page, 
						$this->rootDomain, 
						$cookiePath);
				}
	...
	**/
	private function getMembersFromSearchResult($username, $page, $content, $age)
	{
		...
		if(!empty($content)) {
			$html = str_get_html($content);
			if($html->find("td.mout",0)) {
				foreach($html->find("td.mout") as $div) {
					$uname = $div->find('table.mimage',0)->onclick;
					$uname = str_replace("dymitte('profil.php?uid=",'',$uname);
					$uname = str_replace("');",'',$uname);
					$list[] = array(
						'profile_url' => 'http://daten.de/navigation/profil.php?uid='.$uname,
						'username' => $uname,
						'info' => trim($div->find('td',3)->plaintext)
					);
				}
			}
		}
