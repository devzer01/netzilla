...
{
	private $_table_prefix = 'fetisch_';
	private $_searchResultId = 0;
	private $nextSearchPage = '';
	public $sendmsg_total = 0;
	public $rootDomain = 'http://www.fetisch.de/';
	public $searchActionURL = 'http://www.fetisch.de/suche';
	...
	public function __construct($post)
	{
		...
		$this->loginURL = "http://www.fetisch.de/login_prod.php";
		$this->loginActionURL = 'http://www.fetisch.de/login_prod.php';
		$this->loginRefererURL = "";
		$this->loginRetry = 3;
		$this->logoutURL = 'http://www.fetisch.de/logout';
		$this->indexURL = 'http://www.fetisch.de/home';
		$this->indexURLLoggedInKeyword = 'Log-out';
		$this->searchURL = '';
		$this->searchNextURL = '';
	...
	private function sendUserMessage($item, $username, $cookiePath) {
		...
		if($this->reserveUser($item['username']))
		{
			...
			if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
			{
				...
				if($html->find("input[id='targetId']",0)) {
					$uid = $html->find("input[id='targetId']",0)->value;
					
					// Crawl
					$content = $this->getHTTPContent('http://www.fetisch.de/newMessenger/profile?uid='.$uid, $item['profile_url'], $cookiePath);
					$message_arr = array(
						'content' => $message,
						'uid' => $uid
					);
					...
					// Check
					$content = $this->getHTTPContent('http://www.fetisch.de/messages/messenger/profile_messages', $item['profile_url'], $cookiePath, 
						array(
							'lastest' => 0,
							'uid' => $uid
						)
					...
					if($json->data->can_send == false) {
						$this->savelog("[failed] Reason : ".$json->data->reason);
					} else {
						// POST
						$content = $this->getHTTPContent('http://www.fetisch.de/messages/messenger/send', $item['profile_url'], $cookiePath, $message_arr, NULL, array(
						...
						{
							$this->savelog("Sending message failed / Reason : ".$json->message);
						}
	...
	private function searchOnlineMember() {
		...
		$content = '';
		$this->savelog("Job criterias => Target age: ". $age[$this->command['age_range']]);
		...
		$content = $this->getHTTPContent(
			'http://www.fetisch.de/online/all', 
			$this->rootDomain, 
			$cookiePath,
			array(
				'age' => $this->command['age_range'],
				'commit' => 'Anzeigen',
		...
		if(!empty($content)) {
			...
			$list = $this->getOnlineUserList($content);
			if(!empty($list)){
		...
		for($age = $this->command['age_from']; $age <= $this->command['age_to']; $age++ )
		{
			...
			do
			{
				...
				$search_arr = array(
					...
					'couple_type' => '',
					'gender' => $this->command['gender'],
					'nickname' => ((empty($this->command['test_username'])) ? '' : $this->command['test_username']),
					'plz' => '',
					'with' => 2,
				);
				...
				if($page == 1) {
					...
					$content = $this->getHTTPContent(
						'http://www.fetisch.de/suche/'.$url, 
						'http://www.fetisch.de/suche/', 
						$cookiePath);
				} else {
					...
					$content = $this->getHTTPContent(
						'http://www.fetisch.de/suche/'.$url, 
						'http://www.fetisch.de/suche/'.$url2, 
						$cookiePath);
				}
	...
	**/
	private function getMembersFromSearchResult($username, $page, $content, $age)
	{
		...
		if(!empty($content)) {
			$html = str_get_html($content);
			if($html->find("ul.profile_li",0)) {
				$ul = $html->find("ul.profile_li",0);
				foreach($ul->find("a.link") as $anchor) {
					$list[] = array(
						'username' => $anchor->plaintext,
						'profile_url' => $this->rootDomain . $anchor->plaintext
					);
				}
			}
		}
	...
	private function getOnlineUserList($content) {
		$list = array();
		if(!empty($content)) {
			$html = str_get_html($content);
			if($html->find("ul.profile_li",0)) {
				$ul = $html->find("ul.profile_li",0);
				foreach($ul->find("a") as $anchor) {
					if(empty($anchor->class)){
						$username = str_replace('/','',$anchor->href);
						$list[] = array(
							'username' => $username,
							'profile_url' => $this->rootDomain . $username
						);
					}
	...
	public function checkTargetProfile($profile = '') {
		...
		if($profile != '') {
			$content = $this->getHTTPContent('http://www.fetisch.de/'.$profile, $this->rootDomain, $cookiePath);
			if(!strpos($content,'Mitglied existiert')) {
				return TRUE;
			} else {
				return FALSE;
			}
