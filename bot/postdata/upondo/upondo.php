...
class Upondo extends bot
{
	public function upondo($post)
	{
		...
		$this->databaseName = "upondo";
		$this->userhash = "";

		//Login
			$this->usernameField = 'ctrl20$TextBox1';
			$this->indexURL = "http://www.upondo.de";
			$this->indexURLLoggedInKeyword = "Logout";
			$this->loginURL = "http://www.upondo.de";
			$this->loginRefererURL = "http://www.upondo.de";
			$this->loginRetry = 3;
			$this->logoutURL = "http://4crazy.de/logout/";
		
		//Search
			$this->searchIndex = "http://www.upondo.de/search";
		...
		//Message
			...
			$this->inboxURL = "http://www.upondo.de/mailbox?in";
			$this->outboxURL = "http://www.upondo.de/mailbox?out";
		...
		$this->setProxy();
		//=== End of Set Proxy ===nteuer mag ich du auch? Blind dates? Am we vielleicht, bin ich zu direkt? Ok schau meine bilder an und entscheide selber ok? Hier bei",
	...
	public function addLoginData($users)
	{
		foreach($users as $user)
		{
			...
			$login_arr['KHF'] = strtoupper(md5($user["password"]));
			$login_arr['ctrl20$Button1'] = "Login";
			$login_arr['ctrl20$TextBox1'] = $user["username"];
			$login_arr['ctrl20$TextBox2'] = "";
	...
	{		
		...
		$param['DropDownList20'] = $this->command['ageStart'];
		$param['DropDownList21'] = $this->command['ageEnd'];
		$param['KHF'] = "";	
		$param['TextBox1'] = $this->command['zipcode'];
		$param['__EVENTARGUMENT'] = "";	
		$param['__EVENTTARGET'] = "LinkButton1";
		
		$headers = array('Content-Type: application/x-www-form-urlencoded');
		$content = $this->getHTTPContent($this->searchIndex."?p=".$page, $this->loginURL, $cookiePath ,$param ,$headers);
		
		$pages = substr($content, strpos($content, '<div class="Pager-Gray-20">'));
		$pages = substr($pages, 0, strpos($pages, "</div>"));
		if($html = str_get_html($pages)){				
			$nodes = $html->find("a");
			foreach ($nodes as $node) {	
				$pages = trim(str_replace("/search?p=","",$node->href));
			}
		}		

		for($page=0;$page < $pages; $page++){
			
			$this->savelog("Page: ".($page+1));
			$content = $this->getHTTPContent($this->searchIndex."?p=".$page, $this->loginURL, $cookiePath);
	...
	private function getMembersFromSearchResult($content){
		...
		if($html = str_get_html($content)){
			...
			$nodes = $html->find("div.foto-border-20 a");
		...
		if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
		{
			...
			if($this->reserveUser($item['username']))
			{
				if($this->command['msg_type']=="pm")
				{
					...
					$content = $this->getHTTPContent($this->indexURL.$item["link"], $this->indexURL.$item["link"], $cookiePath);
					$this->sleep(5);
					...
					$param['Ctrl61'] = $message;
					$param['__EVENTTARGET'] = 'Button1';
					...
					if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
					{
						if(strpos($content, 'Ohne Foto keine Mail') !== false)
						{
							$this->newMessage = true;
							$this->savelog("Sending message failed. Kramler only accept mail from people with photos.");							
							$this->lastSentTime = time();
							$return = true;
						}
						else
						{
							$content = $this->getHTTPContent("http://www.upondo.de/user/".$item['username'], "http://www.upondo.de/user/".$item['username'], $cookiePath, $param, $headers);
	...
	{
		$content = $this->getHTTPContent($this->outboxURL, $this->inboxURL, $cookiePath);
		
		if($html = str_get_html($content)){
					
			$nodes = $html->find("#MailgridContent a");
				
			foreach ($nodes as $node) {
				
				if(strpos($node->href, '/mailbox?out') !== false)
				{
					//<a href="/mailbox?out&d=587606375&p=0">löschen</a>
					$content = $this->getHTTPContent($this->indexURL.$node->href, $this->outboxURL, $cookiePath);
				}
			}
		}
