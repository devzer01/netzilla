...
class bot
{
	...
	protected $indexURL = "";
	protected $indexURLLoggedInKeyword = "Suchen";
	protected $loginURL = "";
	...
	public function login()
	{
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				$this->loginArr[$this->currentUser]['_csrf_token'] = $html->find("input[name=_csrf_token]",0)->value;
				$this->loginArr[$this->currentUser]['action'] = 'login';
				$this->loginArr[$this->currentUser]['commit'] = 'Login';
				$this->loginArr[$this->currentUser]['referer'] = '';
				
				

				$content = $this->getHTTPContent($this->loginActionURL, $this->loginRefererURL, $cookiePath, $this->loginArr[$this->currentUser]);
				$html = str_get_html($content);
	...
	public function returnUsers() {
		...
		if(count($result) > 0){
			...
			$login_arr["pt"] = "";
			$login_arr["user"] = $result[0]['username'];
			$login_arr["cr"] = $result[0]['password'];
	...
	/*public function returnUsers(){
		...
		if($this->command['login_by'] == 1){
			...
			if($this->command['repeat_profile'] == "Y"){
				$sql = "SELECT id, username, password FROM user_profiles WHERE (site_id='".$this->siteID."') AND (status='true') AND (in_use='false') AND (sex='".$this->user_sex."') and (username!='".$this->user_name."') AND ((lastused < DATE_SUB(NOW(), INTERVAL 24 HOUR)) or (lastused='0000-00-00 00:00:00') or (used='false')) ORDER BY rand() Limit 1";
			}else{
	...
	protected function isLoggedIn($username)
	{
		...
		if(empty($content)) {
			...
			$html = str_get_html($content);
			//$this->savelog("<textarea>" . $html . "</textarea>");
			if(!empty($html->find('a.logo_anchor',0))) {
				$content = $this->getHTTPContent($html->find('a.logo_anchor',0)->href, $this->indexURL, $cookiePath);
			}
	...
	protected function getHTTPContent($url, $referer, $cookiePath=null, $postContent=null, $get_info = TRUE, &$header = null)
	{
		$ch = curl_init();
