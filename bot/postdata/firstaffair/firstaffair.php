...
class Firstaffair extends bot
{
	public $count_msg = 0;
	public function firstaffair($post)
	{
		...
		else
		{
			$this->command = array(
									"profiles" => array(
															array(
																	"username" => "PhinePraline",
																	"password" => "Naturfreund"
																	)
														),
									"messages" => array(
															array(
		...
		$this->databaseName = "firstaffair";
		$this->userhash = "";

		//Login
			$this->usernameField = "username";
			$this->indexURL = "http://www.firstaffair.com/n/index";
			$this->indexURLLoggedInKeyword = "/n/logout&mode=profillogout";
			$this->loginURL = "http://www.firstaffair.com/n/index&tmplfail=errorlogin&mode=profillogin&kid=&pid=&lid=2;";
			$this->loginRefererURL = "http://www.firstaffair.com/en/";
			$this->loginRetry = 3;
			$this->logoutURL = "http://www.firstaffair.com/n/logout&mode=profillogout";
	...
	{		
		...
		foreach($this->cities as $city_name){
			
			$content = $this->getHTTPContent("http://www.firstaffair.com/n/dummy&mode=profil/json/geo_get&query=".$city_name."&country=1", "http://www.firstaffair.com/n/suche", $cookiePath, "" ,$headers);
			$locates = json_decode($content);

			if(is_object($locates)){
			
				foreach($locates->geoitem as $locate){
					
					$this->savelog("Search City: ".$locate->city);
					...
					$arr_search = array(
						...
						"fototyp" => 2,
						"gebjahr_bis" => $this->command['ageEnd'],
						"gebjahr_von" => $this->command['ageStart'],
						"groesse_bis" => "229 cm",
						"groesse_von" => "140 cm",
						"rubrik" => 2,
					...
					$pages = 1; $page_result = 0; $endloop = false; $online = ""; $distance = "";
					$this->count_msg = 0;
					...
					while($endloop == false){
						...
						if($pages == 1){
							...
							$headers = array('Content-Type: application/x-www-form-urlencoded');
							$content = $this->getHTTPContent("http://www.firstaffair.com/n/index&tmplok=suche_result&typ=2&mode=profilsearch", $this->searchIndex, $cookiePath, $arr_search, $headers);
							$content = $this->getHTTPContent("http://www.firstaffair.com/n/".$online."suche_result&typ=2&sex=".$this->command['gender']."&rubrik=2&gebjahr_von=".$this->command['ageStart']."&gebjahr_bis=".$this->command['ageEnd']."&fototyp=2&".$distance."cityID=1714760", $this->searchRefererURL, $cookiePath);

						}else{

							$content = $this->getHTTPContent("http://www.firstaffair.com/n/".$online."suche_result&idxid=&cityID=1714760&country=1&sex=".$this->command['gender']."&rubrik=2&gebjahr_von=".$this->command['ageStart']."&gebjahr_bis=".$this->command['ageEnd']."&fototyp=2&distance=".$distance."&start=".$page_result."&orderby=", $this->searchIndex, $cookiePath);

						}

						if(strpos($content, 'Es wurden leider keine Inserate gefunden, die Ihren Suchkriterien entsprechen') !== false)
						{
							$endloop = true;
						}
						else
	...
	private function getMembersFromSearchResult($content){
		...
		if($html = str_get_html($content)){
			...
			$nodes = $html->find("div.klbox .lefth h1");

			foreach ($nodes as $node) {									
				array_push($list, 
					array(
						"username" => trim($node->innertext)
					)
				);
			}
	...
	private function work_visitProfile($username, $item, $cookiePath)
	{
		...
		$content = $this->getHTTPContent("http://www.firstaffair.com/n/profil&profil=".$item["username"], $this->searchIndex, $cookiePath);
		$this->sleep(5);
		return $content;
	}
		...
		if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
		{
			...
			if($this->reserveUser($item['username']))
			{
				if($this->command['msg_type']=="pm")
				{
					...
					$content = $this->getHTTPContent("http://www.firstaffair.com/n/profil&profil=".$item['username']."#step5", "http://www.firstaffair.com/n/profil&profil=".$item['username'], $cookiePath);
					$this->sleep(5);
					...
					if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
					{
						$content = $this->getHTTPContent("http://www.firstaffair.com/n/dummy&mode=tele/json/send", "http://www.firstaffair.com/n/profil&profil=".$item['username'], $cookiePath, $param, $headers);

						$url_log = "URL => http://www.firstaffair.com/n/dummy&mode=tele/json/send \nREFERER => http://www.firstaffair.com/n/profil&profil=".$item['username']." \n";
	...
	private function getInboxMessages($username, $cookiePath)
	{
		...
		$count = 1;
		$endloop = false;
		$content = $this->getHTTPContent("http://www.firstaffair.com/n/post", "http://www.firstaffair.com/n/index", $cookiePath);
		$content = substr($content, strpos($content, '{ "threads": ')+13);
		...
		foreach ($datas as $data) {
			...
			if($data->counterpart != "System"){				
				array_push($list, 
					array(
						"username" => $data->counterpart
					)
				);
			}
		}
	...
	public function testLogin($profile) {
		...
		if(!($this->isLoggedIn($username)))
		{
			$this->savelog("==[TEST]==> This profile: ".$username." does not log in.");
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				$headers = array('Content-Type: application/x-www-form-urlencoded');
				$content = $this->getHTTPContent($this->loginURL, $this->loginRefererURL, $cookiePath, array(
					"jid" => "",
				...
				{
					$this->savelog("==[TEST]==> Log in failed with profile: ".$username);
					$this->savelog("==[TEST]==> Log in failed ".$count_login." times.");
	...
	public function checkTargetProfile($profile = '') {
		...
		if($profile != '') {
			$content = $this->getHTTPContent('http://www.firstaffair.com/n/profil&profil='.$profile, $this->indexURL, $cookiePath);
			if(!strpos($content,'longer exists')) {
				return TRUE;
			} else {
				return FALSE;
			}
