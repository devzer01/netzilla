...
class bot
{
	...
	public function login()
	{
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				// Log
				$content = $this->getHTTPContent($this->loginActionURL, $this->indexURL, $cookiePath, $this->loginArr[$this->currentUser]);
				if(!empty($content)) {
				...
				else if(!($this->isLoggedIn($username)))
				{
					...
					{
						$sleep_time = rand(100,120); // 2 mins
	...
	protected function getHTTPContent($url, $referer, $cookiePath=null, $postContent=null, $get_info = FALSE, $header = null)
	{
		...
		curl_close($ch);
		// echo '<p>URL : ', $url,'</p>';
		// echo '<p><textarea style="width:600px; height:400px;">',str_replace(array('<','>'),array('&lt;','&gt;'),$content),'</textarea></p>';
