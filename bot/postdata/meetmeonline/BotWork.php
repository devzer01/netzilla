...
{
	private $_table_prefix = 'meetmeonline_';
	private $_searchResultId = 0;
	private $nextSearchPage = '';
	public $sendmsg_total = 0;
	public $rootDomain = 'http://meetmeonline.de/';
	...
	public function __construct($post)
	{
		...
		$this->loginURL = "";
		$this->loginActionURL = 'http://meetmeonline.de/member.php';
		$this->loginRefererURL = "";
		$this->loginRetry = 3;
		$this->logoutURL = 'http://meetmeonline.de/member.php?logout=';
		$this->indexURL = 'http://meetmeonline.de/';
		$this->indexURLLoggedInKeyword = 'member.php?logout';
		$this->searchURL = 'http://meetmeonline.de/gallery.html?s=bsc';
		$this->searchNextURL = '';
	...
	private function sendUserMessage($item, $username, $cookiePath) {
		...
		if($this->reserveUser($item['username']))
		{
			...
			if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
			{
				...
				$sid = '';
				$uid = 0;
				if(!empty($html->find('input[name="sid"]',0))) {
					$sid = $html->find('input[name="sid"]',0)->value;
				}
									
				// Crawl
				$content = $this->getHTTPContent($item['message_url'], $this->rootDomain, $cookiePath);
				$message_arr = array(
					'mb' => 'pc',
					'message' => $message,
					'recipient' => $item['username'],
					'rndval' => round(microtime(true) * 1000),
					'send_mail' => 'yes',
					'sid' => $sid,
					'subject' => $subject
				);
				...
				// POST
				$content = $this->getHTTPContent('http://meetmeonline.de/mailboxi.php', $item['profile_url'], $cookiePath, $message_arr);
				
				// Die Nachricht wurde erfolgreich versandt
				if(strpos($content, 'versandt')) {
		...
		for($age = $this->command['age_from']; $age <= $this->command['age_to']; $age++ )
		{
			...
			do
			{
				...
				if($page == 1) {
						...
						$search_arr = array(
							'doSearch' => 'Mitglieder finden',
							'lagefrom' => $age,
							'lageto' => $age,
							'lcntry' => 'DE',
							'lsex' => (($this->command['gender'] == 'm') ? 'm' : 'f'),
							'lzip' => '',
							'no_lfor' => '',
							'online' => ((!empty($this->command['online'])) ? '1' : ''),
							'pic' => ((!empty($this->command['pic'])) ? '1' : ''),
							'radius' => '',	
							's' => 'bsc',
							'sex' => (($this->command['gender'] == 'm') ? 'f' : 'm')
						);
						$content = $this->getHTTPContent(
							'http://meetmeonline.de/gallery.html?s=bsc', 
							$this->rootDomain, 
							$cookiePath,
							$search_arr);
					...
					$content = $this->getHTTPContent(
						'http://meetmeonline.de/gallery.php?s=bsc&pg='.$page, 
						$this->rootDomain, 
						$cookiePath);
				}
	...
	**/
	private function getMembersFromSearchResult($username, $page, $content, $age)
	{
		...
		if(!empty($content)) {
			$html = str_get_html($content);
			if(!empty($html->find("a.linki"))) {
				foreach($html->find("a.linki") as $anchor) {
					if($anchor->href != '#') {
						$str = str_replace('profile.php?', '', $anchor->href);
						parse_str($str, $output);
						if(!empty($output['p'])) {
							$list[] = array(
								'profile_url' => $this->rootDomain . 'profile.php?p='.trim($output['p']).'&s=bsc',
								'message_url' => 'http://meetmeonline.de/popForms.php?load=popMail&p='.trim($output['p']).'&sid=',
 								'username' => trim($output['p']),
							);
						}
					}
				}
