...
class bot
{
	...
	public function login()
	{
		...
		$content = $this->getHTTPContent("http://gayroyal.com/ibr/default.asp", $this->indexURL, $cookiePath);
		...
		if(!($this->isLoggedIn($username,$content)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				if((strpos($content, 'We reserve the right to block') !== false) || (strpos($content, 'Zugriff verweigert') !== false) )
				{
					$this->savelog("This user being blocked from the site.");	
	...
	public function logout()
	{
		$username = $this->loginArr[$this->currentUser][$this->usernameField];
		$cookiePath = $this->getCookiePath($username);	
		$content = $this->getHTTPContent($this->logoutURL, $this->logoutURL, $cookiePath);
		$param = array();
	...
	protected function isLoggedIn($username, $content)
	{
		$cookiePath = $this->getCookiePath($username);
	...
	protected function checkLoggedIn($username, $content)
	{
		$cookiePath = $this->getCookiePath($username);
		$this->savelog("Go to INDEX page to check login status for user: ".$username);
		$content = $this->getHTTPContent("http://gayroyal.com/", $this->indexURL, $cookiePath);
		
		if(strpos($content, $this->indexURLLoggedInKeyword)!==false)//If logged in
		{
			if((strpos($content, 'We reserve the right to block') !== false) or (strpos($content, 'Zugriff verweigert') !== false) )
			{
				$this->savelog("This user being blocked from the site.");	
				DBConnect::execute_q("UPDATE user_profiles SET status='false' WHERE site_id=".$this->siteID." AND username='".$username."'");
				$loggedin = false;
