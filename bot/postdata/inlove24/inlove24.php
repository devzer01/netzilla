...
class Inlove24 extends bot
{
	public function inlove24($post)
	{
		...
		else
		{
			$this->command = array(
									"profiles" => array(
															array(
																	"username" => "kiki-murmeltier@web.de",
																	"password" => "Gja6l8ZW90Pzmr9"
																	)
														),
									"messages" => array(
															array(
		...
		$this->databaseName = "inlove24";
		$this->userhash = "";

		//Login
			$this->usernameField = "mail";
			$this->indexURL = "http://www.inlove24.de/dating/wow/index.php";
			$this->indexURLLoggedInKeyword = "Ausloggen";
			$this->loginURL = "http://www.inlove24.de/dating/loginmodul.php";
			$this->loginRefererURL = "http://www.inlove24.de/dating/wow/index.php";
			$this->loginRetry = 3;
			$this->logoutURL = "http://www.inlove24.de/dating/loginmodul.php?cookielogout=1";
		
		//Search
			$this->searchIndex = "http://www.inlove24.de/dating/kontaktanzeigen.php";
			$this->searchURL = "http://www.inlove24.de/dating/kontaktanzeigen.php";
			$this->searchRefererURL = "http://www.inlove24.de/dating/wow/index.php";			
			
			$this->searchResultsPerPage = 10;		
		
		//Message
			$this->messageURL = "http://www.inlove24.de/dating/messanger.php";
			$this->messageReferer = "http://www.inlove24.de/dating/messanger.php?outgo=4&profilstep=3&mailrecipientsfriends=";			
	...
	{		
		...
		$endloop = false; $page = 0; $pages = 0; $param = array(); $n = 1;$this->count_msg = 0;
		...
		{			
			...
			if($page == 0)
			{
				...
				$content = $this->getHTTPContent($this->searchURL, $this->searchRefererURL, $cookiePath, $search_arr, $header);
				...
				$param["eintrag"] = $page*10;
				$param["mitlaufc"] = $pages;
				
				$content = $this->getHTTPContent($this->searchURL, $this->searchRefererURL, $cookiePath, $param, $header);
			}

			$content = substr($content, strpos($content, 'function controlfield(formdoc, field1, field2) {'));
			$content = substr($content, 0, strpos($content, '<form name="formname" method="POST" action="/dating/kontaktanzeigen.php">'));
			
			$result = $this->getMembersFromSearchResult($content);

			if(count($result) > 0){
				$this->savelog("Go to Page: ".($page+1));
		...
		if($html = str_get_html($content)){
			...
			foreach ($html->find('tr') as $key => $tr) {
				...
				{	
					...
					{	
						$vowels = array("javascript:profile('","', 'profil', '900', '600');");
						$links = trim(str_replace($vowels,"",$anchor->href));	
							array_push($list, 
								array(
									"username" => trim($td[0]->plaintext),
									"links" => $links
								)
							);
	...
	private function work_visitProfile($username, $item, $cookiePath)
	{
		...
		$content = $this->getHTTPContent("http://www.inlove24.de".$item["links"], $this->searchIndex, $cookiePath);
		$this->sleep(5);
		return $content;
	}
	...
	private function work_sendMessage($username, $item, $cookiePath, $enableMoreThanOneMessage=false){
		...
		if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
		{
			...
			if($this->reserveUser($item['username']))
			{
				...
				{
					$content = $this->getHTTPContent("http://www.inlove24.de/dating/messanger.php?outgo=4&profilstep=3&mailrecipientsfriends=".$item["username"], "http://www.inlove24.de/dating/kontaktanzeigen.php", $cookiePath);
					$this->savelog("Go to send message page: ".$item['username']);					
					$this->sleep(5);

					$param = array(
						"Eintrag abschicken.x" => 66,
						"Eintrag abschicken.y" => 6,
						"contentType" => "HTML",
						"mailadvertise" => 1,
						"mailrecipientsfriends" => $item["username"],
					...
					if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
					{
						$content = $this->getHTTPContent($this->messageURL, $this->messageReferer.$item['username'], $cookiePath, $param, $headers);

						$url_log = "URL => ".$this->messageURL." \n REFERER => ".$this->messageReferer.$item['username']." \n";
						file_put_contents("sending/pm-".$username."-".$item["username"].".html",$url_log.$content);

						if(strpos($content, 'Nachrichten-Center >> OK') !== false)
						{
							$this->newMessage = true;
