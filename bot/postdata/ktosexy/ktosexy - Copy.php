...
class ktosexy extends bot
{
	...
	public function work()
	{
		...
		//$content = $this->getHTTPContent("http://www.ktosexy.de/user.php", $this->loginRefererURL, $cookiePath);
		//echo $content;exit;
		...
		for($age=$this->command['age_from']; $age<=$this->command['age_to']; $age++)
		{
			...
			{				
				$list=array();
				$log_distance = ", ".$city.", distance: ".$this->command['umkreis'];
				...
				if(!($this->checkLoggedIn($username)))
				{
					...
					$content = $this->getHTTPContent("http://www.ktosexy.de/do.php", "http://www.ktosexy.de/user.php", $cookiePath, $search_arr, $headers);
