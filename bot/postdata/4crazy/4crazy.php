...
class Crazy extends bot
{
	public function crazy($post)
	{
		...
		//Login
			...
			$this->loginURL = "http://www.4crazy.de/login/";
	...
	{		
		...
		$param = array(
				"age1" => $this->command['ageStart'],
				"age2" => $this->command['ageEnd'],
				"country" => $this->command['country'],
				"gender" => $this->command['gender'],
				"lastseen" => $this->command['lastseen'],
				"ort" => $this->command['ort'],
		...
		*/
		...
		while($loop == true){
			...
			if($number == 1){
				...
				$content = $this->getHTTPContent($this->searchURL.$number, $this->searchIndex, $cookiePath);
			}

			if(strpos($content, 'Es wurden keine weiteren Mitglieder gefunden') !== false)
			{
				$loop = false;

			}else{

				$this->savelog("Go to Page: ".$number); 
