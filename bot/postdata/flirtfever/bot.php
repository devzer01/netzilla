...
class bot
{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				elseif(!($this->isLoggedIn($username)))
				{
					...
					if($count_login>($this->loginRetry-1))
					{
						...
						DBConnect::execute_q("UPDATE user_profiles SET status='false' WHERE site_id=".$this->siteID." AND username='".$this->loginArr[$this->currentUser]["log"][$this->usernameField]."'");
	...
	public function checkTargetProfile($profile = '') {
		...
		if($profile != '') {
			...
			$this->getHTTPContent('http://www.flirt-fever.de/suche.php', $this->indexURL, $cookiePath, array(
				'name' => $profile,
				'namenssuche' => 1,
				'x' => rand(8,18),
				'y'	=> rand(10,15)
			));
			$content = $this->getHTTPContent('http://www.flirt-fever.de/suche.php?search=nach_'.$profile, $this->indexURL, $cookiePath);
			// Es wurden keine passenden User gefunden!
			if(!strpos($content,'Es wurden keine passenden User gefunden')) {
				return TRUE;
			} else {
				return FALSE;
			}
