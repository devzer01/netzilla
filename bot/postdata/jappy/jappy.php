...
class Jappy extends bot
{
	public function jappy($post)
	{
		...
		else
		{
			$this->command = array(
									...
									"messages" => array(
															array(
																	"subject" => "Hallo",
																	"message" => " poppen 48 pkt net "
																)
														),
									"profile_type" => 1,
									"target_cm" => "lovely-singles.com",
									"mask_url" => "poppen 48 pkt net",
									"start_h" => 00,
									"start_m" => 00,
									"end_h" => 00,
									"end_m" => 00,
		...
		$this->databaseName = "jappy";
		$this->userhash = "";

		//Login
			$this->usernameField = "login[u]";
			$this->indexURL = "http://www.jappy.de/";
			$this->indexURLLoggedInKeyword = "/mailbox";
			$this->loginURL = "https://www.jappy.de/password";
			$this->loginRefererURL = "https://www.jappy.de";
			$this->loginRetry = 3;
			$this->logoutURL = "http://www.jappy.de/password?logout=1&h=";

		//Search
			$this->searchPageURL = "http://www.jappy.de/search/standard";
		...
		//http://www.jappy.de/search/standard/alter
		//http://www.jappy.de/search/standard/settings/settings
		//http://www.jappy.de/backend/search/standard.php

		//$this->profileURL = "http://www.jappy.de/user/".$this->command['profiles'][0]['username'];
		$this->profileURL = "http://www.jappy.de/user/";
		...
		$this->ErrorHandler = array(
										-1 => "Die Anfrage war leider unvollstÃ¤ndig.",
										-2 => "Leere Kommentare sind nicht zulÃ¤ssig.",
										-3 => "Du darfst Kommentare nicht so kurz hintereinander verfassen.",
										-4 => "Der Kontaktfilter verhindert das Kommentieren.",
										-5 => "Eine aktive Sperre verhindert das Kommentieren.",
										-6 => "Dieser Eintrag darf nicht kommentiert werden.",
										-7 => "Der Kommentar enthÃ¤lt unerwÃ¼nschten Inhalt.",
										-8 => "Der Nutzer existiert nicht (mehr).",
										-9 => "Du wirst vom Inhaber dieses Eintrags ignoriert.",
										-10 => "Das GÃ¤stebuch ist gesperrt.",
										-500 => "Es ist ein unbekannter Fehler aufgetreten.",
										1 => "Unbekannter Fehler",
										2 => "Du bist nicht (mehr) eingeloggt",
										3 => "Dein Profil ist noch nicht aktiviert",
										4 => "Falscher Hash"
									);
	...
	public function addLoginData($users)
	{
		foreach($users as $user)
		{
			...
			$content = $this->getHTTPContent($this->loginURL, $this->loginRefererURL, $cookiePath);
			...
			if($html = str_get_html($content)){
				...
				foreach ($nodes as $node) {
					...
					if($node->name == "login[ts]"){$ts = $node->value;}
					if($node->name == "login[key]"){$key = $node->value;}
					if($node->name == "login[useSSL]"){$ssl = $node->value;}
					if($node->name == "source"){$source = $node->value;}
	...
	public function findHash($content)
	{
		if($content)
		{
			$html = str_get_html($content);

			foreach($html->find('div.headline') as $e)
			{
				foreach($e->find('a.button') as $item)
				{
					if($count>0) break;
					list($s,$h) = @explode("=",$item->href);
					$this->userhash = $h;
					$count++;
				}
			}
			...
			$this->savelog("Hash ID: ".$this->userhash);
		}
	}
	...
	public function work()
	{
		...
		$content = $this->getHTTPContent("http://www.jappy.de/search/standard/settings", "http://www.jappy.de", $cookiePath);

		$count = 0;
		$this->findHash($content);

		//$range_start = 31;
		if($this->command['isOnline']==1){
			$range_end = 392910;

			foreach ($this->cities as $k => $v) {
				if (!($k >= $this->command['range_start'] && $k <= $range_end)) {
					unset($this->cities[$k]);
				}
			}
		}
		...
		foreach($loops as $cityID => $city)
		{
			/*
			$search_arr = array(
					"ageStart"			 => $this->command['ageStart'],
					"ageEnd"			 => $this->command['ageEnd'],
					"distance"			 => $this->command['distance'],
					"gender"			 => $this->command['gender'],
					"isOnline"			 => $this->command['isOnline'],
					"locationId"		 => $cityID,
					"locationName"		 => $city,
					"name"				 => "",
			...
			*/
			...
			$start = 0; $pages = 0;
			$resultPerPages = $this->searchResultsPerPage;
			$search_arr2 = array("h" => $this->userhash,"numberOfResults" => $resultPerPages,"start" => $start);

			$content = $this->getHTTPContent("http://www.jappy.de/search/standard/settings", "http://www.jappy.de/", $cookiePath);
			//$content2 = $this->getHTTPContent("http://www.jappy.de/search/standard", "http://www.jappy.de/search/standard/settings", $cookiePath);
			...
			while($loop == false){
				...
				if($pages > 1){
					$headers = array('X-JpyAx:1');
					$content2 = $this->getHTTPContent("http://www.jappy.de/search/standard?&start=".$pages, "http://www.jappy.de/search/standard", $cookiePath,"",$headers);
				}else{
					$headers = array('Content-Type: application/x-www-form-urlencoded');
					$content2 = $this->getHTTPContent("http://www.jappy.de/search/standard/settings/alter", "http://www.jappy.de/search/standard/settings", $cookiePath,$search_arr,$headers);
				}


				if(strpos($content2, 'Keine Nutzer gefunden')!==false){
					$loop = true;
				}else{

					$this->savelog("Go to Pages: " . $pages." [age: ".$this->command['ageStart']."-".$this->command['ageEnd'].", city: ".$cityID."=>".$city."]");
					$result = $this->getMembersFromSearchResult2($username, $content2);
...
/*
	...
	private function getMembersFromSearchResult($username, $content){
		...
		if($html = str_get_html($content)){
			...
			$nodes = $html->find("#userMatching a.picture");

			foreach ($nodes as $node) {
				$text = substr($node->outertext, strpos($node->outertext, "User.showUserMatch(this, '")+26);
				$text = substr($text, 0, strpos($text, "');"));

				list($userId,$nickname) = @explode("', '",$text);
				$item = array();
				$item['username'] = trim($nickname);
				$item['userId'] = trim($userId);
				$item['h'] = $this->userhash;
	...
	private function getMembersFromSearchResult2($username, $content){
		$list = array();
		$vowels = array('\\','{"s":1,"html":"','"}');
		$content = trim(str_replace($vowels,'',$content));

		if($html = str_get_html($content)){

			$nodes = $html->find("#usResults a");

			foreach ($nodes as $node) {
				$vowels = array('/user/');
				$name = trim(str_replace($vowels,'',$node->href));

				array_push($list,array("username" => $name,"h" => $this->userhash));
			}
		}
	...
	public function checkLogin($username, $password){
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				$headers = array("Content-Type:application/x-www-form-urlencoded","Content-Length:"+strlen(http_build_query($login_arr)));
				$content = $this->getHTTPContent("https://www.jappy.de/password", "http://www.jappy.de/", $cookiePath, $login_arr,$headers);
				file_put_contents("login/".$username."-".date("YmdHis").".html",$content);

				if((strpos($content, 'Du hast entweder ein falsches <span class="fwB">Passwort</span> oder einen ungültigen <span class="fwB">Nutzernamen</span> eingegeben.')!==false) || !($this->isLoggedIn($username)))
				{
	...
	private function work_sendMessage($username, $item, $cookiePath, $enableMoreThanOneMessage=false){
		...
		if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
		{
			...
			if($this->reserveUser($item['username']))
			{
				// Go to profile page
				$content = "";
				$retry_visit_profile = 3;
				while((strpos($content, 'onclick="MessageTaskBar.loadConversation(')===false) && ($retry_visit_profile > 0))
				{
					$content = $this->work_visitProfile($username, $item, $cookiePath);
					$retry_visit_profile--;
					$this->savelog("Can not find userID, trying again.");
					$this->sleep(10);
				}
				if(!file_exists("profiles"))
					mkdir("profiles");
				file_put_contents("profiles/".$item['username'].".html",$url_log.$content);

				if($this->command['msg_type']=="pm")
				{
					$userId = substr($content, strpos($content, '/mailbox/conversation/'.$item['username']));
					$userId = substr($userId, strpos($userId, 'MessageTaskBar.loadConversation')+32);
					$userId = substr($userId, 0, strpos($userId, ')'));
					$this->savelog("Found userID: ".$userId);
					...
					if(is_numeric($userId) && ($userId>0))
					{
						...
						$content2 = $this->getHTTPContent("http://www.jappy.de/backend/messageSystem/getConversation.php?h=".$this->userhash, $this->profileURL.$item['username'], $cookiePath, $page_arr);
						...
						if($this->login(false))
						{
							...
							if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
							{
								$headers = array("Content-Type: application/x-www-form-urlencoded","X-JpyAx:1");
								$url = "http://www.jappy.de/backend/messageSystem/send.php?h=".$this->userhash;
								$url_referer = "http://www.jappy.de/user/".$item['username'];

								$this->savelog("Sending string: ".urldecode(http_build_query($message_arr)));
								...
								{
									$this->newMessage = true;
									$this->savelog("Sending message failed. [".$this->ErrorHandler[$result->code]."]");
									$this->lastSentTime = time();
									$this->count_msg++;
									$return = true;
								}
					...
					else
					{
						$this->newMessage=false;
						$this->cancelReservedUser($item['username']);
						$this->savelog("Sending message failed. Can not find user ID.");
						$return = true;
					}
				}
				elseif($this->command['msg_type']=="gb")
				{
					...
					$content = $this->getHTTPContent("http://www.jappy.de/user/".$item['username'],  $this->profileURL.$item['username'], $cookiePath);

					if(strpos($content, 'Gästebuch')!==false)
					{
						$this->sleep(5);

						$message_arr = array(
							"content" => $message,
							"entryVisibility" => $this->command['entryVisibility']
						);
						...
						{
							$headers = array("Content-Type: application/x-www-form-urlencoded","X-JpyAx:1");
							$content = $this->getHTTPContent("http://www.jappy.de/user/".$item['username']."/guestbook/entry/add", $this->profileURL.$item['username']."/guestbook", $cookiePath, $message_arr, $headers);
							file_put_contents("sending/gb-".$username."-".$item['username'].".html",$content);

							if(strpos($content, '"s":0') !== false)
							{
								$this->newMessage=true;
						...
						$this->savelog("Profile: ".$item['username']." can't send guestbook.");
	...
	private function sendTestMessage($username, $cookiePath){
		//http://www.jappy.de/mailbox/conversation/Ziegenbart
		...
		$this->savelog("Go to send test message page: ".$item["username"]);

		if(time() < ($this->lastSentTime + $this->messageSendingInterval)){
			$this->sleep(($this->lastSentTime + $this->messageSendingInterval)-time());
		}

		$this->savelog("Sending test message to ".$item["username"]);


		if($this->command['msg_type'] == "pm"){

			$content = $this->getHTTPContent($this->indexURL."mailbox/conversation/".$item['username'], $this->sendMessagePageURL, $cookiePath);
			...
			if($html = str_get_html($content)){
				...
				foreach ($nodes as $node) {
					$send_arr[$node->name] = $node->value;
				}
			}

			$send_arr["recipient"] = $item["username"];
			$send_arr["submit"] = "Senden";
			$send_arr["text"] = "hey kennen wir uns nicht?";

			$headers = array('Content-Type: application/x-www-form-urlencoded');
			$content = $this->getHTTPContent("http://www.jappy.de/mailbox/compose", $this->sendMessagePageURL, $cookiePath, $send_arr, $headers);

			file_put_contents("sending/pm-".$username."-".$item['username'].".html",$content);

			if(strpos($content, 'Die Mail wurde erfolgreich versendet')!==false)
			{
				$this->newMessage=true;
				$this->savelog("Sending message completed.");
				DBConnect::execute_q("INSERT INTO ".$this->databaseName."_sent_messages (to_username,from_username,subject,message,sent_datetime) VALUES ('".addslashes($item["username"])."','".$username."','".addslashes($subject)."','".addslashes($message)."',NOW())");
			...
			if(strpos($content, $item['username'])!==false)
			{
				$this->newMessage=true;
				$this->savelog("Sending message completed.");
				$this->lastSentTime = time();
	...
	public function checkTargetProfile($profile = '') {
		...
		if($profile != '') {
			$content = $this->getHTTPContent('http://www.jappy.de/user/'.$profile, $this->indexURL, $cookiePath);
			if(empty($content)) {
				return 'EMPTY';
			} else {
