...
class bot
{
	...
	public function login($changeProxy=true)
	{
		$this->userAgent = botutil::getAgentString();
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				/*
				if(count($this->loginArr)>0)
				{
					foreach($this->loginArr as $key => $val){
						foreach($val as $k => $v){
							$this->savelog("A: ".$key." => key: ".$k." => val: ".$v);
						}
					}
				}*/
				...
				if((strpos($content, 'Du hast entweder ein falsches <span class="fwB">Passwort</span> oder einen ung�ltigen <span class="fwB">Nutzernamen</span> eingegeben.')!==false) || !($this->isLoggedIn($username)))
				*/if((strpos($content, 'Du hast entweder ein falsches')!==false))
				{
					$this->savelog("Log in failed with profile: ".$username." $count_login times.");
	...
	public function reLogin($username)
	{
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				$this->savelog("Logging in.");
				$login_arr = $this->loginArr[$this->currentUser];
				$content = $this->getHTTPContent($this->loginURL, $this->loginRefererURL, $cookiePath, $login_arr);
				file_put_contents("login/".$username."-".date("YmdHis").".html",$content);
	...
	public function logout()
	{
		...
		$content = $this->getHTTPContent($this->logoutURL.$this->userhash, $this->loginRefererURL, $cookiePath);
		return true;
	}
	...
	public function returnUsers(){
		...
		if($this->command['login_by'] == 1){
			$result = array();
			$result[0] = botutil::getNewProfile($this->siteID, $username, $this->command, $this);
		...
		{	
			$this->savelog("Found ".count($result)." profile(s) to use.");
			$cookiePath = $this->getCookiePath($result[0]['username']);
			...
			if($html = str_get_html($content)){
				...
				foreach ($nodes as $node) {							
					...
					$login_arr = array(
						"login[u]"		=> $result[0]['username'],
						"login[p]"		=> $result[0]['password'],
						"login[ts]"		=> $ts,
						"login[key]"	=> $key,
						"login[useSSL]"	=> 0,
						"source"		=> $source					
	...
	protected function isLoggedIn($username)
	{
		...
		$content = $this->getHTTPContent("http://www.jappy.de/", $this->loginRefererURL, $cookiePath);
	...
	{
		$txt_time = $this->secondToTextTime($time);
		$this->savelog("Sleep for ".$txt_time);
		$sleep_time = $time;
		while($sleep_time>=60)
		{
			if($sleep_time!=$time)
