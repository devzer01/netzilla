...
class Lesarion extends bot
{
	private $sess_id = null;
	public function lesarion($post)
	{
		...
		$this->databaseName = "lesarion";
		$this->userhash = "";

		//Login
			$this->usernameField = "auth_login";
			$this->indexURL = "http://de.lesarion.com/";
			$this->indexURLLoggedInKeyword = "?logout=logout&logoutuser";
			$this->loginURL = "http://de.lesarion.com/account/login.php";
			$this->loginRefererURL = "http://de.lesarion.com/";
			$this->loginRetry = 3;
			$this->logoutURL = "http://de.lesarion.com/?logout=logout&logoutuser=";
		
		//Search
			$this->searchIndex = "http://de.lesarion.com/search/index.php";
			$this->searchURL = "http://www.4crazy.de/mitglieder/suche/";
			$this->searchRefererURL = "http://de.lesarion.com/search/index.php";		
			
			$this->searchResultsPerPage = 10;		
		
		//Message
			$this->messageURL = "http://de.lesarion.com/contacts/popup.php?target=email_lesarion&id=";
			$this->messageReferer = "http://de.lesarion.com/contacts/details.php?uid=";
			$this->inboxURL = "hhttp://de.lesarion.com/account/mail_overview.php?ordner=-1";
	...
	{		
		...
		$startland = $this->command['s_bundesland'];
		if($startland == 0) {
			$startland = 1;
		}
		$starttime = time();
...
// 			
		...
		for($i = 1; $i <= 16; $i++) {
			...
			$this->savelog("Go to SEARCH page / City ID : ". $startland);
			
			// http://de.lesarion.com/account/main2_ver3.php?PHPSESSID=vg7rbr3v86d2a4gb00r2353g83&login=1&action2=s		
			$content = $this->getHTTPContent($this->searchIndex, $this->inboxURL, $cookiePath);
			$html = str_get_html($content);
			if($html->find('iframe',0)) {
				$ssid  = str_replace('/account/loginrefresh_iframe.php?PHPSESSID=', '', $html->find('iframe',0)->src);
				$this->sess_id = $ssid;
			}		
			echo 'Preparing values<br />';
			$search_arr = array (
				"s_alter" => 1,
				"s_auswahl" => 2,
				"s_auswahl2" => 1,
				"s_bundesland" => $startland,
				"s_erowelt" => 0,
				"s_haushalt" => 0,
				"s_kinder" => 0,
				"s_land" => ((empty($this->command['s_land'])) ? 13 : $this->command['s_land']),
				"s_maxalter" => ((empty($this->command['ageEnd'])) ? 18 : $this->command['ageEnd']),
				"s_maxgroesse" => 220,
				"s_minalter" => ((empty($this->command['ageStart'])) ? 18 : $this->command['ageStart']),
				"s_mingroesse" => 140,
				"s_ort" => "",
				"s_pic" => 0,
				"s_piercing" => 0,
				...
				"s_sortierung" => 1,
				"s_tattoo" => 0,
				"s_umkreis" => $this->command['s_umkreis'],
				"selectedland" => "",	
				"submit" => "Suchen"
			);
			...
			$content = $this->getHTTPContent("http://de.lesarion.com/search/?action=suchinfoexe&action2=", $this->searchIndex, $cookiePath, $search_arr, $headers);
	
			$content = $this->getHTTPContent("http://de.lesarion.com/search/search2_ver4_exe.php?PHPSESSID=".$ssid, "http://de.lesarion.com/search/?action=suchinfoexe&action2=", $cookiePath, NULL, $headers);
			...
			while($endloop == false){
				...
				if($pages > 1){
					$content = $this->getHTTPContent("http://de.lesarion.com/search/search2_ver4_exe.php?block=".$pages, "http://de.lesarion.com/search/?action=suchinfoexe&action2=", $cookiePath);
				}
	
				if(strpos($content, 'keine Einträge vorhanden') == false)
				{				
					...
					if(count($result) > 0){	
						...
						foreach($result as $mid => $item){
							...
							$this->postMessage($username, $item, $cookiePath);										
	...
	private function getMembersFromSearchResult($content){
		...
		if($html = str_get_html($content)){
			...
			$nodes = $html->find("div.cell-two h1");

			foreach ($nodes as $node) {	
								
				//id
					$id = substr($node->outertext, strpos($node->outertext, '?uid=')+5);
					$id = substr($id, 0, strpos($id, '"'));

				//name
					$name = substr($node->outertext, strpos($node->outertext, '<a title="')+10);
					$name = substr($name, 0, strpos($name, '"'));

				if($this->command['online'] == 1){

					if(strpos($node->outertext, '<h1 class="online">') !== false)
					{
						array_push($list, 
							array(
								"username" => trim($name),
	...
	private function work_visitProfile($username, $item, $cookiePath)
	{
		...
		$content = $this->getHTTPContent("http://de.lesarion.com/contacts/details.php?action2=&uid=".$item["id"], $this->searchIndex, $cookiePath);
		$this->sleep(5);
		return $content;
	}
	...
	private function postMessage($username, $item, $cookiePath, $enableMoreThanOneMessage=false) {
		$return = true;
		// If not already sent
		if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
		{
			...
			if($this->reserveUser($item['username'])) {
				...
				if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
				{
					...
					$content = $this->getHTTPContent('http://de.lesarion.com/contacts/mail_new.php?PHPSESSID='.$this->sess_id.'&uid='.$item["id"].'&supportmail=&message=', $this->messageURL.$item["id"], $cookiePath);
					$html = str_get_html($content);
					
					$message_arr['realname'] = trim($html->find('input[name=realname]',0)->value);
					$message_arr['login'] = trim($html->find('input[name=login]',0)->value);
					$content = $this->getHTTPContent('http://de.lesarion.com/contacts/mail_new_exe.php?PHPSESSID='.$this->sess_id, 'http://de.lesarion.com/contacts/mail_new.php?PHPSESSID='.$this->sess_id.'&uid='.$item['id'], $cookiePath, $message_arr );
				
					file_put_contents("sending/pm-".$username."-".$item['username'].".html",$content);
					if(! strpos($content, "Fehler"))
					{
						DBConnect::execute_q("INSERT INTO lesarion_sent_messages (to_username,from_username,subject,message,sent_datetime) VALUES ('".addslashes($item['username'])."','".$username."','".addslashes($subject)."','".addslashes($message)."',NOW())");
	...
	private function work_sendMessage($username, $item, $cookiePath, $enableMoreThanOneMessage=false){
		...
		if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
		{
			...
			{
				//RANDOM SUBJECT AND MESSAGE
				$this->savelog("Random new subject and comment");
				$this->currentSubject = rand(0,count($this->command['messages'])-1);
				$this->currentMessage = rand(0,count($this->command['messages'])-1);
			...
			$this->savelog("Comment is : ".utf8_decode($message));

			// Go to profile page
			$content = $this->work_visitProfile($username, $item, $cookiePath);
			
			///reserve this user, so no other bot can send msg to
			$this->savelog("Reserving profile to send comment: ".$item['username']);
			if($this->reserveUser($item['username']))
			{
				...
				{
					$this->savelog("Go to send comment page: ".$item['username']);
					...
					if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
					{
						$content = $this->getHTTPContent("http://de.lesarion.com/contacts/details.php?action2=stream&uid=".$item["id"], "http://de.lesarion.com/search/?action=suchinfoexe&action2=", $cookiePath);
						
						if(strpos($content, 'kommentieren') !== false)
						{
							$content = substr($content, strpos($content, '<a onclick="javascript:stream.comment(')+38);
							$content = substr($content, 0, strpos($content, ');'));
														
							$vowels = array("'");
							$content = trim(str_replace($vowels,"", $content));
							list($ssid,$event_id) = @explode(",",$content);
							...
							$content = $this->getHTTPContent("http://de.lesarion.com/account/stream_update.php", "http://de.lesarion.com/contacts/details.php?action2=stream&uid=".$item["id"], $cookiePath, $param, $headers);

							$url_log = "URL => http://de.lesarion.com/account/stream_update.php\nREFERER => http://de.lesarion.com/contacts/details.php?action2=stream&uid=".$item["id"]."\n";
							...
							{
								$this->newMessage = true;
								$this->savelog("Sending comment failed.");							
								$this->lastSentTime = time();
								$return = true;
							}

						}else{
							$this->savelog("This Profile can't comment");
						}
					...
					else
					{
						...
						$this->cancelReservedUser($item['username']);
						$this->savelog("Sending comment failed. This profile reserved by other bot: ".$item['username']);
						$return = true;
					}
				}				
		...
		{
			$this->savelog("Already send comment to profile: ".$item["username"]);
			$return = true;
		}
