...
class bot
{
	...
	public function login()
	{
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				if($html->find('meta',0)) {
					$url = str_replace('1; URL=','',$html->find('meta',0)->content);
					$this->getHTTPContent($url, $this->loginURL , $cookiePath );
					$this->sleep(2);
				}
	...
	public function logout()
	{
		...
		$content = $this->getHTTPContent($this->logoutURL.$username, $this->loginRefererURL, $cookiePath);
		return true;
	}
	...
	{
		$filename = dirname($_SERVER['SCRIPT_FILENAME'])."/cookies/_".$username.".txt";
		// if($this->firsttime == TRUE) {
			// if(file_exists($filename)) {
				// $this->savelog('Clear cookie of '.$username);
				// unlink($filename);
	...
	protected function getHTTPContent($url, $referer, $cookiePath=null, $postContent=null, $customHeaders=null)
	{
		...
		// echo '<textarea style="width:400px; height:200px">'.$content.'</textarea><br />';
