...
{
	private $_table_prefix = 'reif_';
	private $_searchResultId = 0;
	private $nextSearchPage = '';
	public $sendmsg_total = 0;
	public $rootDomain = 'http://www.reif-trifft-jung.de';
	public $searchActionURL = 'http://www.reif-trifft-jung.de/de/quick_search.php';
	...
	public function __construct($post)
	{
		...
		$this->usernameField = 'login_lg';
		$this->loginURL = "http://www.reif-trifft-jung.de/index.php";
		$this->loginActionURL = 'http://www.reif-trifft-jung.de/de/index.php';
		$this->loginRefererURL = "http://www.reif-trifft-jung.de/index.php";
		$this->loginRetry = 3;
		$this->logoutURL = "";
		$this->indexURL = "http://www.finya.de/";
		$this->indexURLLoggedInKeyword = 'AUSLOGGEN';
		$this->searchURL = "http://www.reif-trifft-jung.de/de/quick_search.php";
		$this->searchNextURL = '';
	...
	public function addLoginData($users)
	{
		...
		{
			$login_arr = array(
				'login_lg' => $user['username'],
				'pass_lg' => $user['password']
			);
	...
	private function sendUserMessage($item, $username, $cookiePath) {
		...
		if($this->reserveUser($item['username']))
		{
			...
			{
				$this->getHTTPContent($this->rootDomain.'/de/mailbox.php?sel=fs&id='.$item['user_id'], $item['profile_url'], $cookiePath);
				$content = $this->getHTTPContent($this->rootDomain.'/de/mailbox.php', $this->rootDomain.'/de/mailbox.php?sel=fs&id='.$item['user_id'], $cookiePath, array(
					'body' => $message,
					'id_quickmail' => 'Bitte wählen...',
					'reply' => '',	
					'sel' => 'send',
					'send_block' => '',	
					'subject' => $subject,
				...
				if(strpos($content, 'wurde gesendet')) {
		...
		for($age=$this->command['age_from']; $age<=$this->command['age_to']; $age++)
		{
			...
			do
			{
				...
				$search_arr = array(
					...
					'country' => 0,
					'flag_country' => 0,
					'foto_only' => $this->command['foto_only'],
					'gender_1' => (($this->command['gender'] == 2) ? 1 : 2),
					'gender_2' => $this->command['gender'],
					'region' => ((empty($this->command['region'])) ? 0 : $this->command['region']),
					'relation' => $this->command['relation'],
					'search_type' => 1,
					'online_only' => $this->command['online_only'],
					'sel' => 'search'
				);
				...
				/**
					END PRE SEARCH
				**/
				$this->savelog("Search for Target age: ".$age." to ".$age.", gender: ".$this->command['gender']." / page ".$page);
				if($page == 1) {
	...
	**/
	private function getMembersFromSearchResult($username, $page, $content, $age)
	{
		...
		if(!empty($content)){
			$html = str_get_html($content);
			if(!empty($html->find('td.text'))){
				$loop = 0;
				$i = 0;
				foreach($html->find('td.text') as $lst) {
					$i++;
					$m = $i%3;
					
					if($m == 2) {
						$link = $lst->find('a',0)->href;
						$list[$loop]['profile_url'] = $this->rootDomain . $link;
						parse_str(str_replace('/de/viewprofile.php?','',$link), $output);
						$list[$loop]['user_id'] = $output['id'];
					} else {
						$list[$loop]['username'] = $lst->find('font.ua_cell_head',0)->plaintext;
					}
										
					if(($i%3) == 0) {
						
						if(empty($list[$loop]['username'])) {
							unset($list[$loop]);
						}
