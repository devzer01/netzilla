...
{
	private $_table_prefix = 'grosseleute_';
	private $_searchResultId = 0;
	private $nextSearchPage = '';
	public $sendmsg_total = 0;
	public $rootDomain = 'http://www.grosseleute.de';
	public $searchActionURL = '';
	public $sendMessageActionURL = 'http://www.grosseleute.de/index.php?form=PMNew';
	public $logged = FALSE;
	...
	public function __construct($post)
	{
		...
		$this->loginURL = "http://www.grosseleute.de/index.php?form=UserLogin";
		$this->loginActionURL = 'http://www.grosseleute.de/index.php?form=UserLogin';
		$this->loginRefererURL = "";
		$this->loginRetry = 3;
		$this->logoutURL = "";
		$this->indexURL = "http://www.grosseleute.de/";
		$this->indexURLLoggedInKeyword = 'action=UserLogout';
		$this->searchURL = "http://www.grosseleute.de/kontaktanzeigen";
		$this->searchNextURL = '';
		...
		// if($item['username'] == 'Johann')
		{
			...
			if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
			{
				...
				if(!empty($content)) {
					...
					if(!empty($html->find("input[name=idHash]",0))) {
						$idHash = $html->find("input[name=idHash]",0)->value;
					}
					if(!empty($html->find("input[name=recipients]",0))) {
						$uname = $html->find("input[name=recipients]",0)->value;
					}
				}
				$content = $this->getHTTPContent($this->sendMessageActionURL, $item['message_url'], $cookiePath, array(
					"recipients" => $uname, 
					"blindCopies" => '',
					"subject" => $subject, 
					"text" => $message,
	...
	private function searchMember($online = 0) {
		...
		for($age=$this->command['age_from']; $age<=$this->command['age_to']; $age++)
		{
			...
			do
			{
				...
				$search_arr = array(
					'formular' => 'ja',
					'geschlecht' => ((empty($this->command['gender'])) ? 1 : $this->command['gender']),
					'interesse' => 'partner',
					'layout' =>((empty($this->command['layout'])) ? '' : $this->command['layout']),
					'nurbild' =>((empty($this->command['nurbild'])) ? '' : $this->command['nurbild']),
					'abalter' => $age,
					'bisalter' => $age,
					'abcm' => 155,
					'biscm' => 245,
				...
				**/
				...
				$content = $this->getHTTPContent($this->searchURL.'?s='.$this->token, $this->searchURL.'?s='.$this->token, $cookiePath, $search_arr);
				$html = str_get_html($content);
	...
	**/
	private function getMembersFromSearchResult($username, $page, $content, $age)
	{
		...
		if(!empty($content)){
			$html = str_get_html($content);
			if($html->find('div.container-2',0)) {
				foreach($html->find('div.container-2') as $cont) {
					if(!empty($cont->find('div',1)->find('a',0))) {
						$anchor = $cont->find('div',1)->find('a',0);
						$id = explode('.html',$anchor->href);
						$uid = str_replace('profil', '', $id[0]);
						$list[] = array(
							'user_id' => $uid,
							'profile_url' => $this->rootDomain .'/'. $anchor->href,
							'message_url' => 'http://www.grosseleute.de/index.php?form=PMNew&userID='.$uid,
