...
if($bot->getAction()=='check')
{
	...
	if($bot->login())
	{
		$sql = "SELECT id, username, password FROM user_profiles WHERE site_id='".$bot->getSiteID()."' AND status = 'false' AND in_use = 'false' ORDER BY created_datetime DESC ";
		...
		if(is_array($profiles) && count($profiles))
		{
			...
			foreach($profiles as $profile)
			{
				...
				if($bot->checkUser($profile['username']))
				{
