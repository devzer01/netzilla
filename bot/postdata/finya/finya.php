...
class Finya extends bot
{
	public function finya($post)
	{
		...
		//Login
			...
			$this->loginURL = "http://www.finya.de/Index/trylogin";	
			$this->loginRefer = "http://www.finya.de";
			$this->loginRetry = 3;
			$this->logoutURL = "http://www.elflirt.de/index.php?toDo=logout";
			$this->homeURL = "http://www.finya.de/MyFinya";
	...
	public function addLoginData($users)
	{
		foreach($users as $user)
		{
			...
			$content = substr($content, strpos($content, '<form id="frmLogin" action="/Index/trylogin" spellcheck="false" autocomplete="off">'));
			$content = substr($content, 0, strpos($content, '</form>'));

			$datach = substr($content, strpos($content, 'data-ch'));
			$datach = substr($datach, 0, strpos($datach, 'data-ch2'));
			$vowels = array('data-ch="','"');
			$datach = trim(str_replace($vowels,"", $datach));

			$datach2 = substr($content, strpos($content, 'data-ch2'));
			$datach2 = substr($datach2, 0, strpos($datach2, '">'));
			$vowels = array('data-ch2="','"');
			$datach2 = trim(str_replace($vowels,"", $datach2));
			
			$aaa = hash_hmac('sha256', $user["password"], $datach2);
			$aab = hash_hmac('sha256', hash('sha256', $user["password"]), $datach);

			$login_arr = array(
				"aaa" => trim($aaa),
				"aab" => trim($aab),
				"aba" => trim($user["username"]),
				"abb" => 1
			);	
	...
	{		
		...
		$content = $this->getHTTPContent($this->homeURL, $this->homeURL, $cookiePath);	
