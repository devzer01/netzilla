			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				**/
				...
				if(!empty($content)){
					...
					$ch1 = $html->find('input[id=data-pwd]',0)->{'data-ch'};
					$ch2 = $html->find('input[id=data-pwd]',0)->{'data-ch2'};
					
					// Fill form
					$this->loginArr[$this->currentUser]['aaa'] = hash_hmac('sha256', $this->loginArr[$this->currentUser]['password'], $ch2); // CH2
					$this->loginArr[$this->currentUser]['aab'] = hash_hmac('sha256', hash('sha256',$this->loginArr[$this->currentUser]['password']), $ch1);
					$user_arr = $this->loginArr[$this->currentUser];
				...
				*/
				...
				$content = $this->getHTTPContent($this->loginActionURL, $this->rootDomain, $cookiePath, $user_arr, FALSE, array('X-Requested-With: XMLHttpRequest'));
				if(!empty($content)) {
					$json = json_decode($content);
					if(!empty($json->fwd_url)){
						$this->getHTTPContent('http://www.finya.de'.$json->fwd_url, $this->loginURL, $cookiePath);
					}
				}
