...
*/
		...
		for($age=$this->command['ageStart']; $age<=$this->command['ageEnd']; $age++)
		{
			//foreach($this->cities as $city_id => $city_name){
				...
				$member = $this->getMembersFromSearchResult($username, $city_id, $content, $search_arr, $cookiePath);
				$this->savelog("There were about ".count($member)." members found.");

				if(count($member) > 0){
					foreach($member as $mid => $item)
					{
	...
	private function getMembersFromSearchResult($username, $city, $content, $search_arr, $cookiePath){
		...
		while($endloop == false){
			...
			if($count > 1){
				...
				$search_arr = array(
					"search[PLZ]" => "",
					"search[alterbis]" => $this->command['ageEnd'], //age to
					"search[altervon]" => $this->command['ageStart'], //age from
					"search[augen]" => 0,
					"search[bildung]" => 0,
					"search[ernaehrung]" => 0,
					"search[familienstand]" => 0,
					"search[geschlecht]" => 0,
					"search[gewichtbis]" => $this->command['gewichtbis'],
					"search[gewichtvon]" => $this->command['gewichtvon'],
					"search[groeszebis]" => "",	
					"search[groeszevon]" => "",	
					"search[haare]" => 0,
					"search[hdSend]" => 1,
	...
	private function work_sendMessage($username, $item, $cookiePath, $enableMoreThanOneMessage=false){
		...
		if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
		{
			...
			{				
					...
					echo $item["link"];exit;
					$p = @explode("?",$item["link"]);
					list($f,$l) = @explode("=",$p[1]);
					$links = @explode("&",$l);

					//RID
						list($frid,$rid) = @explode("=",$links[0]);

					//http://www.flirtmit.de/alpha/mitglieder/profilneu.php?RID=245252NI=adala1991.html
					//http://www.flirtmit.de/alpha/mitglieder/profilneu.php?RID=142531&NI=zschopauperle&G=&O=1
					//http://www.flirtmit.de/alpha/mitglieder/profilpopup/formneu.php?RID=142531&N=zschopauperle
