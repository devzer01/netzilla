...
class Flirtmit extends bot
{
	...
	{		
		...
		if($this->command['country'] > 0){
			...
			if(isset($this->plz[$this->command['country']]) && ($this->command['sort'] == "entfernung")){
		...
		{	
			...
			foreach($loops as $key => $val){
				...
				if ((time() - $starttime) >= $time_limit){
					$this->savelog("Limit for script execution.");
					$this->logout();
					$this->sleep($time_sleep);
					break 2;						
				}
				...
				// Es wurden keine Mitglieder gefunden, die Deiner Suche entsprechen.Versuche es erneut, mit veränderten Suchkriterien
				if(strpos($content,'Es wurden keine Mitglieder gefunden')) {
					$this->savelog("[failed] : Search blocked to profile : ". $username);
					// Disabled Users
				...
				while($endloop == false){
					...
					//$this->savelog((time() - $starttime) ." >= ". $time_limit);
					...
					if(strpos($content, 'Es wurden keine Mitglieder gefunden, die Deiner Suche entsprechen') == false)
					{
						...
						$this->savelog("No members were found that match your search");
						$endloop = true;
					}
	...
	private function work_sendMessage($username, $item, $cookiePath, $enableMoreThanOneMessage=false){
		...
		if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
		{
			...
			{				
					...
					// /alpha/mitglieder/profilneu.php?RID=252356&NI=Phourromposs19&G=&O=0
					$p = @explode("?",$item["link"]);					
					$links = @explode("&",$p[1]);
	...
	public function testBlockSearch($username) {
		$cookiePath = $this->getCookiePath($username);
		$content = $this->getHTTPContent('http://www.flirtmit.de/alpha/mitglieder/sucheneu.php?search[hdSend]=1&search[hdSubCateg]=0&search[nickname]='.$username.'&suchen=absenden', $this->searchIndex, $cookiePath);
		if(strpos($content, 'keine Mitglieder gefunden')) {
			return TRUE;
		} else {
			return FALSE;
		}
