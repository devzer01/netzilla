...
require_once('flirtmit.php');
require_once('config.php');

$bot = new Flirtmit($_POST);
	...
	if(!empty($profiles))
	{
		...
		foreach($profiles as $profile) {
			if($bot->testLogin($profile) == TRUE) {
				if($bot->testBlockSearch($profile['username']) == TRUE) {
					...
					$bot->savelog($i.'/'.$count.' => Test failed : '.$profile['username'].' will be deleted. [Search Blocked]');
				}
			} else {
				DBConnect::execute_q("UPDATE user_profiles SET status='false' WHERE id=".$profile['id']);
				$bot->savelog($i.'/'.$count.' => Test failed : '.$profile['username'].' will be deleted. [Profile Blocked]');
			}
			$i++;
		}
