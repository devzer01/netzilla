...
{
	private $_table_prefix = 'gaycomy_';
	private $_searchResultId = 0;
	private $nextSearchPage = '';
	public $sendmsg_total = 0;
	public $rootDomain = 'http://gaycomy.de/';
	public $searchActionURL = 'http://gaycomy.de/index.php?dll=search&view_page=1';
	...
	public function __construct($post)
	{
		...
		$this->loginURL = "http://gaycomy.de/index.php?dll=register";
		$this->loginActionURL = 'http://gaycomy.de/index.php';
		$this->loginRefererURL = "";
		$this->loginRetry = 3;
		$this->logoutURL = 'http://gaycomy.de/index.php?dll=logout';
		$this->indexURL = 'http://gaycomy.de/account/0/index.html';
		$this->indexURLLoggedInKeyword = 'index.php?dll=logout';
	...
	private function sendUserMessage($item, $username, $cookiePath) {
		...
		if($this->reserveUser($item['username']))
		{
			...
			if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
			{
				...
				// POST
				$content = $this->getHTTPContent('http://gaycomy.de/index.php', $item['message_url'], $cookiePath, $message_arr);
		...
		for($age = $this->command['age_from']; $age <= $this->command['age_to']; $age++ )
		{
			...
			do
			{
				...
				if($page == 1) {
					...
					if(!empty($this->command['test']) && !empty($this->command['test_username'])) {
		$search_arr = array(
							'Extra' => array(
								'keyword' => $this->command['test_username'],
								'keyword_username' => 1,
								'zero' => 1
							),
							'do_page' => 'search',
						...
						$search_arr = array(
							...
							'SeN' => array(
								'1' => 'gender',
								'2' => $html->find('input[name="SeN[2]"]',0)->value,
								'3' => $html->find('input[name="SeN[3]"]',0)->value,
								'4' => 'age',
								'5' => 'country',
								'6' => $html->find('input[name="SeN[6]"]',0)->value,
								'7' => $html->find('input[name="SeN[7]"]',0)->value,
								'8' => $html->find('input[name="SeN[8]"]',0)->value,
								'9' => $html->find('input[name="SeN[9]"]',0)->value,
								'10' => $html->find('input[name="SeN[10]"]',0)->value
							),
							'SeT' => array(
								'1' => '3',
								'2' => '3',
							...
							'TotalNumberOfRows' => $html->find('input[name=TotalNumberOfRows]',0)->value,
							'do_page' => 'search',
							'page' => 1,
							'submit' => 'Suchen'	
						);
					...
					// Get Content
					$content = $this->getHTTPContent(
						'http://gaycomy.de/index.php?dll=search&view_page=1', 
						$this->rootDomain, 
						$cookiePath,
						$search_arr);
					...
					$content = $this->getHTTPContent(
						'http://gaycomy.de/index.php?dll=search&view_page='.($page - 1), 
						$this->rootDomain, 
						$cookiePath,
						array(
							'SavePage' => 0,
	...
	**/
	private function getMembersFromSearchResult($username, $page, $content, $age)
	{
		...
		if(!empty($content)) {
			$html = str_get_html($content);
			if(!empty($html->find('#searchblock div.Acc_ListBox'))) {
				foreach($html->find('#searchblock div.Acc_ListBox') as $div) {
					$name = $div->find('b',0)->plaintext;
					$list[] = array(
						'username' => $name,
						'profile_url' => $div->find('a',0)->href,
						'message_url' => 'http://gaycomy.de/index.php?dll=messages&sub=create&n='. $name
					);
				}
			}
		}
