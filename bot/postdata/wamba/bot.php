		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				if(!empty($content)){
					...
					if(!empty($html->find("input[name='s_post']",0))){
						$this->loginArr[$this->currentUser]['s'] = $html->find("input[name='s']",0)->value;
						$this->loginArr[$this->currentUser]['s_post'] = $html->find('input[name=s_post]',0)->value;
					}
				}
				$content = $this->getHTTPContent($this->loginActionURL, $this->indexURL, $cookiePath, $this->loginArr[$this->currentUser], null, array(
				...
				if(empty($content))
				{
					...
					if($json->a != 0 && $this->isLoggedIn($username)) {
