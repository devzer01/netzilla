...
class bot
{
	...
	public function login()
	{
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				 ...
				 * PRE Login
				 */
				$content = $this->getHTTPContent($this->loginActionURL, $this->rootDomain, $cookiePath);
				if(!empty($content)){
	...
	protected function isLoggedIn($username)
	{
		...
		$content = $this->getHTTPContent($this->indexURL, $this->loginActionURL, $cookiePath);
		if(empty($content)) {
			return false;
		} else {
			if(strpos($content, $this->indexURLLoggedInKeyword))//If logged in
			{
				if(empty($this->jsessionid)) {
					$html = str_get_html($content);
					if(!empty($html->find('a.submenulink',0 ))) {
						$href = $html->find('a.submenulink',0 )->href;
						$dat = explode("jsessionid=", $href);
						$this->jsessionid = $dat[1];
					}
				}
				$this->logged = TRUE;
				$this->savelog("This profile: ".$username." has been logged in. / SESSION ID : ".$this->jsessionid);
				return true;
			}
			else
			{
