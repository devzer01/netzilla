...
class bot
{
	...
	public function logout()
	{
		...
		$content = $this->getHTTPContent($this->logoutURL, $this->indexURL, $cookiePath);
		return true;
	}
	...
	public function returnUsers(){
		...
		if($this->command['login_by'] == 1){
			...
			$row = botutil::getNewProfile($this->siteID, $this->user_name, $this->command);
			$result[0] = $row;
		...
		if(count($result) > 0){	
			...
			$login_arr = array(
				"frmPassword" => $result[0]['password'],
				"txtLogin" => $result[0]['username']
			);	
