...
class bot
{
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				{		
					...
					$this->logged = TRUE;
					$this->savelog('Site : '.$this->getSiteID());
	...
	public function returnUsers(){
		...
		if($this->command['repeat_profile'] == "Y"){
			$sql = "SELECT id, username, password FROM user_profiles WHERE (site_id='".$this->siteID."') AND (status='true') AND (in_use='false') AND (sex='".$this->user_sex."') and (username!='".$this->user_name."') AND ((lastused < DATE_SUB(NOW(), INTERVAL 24 HOUR)) or (lastused='0000-00-00 00:00:00')) ORDER BY rand() Limit 1";
		}else{
