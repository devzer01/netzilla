...
class Altersvorsprung extends bot
{
	public function altersvorsprung($post)
	{
		...
		$this->databaseName = "altersvorsprung";
		$this->userhash = "";

		//Login
			$this->usernameField = "benutzname";
			$this->indexURL = "http://www.altersvorsprung.de/eingeloggt.php";
			$this->indexURLLoggedInKeyword = "Logout.php";
			$this->loginURL = "http://www.altersvorsprung.de/Login.php";
			$this->loginRefererURL = "http://www.altersvorsprung.de/index.html";
			$this->loginRetry = 3;
			$this->logoutURL = "http://www.altersvorsprung.de/Logout.php";
		
		//Search
			$this->searchIndex = "http://www.altersvorsprung.de/SuchePersonGallerie.php";
			$this->searchURL = "http://www.altersvorsprung.de/sendenGalerie.php";
			$this->searchRefererURL = "http://www.altersvorsprung.de/SuchePersonGallerie.php";			
		...
		if($this->command['gender'] == "einen Mann"){
			$target = "Male";
		}elseif($this->command['gender'] == "eine Frau"){
			$target = "Female";
		}
	...
	{		
		...
		$search_arr = array(
			"ewtichsuche" => $this->command['gender'],
			"ewtmaxalter" => $this->command['ageEnd'],
			"ewtminalter" => $this->command['ageStart'],
			"ewtsuchebild1" => $this->command['ewtsuchebild1'],
			"ewtsucheland" => $this->command['land'],
			"ewtsucheplz" => "",	
			"sortierennach" => $this->command['sortierennach']
		);

		$headers = array("Content-Type:application/x-www-form-urlencoded");
		$endloop = false; $pages = 0;
		
		while($endloop == false){
			
			if($pages > 0){
				$content = $this->getHTTPContent($this->searchURL."?i=".$pages, $this->searchRefererURL, $cookiePath);
			}else{
				$content = $this->getHTTPContent($this->searchIndex, $this->searchRefererURL, $cookiePath, $search_arr, $headers);
				$content = $this->getHTTPContent($this->searchURL, $this->searchRefererURL, $cookiePath);
			}
			...
			if(count($result) > 0){
				...
				$this->savelog("Go to Pages: ".($pages+1));
	...
	private function getMembersFromSearchResult($content){
		...
		if($html = str_get_html($content)){
			...
			$nodes = $html->find("div.gallery-field a");

			foreach ($nodes as $node) {	
				
				$user = substr($node->href,strpos($node->href,"geklickt=")+9);
	...
	private function work_visitProfile($username, $item, $cookiePath)
	{
		...
		$content = $this->getHTTPContent("http://www.altersvorsprung.de/".$item["link"], $this->searchIndex, $cookiePath);
		$this->sleep(5);
		return $content;
	}
	...
	private function work_sendMessage($username, $item, $cookiePath, $enableMoreThanOneMessage=false){
		...
		if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
		{
			...
			if($this->reserveUser($item['username']))
			{
				...
				$content = $this->getHTTPContent("http://www.altersvorsprung.de/NachrichtSenden.php?empfaenger=".$item["username"], "http://www.altersvorsprung.de/NachrichtSchreiben.php?geklickt=".$item["username"], $cookiePath );
				...
				if($this->command['msg_type']=="pm")
				{
					...
					$param = array(
						"nachricht1" => utf8_decode($message)
					);
				
					if(strpos($content, 'Premium Mitglied werden'))
					{
						$this->newMessage = true;
						$this->savelog("Sending message failed / Sie müssen Premiummitglied sein, um diese Funktion nutzen zu können.");							
						$this->lastSentTime = time();
						$return = true;
					}
					else if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
					{
						$content = $this->getHTTPContent("http://www.altersvorsprung.de/NachrichtSenden.php?empfaenger=".$item["username"], "http://www.altersvorsprung.de/NachrichtSchreiben.php?geklickt=".$item["username"], $cookiePath, $param, $headers);
						...
						if(strpos($content, 'Nachricht erfolgreich versendet'))
						{
							$this->newMessage = true;
						...
						elseif(strpos($content, 'Premiummitglied'))
						{
							$this->newMessage = true;
							$this->savelog("Sending message failed / Sie müssen Premiummitglied sein, um diese Funktion nutzen zu können.");							
							$this->lastSentTime = time();
	...
	{
		$content = $this->getHTTPContent("http://www.altersvorsprung.de/Postausgang.php","http://www.altersvorsprung.de/Posteingang.php", $cookiePath);
		$content = substr($content,strpos($content,'<h3 class="main">Postausgang<br /><br /></h3>'));
		$content = substr($content,0,strpos($content,'<h5>keine Freunde online</h5>'));
		...
		if($html = str_get_html($content)){
			...
			foreach ($nodes as $node) {	
				$headers = array("Content-Type:application/x-www-form-urlencoded");
				$content = $this->getHTTPContent("http://www.altersvorsprung.de/".$node->action, $cookiePath, "", $headers);				
			}
		}
	...
	private function getInboxMessages($username, $cookiePath)
	{
		...
		$count = 1;
		$endloop = false;
		$content = $this->getHTTPContent("http://www.altersvorsprung.de/Posteingang.php", "http://www.altersvorsprung.de/eingeloggt.php", $cookiePath);
		$content = substr($content, strpos($content, '<div id="box_auswahl" class="user-profile pe">'));
		$content = substr($content, 0, strpos($content, '<div id="box_singles">'));
		...
		if($html = str_get_html($content)){					
			...
			foreach($nodes as $node){
				...
				if(strpos($node->href, 'UserProfil.php?geklickt=') !== false)
				{
					$user = substr($node->href,strpos($node->href,"geklickt=")+9);
					$user = substr($user,0,strpos($user,'"'));
