...
class bot
{
	...
	public function login()
	{
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				{		
					if($this->command['msg_type'] == "pm"){
						$sql = "update user_profiles set lastused='".date("Y-m-d H:i:s", time())."' where (site_id='".$this->siteID."') and (username='".$this->user_name."')";
			...
			$content = $this->getHTTPContent("http://www.spin.de/loggedin", $this->loginRefererURL, $cookiePath);
			if($html = str_get_html($content)){				
				$nodes = $html->find("#logout a");
				foreach ($nodes as $node) {	
					$this->logoutURL = "http://www.spin.de".$node->href;					
				}
			}
		}
		else
	...
	public function logout()
	{
		...
		$headers = array("Content-Type:application/x-www-form-urlencoded");
		$content = $this->getHTTPContent("http://triff-chemnitz.de/community/startseite.html?", "	http://triff-chemnitz.de/community/startseite.html", $cookiePath, $param, $headers);
		return true;
	}
	...
	public function returnUsers(){
		...
		if($this->command['login_by'] == 1){
			...
			if($this->command['repeat_profile'] == "Y"){				
				...
				$sql = "SELECT id, username, password FROM user_profiles WHERE (site_id='".$this->siteID."') AND (status='true') AND (in_use='false') AND (sex='".$this->user_sex."') and (username!='".$this->user_name."') AND ";
				$sql .= "(";
				...
				$sql = "SELECT id, username, password FROM user_profiles WHERE (site_id='".$this->siteID."') AND (status='true') AND (in_use='false') AND (used='false') AND (sex='".$this->user_sex."') and (username!='".$this->user_name."') ORDER BY rand() Limit 1";
			}
	...
	public function savelog($msg)
	{
		...
		if(file_exists("logs/".$this->commandID."_command.log"))
		{
			...
			*/
			...
			$rs = DBConnect::assoc_query_2D("SELECT status FROM commands WHERE id='".$this->commandID."'");
			if($rs[0]["status"] == "false")
			{
				file_put_contents("logs/".$this->commandID."_latest.log","[$time] Force stop");
				file_put_contents("logs/".$this->commandID.".log","[$time] Force stop\r\n",FILE_APPEND);
				unlink("logs/".$this->commandID."_command.log");
