...
class Triffchemnitz extends bot
{
	public function triffchemnitz($post)
	{
		...
		//Login
			...
			$this->indexURL = "http://triff-chemnitz.de/community/startseite.html";
			$this->indexURLLoggedInKeyword = 'name="logout"';
			$this->loginURL = "http://triff-chemnitz.de/community/startseite.html";
			$this->loginRefererURL = "http://triff-chemnitz.de/community/startseite.html";
			$this->loginRetry = 3;
			$this->logoutURL = "http://www.eromeet.com/?menu=start&login=2";
		
		//Search
			$this->searchIndex = "http://triff-chemnitz.de/mitglieder/suche.html";
	...
	public function addLoginData($users)
	{
		...
		foreach($users as $user)
		{
			...
			$content = $this->getHTTPContent("http://triff-chemnitz.de/", "http://triff-chemnitz.de/", $cookiePath);
	...
	{		
		...
		$gender = ($this->command['gender'] == 1)? "Female" : "Male";
		$this->savelog("Search => Age: ".$this->command['ageStart']." to ".$this->command['ageEnd'].", Gender: ".$gender);
		...
		while($endloop == false)
		{
				if($this->checkLoggedIn($username))
				{
					$search_option = array(
								"action" => "search",
								"ageFrom" => $this->command['ageStart'],
								"ageTo" => $this->command['ageEnd'],
								"firstname" => "", 	
					...
					if($pages == 1)
					{
						...
						$content = $this->getHTTPContent($this->searchURL, $this->searchRefererURL, $cookiePath,$search_option ,$headers);
					}else{
						$content = $this->getHTTPContent("http://triff-chemnitz.de/mitglieder/suche/".$pages.".html", $this->searchRefererURL, $cookiePath);
					}					
					...
					if(count($result) > 0)
					{
						...
						foreach($result as $mid => $item){
							...
							if (in_array($item["username"], $this->profile_tmp))
							{
								$endloop = true;
							}else{
								...
								$this->profile_tmp[] = $item["username"];
	...
	private function getMembersFromSearchResult($content){
		...
		if($html = str_get_html($content)){						
			$nodes = $html->find("a.nick");
							
			foreach ($nodes as $node) {
				array_push($list, array("username" => trim($node->innertext), "link" => trim($node->href)));
			}
		}
	...
	private function work_visitProfile($username, $item, $cookiePath)
	{
		...
		*/
		...
		$content = $this->getHTTPContent("http://triff-chemnitz.de/profil/".$item["username"]."/uebersicht.html", $this->searchIndex, $cookiePath);
		$this->sleep(5);
		return $content;
	}
		...
		if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
		{
			...
			if($this->reserveUser($item['username']))
			{
				if($this->command['msg_type']=="pm")
				{
					...
					if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
					{
						$content = $this->getHTTPContent("http://triff-chemnitz.de/nachrichten/schreiben/".$item["username"].".html", "http://triff-chemnitz.de/profil/".$item["username"]."/uebersicht.html", $cookiePath);
						...
						$param = array(
							...
							"title" => utf8_decode($subject),
							"token" => 	$this->token
						);

						$headers = array("Content-Type:application/x-www-form-urlencoded");
						$content = $this->getHTTPContent("http://triff-chemnitz.de/nachrichten/schreiben/".$item["username"].".html", "http://triff-chemnitz.de/nachrichten/schreiben/".$item["username"].".html", $cookiePath, $param, $headers);

						$url_log = "URL => http://triff-chemnitz.de/nachrichten/schreiben/".$item["username"].".html \nREFERER => http://triff-chemnitz.de/nachrichten/schreiben/".$item["username"].".html \n";
						file_put_contents("sending/pm-".$username."-".$item["username"].".html",$url_log.$content);

						if(strpos($content, 'Nachricht wurde erstellt') !== false)
						{
							$this->newMessage = true;
