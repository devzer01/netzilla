...
class Elflirt extends bot
{
	public function elflirt($post)
	{
		...
		$this->databaseName = "elflirt";
		$this->userhash = "";

		//Login
			$this->usernameField = "email";
			$this->indexURL = "http://www.elflirt.de";
			$this->indexURLLoggedInKeyword = "toDo=logout";
			$this->loginURL = "http://www.elflirt.de/register.php?action=login&language=german";	
			$this->loginRefer = "http://www.elflirt.de";
			$this->loginRetry = 3;
			$this->logoutURL = "http://www.elflirt.de/index.php?toDo=logout";
	...
	{		
		...
		$content = $this->getHTTPContent($this->searchIndex, $this->homeURL, $cookiePath);	
		
		//Get User ID
			$user_content=substr($content,strpos($content,'var userID   = "')+14);
			$user_content=substr($user_content,0,strpos($user_content,'var wc_hash'));	
			$uid = trim(str_replace(str_split('";'),'',$user_content));
		...
		while($endloop == false){
			...
			if($this->command['search'] == 1){
				
				$search_option = "";

				if($pages > 0){
					$search_option = "?page=" . $pages;
				}
				
				$content = $this->getHTTPContent($this->onlineUserUrl.$search_option, $this->searchURL, $cookiePath);
			
			}elseif($this->command['search'] == 2){
				...
				$content = $this->getHTTPContent($this->newUserUrl.$search_option, $this->searchURL, $cookiePath);
				...
				if($pages <= 0){
					$search_option = array(
						"Suchen" =>	"Suchen",
						"affection" => $this->command['affection'],
						"age_from" => $this->command['ageStart'],
						"age_to" =>	$this->command['ageEnd'],
						"city" =>	"",
						"country" => "de",
						"gender" => $this->command['gender'],
						"lookfor" => $this->command['lookfor'],
						"name" =>	"",
						"picture" => $this->command['picture'],
						"plz" =>	99,
						"utype" =>	0
					);

					$headers = array('Content-Type: application/x-www-form-urlencoded');
					$content = $this->getHTTPContent($this->searchURL, $this->searchURL, $cookiePath, $search_option, $headers);
				}else{
					$search_option = "ef_search.php?toDo=search&page=".$pages."&name=&gender=".$this->command['gender']."&plz=99&city=&country=99&agefrom=".$this->command['ageStart']."&ageto=".$this->command['ageEnd']."&picture=".$this->command['picture']."&lookfor=".$this->command['lookfor']."&affection=".$this->command['affection']."&utype=0";
					$content = $this->getHTTPContent($this->indexURL."/".$search_option, $this->searchURL, $cookiePath);
				}

			}

			if((strpos($content, 'Keine Suchergebnisse') == false) or (strpos($content, 'Keine weiteren Ergebnisse gefunden') == false)){

				$content=substr($content,strpos($content,'<div class="list_td_color1">'));
				$content=substr($content,0,strpos($content,'<div id="pagination">'));
				...
				if(count($result) > 0){	
					...
					foreach($result as $mid => $item){
						...
						//$item = array();
						//$item["username"] = "-Nordlicht";
						//$item["link"] = "profile.php?id=100310";
						$send = 1;

						if($this->command['search'] == 1 or $this->command['search'] == 2){
							if($this->command['gender'] == 1){
								if($item["gender"] == "female") $send = 0;
							}elseif($this->command['gender'] == 0){
								if($item["gender"] == "male") $send = 0;
							}else{
								$send = 1;
							}
						}
					...
					if($this->command['version']==1)
					{
						$this->deleteAllOutboxMessages($username, $uid, $cookiePath);								
					}				
					elseif($this->command['version']==2)
					{
