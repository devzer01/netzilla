...
class bot
{
	...
	{
		$sql = "SELECT COUNT(*) AS cnt FROM email WHERE user LIKE '%gmx.%' AND user NOT IN (SELECT email FROM user_profiles WHERE site_id = " . $this->siteID . ")";
	...
	{
		$sql = "SELECT user, password FROM email WHERE user LIKE '%gmx.%' AND user NOT IN (SELECT email FROM user_profiles WHERE site_id = " . $this->siteID . ") LIMIT 1 ";
		return DBConnect::assoc_query_1D($sql);
	}
	
	public function removeEmailAccount($user)
	{
		$sql = "DELETE FROM email WHERE user = '" . mysql_real_escape_string($user) . "' LIMIT 1";
	...
	{
		$response = $this->getHTTPContent('http://www.google.com'); //return header no
	}
	
	public function login()
	{
		$this->savelog("trying to login");
		
		$username = $this->loginArr[0][$this->usernameField];
		$password = $this->loginArr[0]['password'];
		
	
		$url = "http://mapi.kwick.de/2.0/login?__env=json&__auth=a2e4822a98337283e39f7b60acf85ec9&__app=AndroidApp";
		$headers = array('Content-type: application/x-www-form-urlencoded');
		$data = 'kwick_username=' . $username . '&kwick_password=' . $password;
	
		$response = $this->getHTTPContent($url, null, $data, $headers); //return header no
	
		$json = json_decode($response, true);
		
		//{"id":10906713,"username":"angeliukass","age":25,"gender":1,"city":"Frankfurt (Oder)","thumbnail":"http:\/\/a.kw.cx\/live\/touch\/gfx\/nopic_100_f.jpg","onlinestatus":0},"redirect":""}"
	
		if (isset($json['session_name']) && $json['loggedIn'] == true) {
			$this->profile = $json['context'];
			$this->session_id = $json['session_id'];
			$this->user_id = $json['userid'];
			$this->session_name = $json['session_name'];
			$this->md5 = $json['session_id'] . "0cdf43fd291f9fa5330c79c5e1e91c3df87d6d66";    //paramJSONObject.getString("session_id") + getContext().getString(2131624002)
			$this->savelog("Logged in with profile: ".$username);
			botutil::profileCount($this->getSiteID(), $username);
			return true;
		} else
		{
			$this->savelog("login failure, please see debug" . $response);
	...
	protected function getHTTPContent($url, $useragent = null, $postContent=null, $headers = array(), $return_header = false)
	{
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		if ($useragent !== null) curl_setopt($ch, CURLOPT_USERAGENT, $useragent);
