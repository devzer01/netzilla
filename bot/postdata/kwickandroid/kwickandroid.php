...
class kwickandroid extends bot 
{
	private $_table_prefix = 'kwickandroid_';
	...
	function clear_inbox($username, $password) {
		...
		$res = pop_open("ssl://pop.gmx.net", 995, $username, $password);
	...
	public function sendMessage($rcpt, $message)
	{
		...
		$url = "http://mapi.kwick.de/2.0/message/send?__env=json&__auth=" . md5($this->md5) . "&__app=AndroidApp&" . $this->session_name . "=" . $this->session_id;
		$headers = array('Content-type: application/x-www-form-urlencoded');
		$data = 'receiver=' . $rcpt . '&msgText=' . urlencode($message);
		
		$response = $this->getHTTPContent($url, null, $data, $headers); //return header no
		
		$json = json_decode($response, true);
		
		if (isset($json['textMsg']) && $json['textMsg'] == 'Message sent.') {
			return true;
		}
	...
	{
		$url = "http://mapi.kwick.de/2.0/status/get?__env=json&__auth=" . md5($this->md5) . "&__app=AndroidApp&" . $this->session_name . "=" . $this->session_id;
		//$headers = array('Content-type: application/x-www-form-urlencoded');
		//$data = 'kwick_username=' . $this->profile['profile']['profile_id'] . '&kwick_password=' . $rcpt;
	
		$response = $this->getHTTPContent($url); //, null, $data, $headers); //return header no
	
		//$json = json_decode($response, true);
	
		$this->savelog("Ping response " . $response);
	}
	
	public function infobox()
	{
		$url = "http://mapi.kwick.de/2.0/infobox?__env=json&__auth=" . md5($this->md5) . "&__app=AndroidApp&" . $this->session_name . "=" . $this->session_id;
		//$headers = array('Content-type: application/x-www-form-urlencoded');
		//$data = 'kwick_username=' . $this->profile['profile']['profile_id'] . '&kwick_password=' . $rcpt;
		...
		$this->savelog("Infobox response " . $response);
	}
	
	public function community()
	{
		$url = "http://mapi.kwick.de/2.0/index/community/get/0?__env=json&__auth=" . md5($this->md5) . "&__app=AndroidApp&" . $this->session_name . "=" . $this->session_id;
		//$headers = array('Content-type: application/x-www-form-urlencoded');
		//$data = 'kwick_username=' . $this->profile['profile']['profile_id'] . '&kwick_password=' . $rcpt;
		...
		$this->savelog("Community response " . $response);
	}
	
	public function defaultSearch()
	{
		$url = "http://mapi.kwick.de/2.0/search/members?limit=20&offset=0&__env=json&__auth=" . md5($this->md5) . "&__app=AndroidApp&" . $this->session_name . "=" . $this->session_id;
		//$headers = array('Content-type: application/x-www-form-urlencoded');
		//$data = 'kwick_username=' . $this->profile['profile']['profile_id'] . '&kwick_password=' . $rcpt;
		...
		$this->savelog("Default Search " . $response);
	}
	
	public function viewProfile($username)
	{
		$url = "http://mapi.kwick.de/2.0/" . $username . "?__env=json&__auth=" . md5($this->md5) . "&__app=AndroidApp&" . $this->session_name . "=" . $this->session_id;
		//$headers = array('Content-type: application/x-www-form-urlencoded');
		//$data = 'kwick_username=' . $this->profile['profile']['profile_id'] . '&kwick_password=' . $rcpt;
		...
		$this->savelog("View Profile ");
	
		if ($json['perms']['MESSAGE'] == 1) {
			return  true;
		}
	...
	public function search($age_from, $age_to, $gender, $online)
	{
		$url = "http://mapi.kwick.de/2.0/search/members?limit=20&offset=0age_from=" . $age_from . "&gender=" . $gender . "&age_to=" . $age_to . "&distance=0&online=" . $online . "&__env=json&__auth=" . md5($this->md5) . "&__app=AndroidApp&" . $this->session_name . "=" . $this->session_id;
		//$headers = array('Content-type: application/x-www-form-urlencoded');
		//$data = 'kwick_username=' . $this->profile['profile']['profile_id'] . '&kwick_password=' . $rcpt;
...
03-04 23:41:59.884: D/org.apache.http.wire(1230): << "Alternate-Protocol: 443:quic[EOL]"
	...
	public function startCreateAccount()
	{
		...
		$ret = $this->createProfile($name, $account['user'], strrev($account['password']), $location['zip'], $location['name'], $this->command['gender'], $this->command['pref_gender'], $dob, 18, 60);
		...
		if ($ret) {
			...
			$this->performActivation($link, $account['user'], strrev($account['password']));
	...
	public function work()
	{
		...
		$usernameFrom = $this->profile['username']; //['nickname'];
		...
		{	
			...
			$profiles = $this->search($age, $age + 1, $this->command['pref_gender'], $this->command['online_user']);
			//$profiles = $this->loadProfilesWithImages(, $this->profile['profile']['lon'], $this->profile['profile']['lat'], $page, $age, $age+1);
			
			if (!isset($profiles['users']) || count($profiles['users']) == 0) {
				$this->savelog("Search rendered no results " . $profiles);
				continue;
			}
			
			$this->savelog("Found ".count($profiles['users'])." member(s)");
			
			for ($i=0; $i< count($profiles['users']); $i++) {
				
				$member_id = $profiles['users'][$i]['id'];
				$username = $profiles['users'][$i]['username'];
								
				if(!$this->viewProfile($username)) continue;
						...
						{								
							...
							if(!$this->isAlreadySent($username))
							{
								if (trim($this->command['test_username']) != '') {
									$this->sendMessage($this->command['test_username'], $message);
									$this->savelog("Test Message Completed");
									$this->savelog("FINISHED");
									exit;
								}
								
								if($this->sendMessage($username, $message))
								{
