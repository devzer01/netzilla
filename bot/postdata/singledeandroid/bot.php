...
class bot
{
	...
	{
		$sql = "SELECT COUNT(*) AS cnt FROM email WHERE status = 1 AND user NOT IN (SELECT email FROM user_profiles WHERE site_id = " . $this->siteID . ")";
		$result = DBConnect::assoc_query_1D($sql);
		$sql = "SELECT COUNT(*) AS cnt FROM email_alias WHERE alias NOT IN (SELECT email FROM user_profiles WHERE site_id = " . $this->siteID . ")";
		$result2 = DBConnect::assoc_query_1D($sql);
		return $result['cnt'] + $result2['cnt'];
	}
	
	public function getNextEmailAccount()
	{
		$sql = "SELECT id, user, password FROM email WHERE status = 1 AND user NOT IN (SELECT email FROM user_profiles WHERE site_id = " . $this->siteID . ") LIMIT 1 ";
		return DBConnect::assoc_query_1D($sql);
	}
	
	//getNextEmailAliasAccount
	public function getNextEmailAliasAccount($email_id)
	{
		$sql = "SELECT alias FROM email_alias WHERE email_id = " . $email_id . " AND alias NOT IN (SELECT email FROM user_profiles WHERE site_id = " . $this->siteID . " AND status = 1) LIMIT 1 ";
		$alias = DBConnect::assoc_query_1D($sql);
		$sql = "SELECT id, user, password FROM email WHERE id = " . $email_id . " LIMIT 1 ";
		$email = DBConnect::assoc_query_1D($sql);
		return array_merge($email, $alias);
	}
	
	//hasMoreEmailAlias
	public function hasMoreEmailAlias($email_id)
	{
		$sql = "SELECT COUNT(*) AS cnt FROM email_alias WHERE email_id = " . $email_id . " AND alias NOT IN (SELECT email FROM user_profiles WHERE site_id = " . $this->siteID . " AND status = 1) ";
		$count = DBConnect::assoc_query_1D($sql);
		if ($count['cnt'] > 0) return true;
		return false;
	}
	
	public function isMobileDataActive()
	{
		return true;
		...
		if ($API->connect('192.168.1.1', 'admin', 'netzillacompany')) {
			...
			{
				if($interface["name"] == "PPTP-Germany1")
				{
					if($interface["running"]=="true") $return=true;
				}
			}
			$API->disconnect();
		}
	...
	public function removeEmailAccount($user)
	{
		$sql = "UPDATE email SET status = 0 WHERE user = '" . mysql_real_escape_string($user) . "' LIMIT 1";
	...
	protected function getHTTPContent($url, $useragent, $postContent=null, $headers = array(), $return_header = false)
	{
		$ch = curl_init();
