...
class botwork extends bot
{
	...
	public function __construct($post)
	{
		...
		$this->loginURL = "http://www.injoyage.de/";
		$this->loginActionURL = 'http://www.injoyage.de/login';
		$this->loginRefererURL = "http://www.injoyage.de/login";
		$this->loginRetry = 3;
		$this->logoutURL = 'http://www.injoyage.de/logout/';
		$this->indexURL = 'http://www.injoyage.de/news/all';
		$this->indexURLLoggedInKeyword = '/logout/';
		$this->searchURL = 'http://www.injoyage.de/matching';
		$this->searchNextURL = '';
	...
	private function liveSession($url) {
		if($this->logged == TRUE){
			$username = $this->loginArr[$this->currentUser]['data']['User'][$this->usernameField];
			$cookiePath = $this->getCookiePath($username);
			$this->getHTTPContent('http://www.injoyage.de/heartbeat/', $url, $cookiePath, null, FALSE, array('X-Requested-With: XMLHttpRequest'));
			$this->getHTTPContent('http://www.injoyage.de/alert/', $url, $cookiePath, null, FALSE, array('X-Requested-With: XMLHttpRequest'));
			// $this->getHTTPContent('http://www.injoyage.de/refresh/', $url, $cookiePath, null, FALSE, array('X-Requested-With: XMLHttpRequest'));
		}
	}
	...
	private function sendUserMessage($item, $username, $cookiePath) {
		...
		if($this->reserveUser($item['username']))
		{
			...
			$this->liveSession($item['message_url']);
			...
			if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
			{
				...
				if(!empty($html->find('input[name="data[_Token][fields]"]',0))) {
					...
					$message_arr = array(
						...
						'data' => array(
							...
							'_Token' => array(
								'fields' => $html->find('input[name="data[_Token][fields]"]',0)->value,
								'key' => $html->find('input[name="data[_Token][key]"]',0)->value
							)
						)
					);
					
					$field = $html->find("input[name='data[_Token][fields]']",0)->value;
					echo '<p>Field : ', $field,'</p>';
					// $message_arr = '_method=POST&data%5B_Token%5D%5Bkey%5D='.$html->find('input[name="data[_Token][key]"]',0)->value.'&data%5BMessage%5D%5Bbody%5D='.urlencode(trim($message)).'&data%5B_Token%5D%5Bfields%5D='.urlencode($field);
					
					// Go to Message Form
					$this->liveSession($item['message_url']);
					// sleep(2);
					// $this->getHTTPContent('http://www.injoyage.de/ajax/load/message/unread/'.strtolower($item['username']), $item['message_url'], $cookiePath, NULL, FALSE, array(
					...
					$content = $this->getHTTPContent(
						'http://www.injoyage.de/ajax/message/dialog/'. strtolower($item['username']), 
						'http://www.injoyage.de/message/dialog/'.strtolower($item['username']), 
						$cookiePath, 
						$message_arr, 
						FALSE,
						array(
		...
		for($age = $this->command['age_from']; $age <= $this->command['age_to']; $age++ )
		{
			...
			do
			{
				...
				if(!empty($this->command['online'])) {
					$this->savelog("Search for Target Online Users age: ".$age." to ".$age." / page ".$page);
				} else {
				...
				if($page == 1) {
					...
					$content = $this->getHTTPContent(
						'http://www.injoyage.de/matching', 
						$this->rootDomain, 
						$cookiePath);
					$html = str_get_html($content);
					$this->liveSession('http://www.injoyage.de/matching');
					...
					// $search_arr = array(
						...
						// 'data' => array(
							...
							// )
						// ),
						// 'filter_matchings_btn' => 'filter_matchings_btn'
					// );
					
					
					$search_arr = '_method=POST&data%5B_Token%5D%5Bkey%5D='.$html->find('input[name="data[_Token][key]"]',0)->value.'&data%5BMatch%5D%5BUser%5D%5Bage_from%5D='.$age.'&data%5BMatch%5D%5BUser%5D%5Bage_to%5D='.$age.'&data%5BMatch%5D%5BUser%5D%5Bradius%5D=1000&filter_matchings_btn=filter_matchings_btn&data%5B_Token%5D%5Bfields%5D='.$html->find('input[name="data[_Token][fields]"]',0)->value;
					
					// SET GENDER
					$content = $this->getHTTPContent(
						'http://www.injoyage.de/smatch/user/gender/'.$this->command['gender'], 
						$this->rootDomain, 
						$cookiePath,
						NULL, 
						FALSE, 
					...
					// SORT BY
					$content = $this->getHTTPContent(
						'http://www.injoyage.de/smatch/user/sortby/'.$this->command['sortby'], 
						$this->rootDomain, 
						$cookiePath,
						NULL, 
						FALSE, 
					...
					// PHOTO
					$content = $this->getHTTPContent(
						'http://www.injoyage.de/smatch/user/picture/'.(empty($this->command['picture']) ? 0 : $this->command['picture']),  // 0 , 1
						$this->rootDomain, 
						$cookiePath,
						NULL, 
						FALSE, 
					...
					$content = $this->getHTTPContent(
						...
						FALSE,
						array(
							'Content-Length: '.strlen($search_arr),
					...
					$content = $this->getHTTPContent(
						'http://www.injoyage.de/matching/page:'.$page, 
						$this->rootDomain, 
						$cookiePath);
				}
	...
	**/
	private function getMembersFromSearchResult($username, $page, $content, $age)
	{
		...
		if(!empty($content)) {
			$html = str_get_html($content);
			if(!empty($html->find("div.home",0))) {
				foreach($html->find("div.matching_li a.matching_thumb") as $anchor){
					$username = str_replace('/','',$anchor->href);
					$list[] = array(
						'profile_url' => 'http://www.injoyage.de/'.$username,
						'message_url' => 'http://www.injoyage.de/message/dialog/'.$username,
						'username' => $username
					);
				}
			}
