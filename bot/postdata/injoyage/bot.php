		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				if(!empty($content)) {
					...
					$this->loginArr[$this->currentUser]['data']['_Token']['key'] = $html->find('input[name="data[_Token][key]"]',0)->value;
					$this->loginArr[$this->currentUser]['data']['_Token']['fields'] = $html->find('input[name="data[_Token][fields]"]',0)->value;
				}
				
				// Log
				$content = $this->getHTTPContent($this->loginActionURL, $this->loginURL, $cookiePath, $this->loginArr[$this->currentUser]);
				if(!empty($content)) {
	...
	protected function getHTTPContent($url, $referer, $cookiePath=null, $postContent=null, $get_info = FALSE, $header = null)
	{
		...
		if($postContent !== null)
		{
			if($this->_special_post == 1) {
				...
				curl_setopt($ch, CURLOPT_POSTFIELDS, ((!is_array($postContent)) ? $postContent : http_build_query($postContent)));
				
				/** use a max of 256KB of RAM before going to disk */
				// $body = ((!is_array($postContent)) ? $postContent : http_build_query($postContent));
				// die($body);
