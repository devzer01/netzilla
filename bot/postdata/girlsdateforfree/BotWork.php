...
{
	private $_table_prefix = 'girlsdateforfree_';
	private $_searchResultId = 0;
	private $nextSearchPage = '';
	private $target = 'Male';
	public $sendmsg_total = 0;
	public $rootDomain = 'https://www.girlsdateforfree.com/';
	...
	public function __construct($post)
	{
		...
		$this->loginURL = "";
		$this->loginActionURL = 'https://www.girlsdateforfree.com/login.html';
		$this->loginRefererURL = "";
		$this->loginRetry = 3;
		$this->logoutURL = "http://www.girlsdateforfree.com/logout.asp";
		$this->indexURL = "http://www.girlsdateforfree.com/mypage.php?source=login&chapter=mypage";
		$this->indexURLLoggedInKeyword = 'member.profile.php';
		$this->searchURL = "http://www.girlsdateforfree.com/find.php";
		$this->searchNextURL = '';
		...
		if($this->reserveUser($item['username']))
		{
			...
			{			
				if(!empty($content)) {
					...
					$user_id = $html->find('a[id=block_button]',0)->xid;
					...
					// {"mail_status":"Message sent OK.<BR><BR>A copy has been kept in your sent mail folder.","nick":"kenneth","mail_sent":true,"specialDelivery":"0"}
	...
	**/
	private function getMembersFromSearchResult($username, $page, $content)
	{
		...
		if(!empty($content)){
			...
			if(!empty($html->find('div.profiles',0))) {
				foreach($html->find('div.profiles',0)->find('span.screenname') as $profile) {
					$anchor = $profile->find('a',0);
					$uid = str_replace('/member.profile.php?oid=','', $anchor->href);
					$list[] = array(
						'username' => trim($anchor->plaintext),
						'profile_url' => 'http://www.girlsdateforfree.com'. $anchor->href,
						'message_url' => 'http://www.girlsdateforfree.com/mailbox/write/-/'.$uid.'.html',
						'user_id' => $uid
					);
				}
			}
	...
	public function checkTargetProfile($profile = '') {
		...
		if($profile != '') {
			$content = $this->getHTTPContent('http://www.girlsdateforfree.com/find.php?tab=screenname&screenname='.$profile.'&go=', $this->rootDomain, $cookiePath);
