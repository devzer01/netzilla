...
class botwork extends bot
{
	...
	public function __construct($post)
	{
		...
		$this->loginURL = "http://gay.de/einloggen";
		$this->loginActionURL = 'http://gay.de/einloggen';
		$this->loginRefererURL = "";
		$this->loginRetry = 3;
		$this->logoutURL = 'http://gay.de/ausloggen/';
		$this->indexURL = 'http://gay.de/';
		$this->indexURLLoggedInKeyword = '/ausloggen/';
		$this->searchURL = 'http://gay.de/kontakte';
		$this->searchNextURL = '';
	...
	private function sendUserMessage($item, $username, $cookiePath) {
		...
		if($this->reserveUser($item['username']))
		{
			...
			if(!$this->isAlreadySent($item['username']) || $enableMoreThanOneMessage)
			{
				...
				// Crawl
				$this->nullPost = 1;
				$content = $this->getHTTPContent($item['message_url'], $item['profile_url'], $cookiePath, NULL, FALSE, array(
					'X-Prototype-Version: 1.7',
					'X-Requested-With: XMLHttpRequest'
				));
				$message_arr = array(
					'commit' => 'Nachricht versenden',
				...
				// POST
				$content = $this->getHTTPContent('http://gay.de/message/send/page_action/send', $item['profile_url'], $cookiePath, $message_arr, FALSE, array(
					'X-Prototype-Version: 1.7',
					'X-Requested-With: XMLHttpRequest'
				));
				
				// Ihre Nachricht wurde erfolgreich an antonR74 gesendet
				if(strpos($content, 'gayLightboxClose(true)')) {
			...
			do
			{
				...
				if($page == 1) {
						...
						if(!empty($this->command['online'])) {
							$search_arr['status']['online'] = 1;
						}
						// Post
						$this->getHTTPContent(
							'http://gay.de/suchergebnisse', 
							$this->rootDomain, 
							$cookiePath,
							$search_arr);
					...
					$content = $this->getHTTPContent(
						'http://gay.de/search/searchResultAjax', 
						$this->rootDomain, 
						$cookiePath,
						array(
							'page' => $page
	...
	**/
	private function getMembersFromSearchResult($username, $page, $content, $age)
	{
		...
		if(!empty($content)) {
			$html = str_get_html($content);
			if($html->find("span.shortSearchResultNickname",0)) {
				foreach($html->find("span.shortSearchResultNickname") as $span) {
					$uname = trim($span->plaintext);
					$list[] = array(
						'profile_url' => 'http://gay.de/kontakte/'.$uname,
						'message_url' => 'http://gay.de/nachrichten/senden/nickname/'.$uname,
						'username' => $uname
					);
				}
			}
