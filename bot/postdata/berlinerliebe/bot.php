...
class bot
{
	...
	public function logout()
	{
		...
		$content = $this->getHTTPContent(
			'http://berlinerliebe.de/de_DE/'.$this->user_name.'/community_logout', 
			'http://berlinerliebe.de/de_DE/'.$this->user_name.'/community_logout', 
			$cookiePath);
		return true;
	}
	...
	protected function isLoggedIn($username)
	{
		...
		$content = $this->getHTTPContent('http://berlinerliebe.de/de_DE/'.strtolower($username).'/usr_start', 'http://berlinerliebe.de/de_DE/'.strtolower($username).'/usr', $cookiePath);
		
		if(empty($content)) {
			$this->savelog("Failed : Tor Connection is slow & CURL Timed out ...");
			return false;
		} else {
			if(strpos($content, $username))//If logged in
			{
				$this->getHTTPContent('http://berlinerliebe.de/de_DE/'.strtolower($username).'/usr_bottom', 'http://berlinerliebe.de/de_DE/'.strtolower($username).'/usr', $cookiePath);
				$this->user_name = strtolower($username);
				$this->logged = TRUE;
