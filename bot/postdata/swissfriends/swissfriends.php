...
class Swissfriends extends bot
{
	public $sendmsg_total = 0;
	public function swissfriends($post)
	{
		...
		$this->databaseName = "swissfriends";
		$this->userhash = "";

		//Login
			$this->usernameField = "pseudo";
			$this->indexURL = "http://www.swissfriends.ch";
			$this->indexURLLoggedInKeyword = "Application error";
			$this->loginURL = "http://www.swissfriends.ch/identify.php?lang=de";
			$this->loginRefererURL = "http://www.swissfriends.ch/en/";
			$this->loginRetry = 3;
			$this->logoutURL = "http://www.swissfriends.ch/logout.php";
		
		//Search
			$this->searchIndex = "http://www.swissfriends.ch/zf/embed/index.php/search/online";
			$this->searchURL = "http://www.swissfriends.ch/zf/embed/index.php/search/members";
			$this->searchRefererURL = "http://www.swissfriends.ch/zfembed/item/profile/sub/search/search/members";			
			
			$this->searchResultsPerPage = 8;		
		
		//Message
			$this->messageURL = "http://www.swissfriends.ch/my_sf/mailbox/write.php";
		...
		for($age=$this->command['ageStart']; $age<=$this->command['ageEnd']; $age++)
		{
			...
			do
			{
				$param = array(
					...
					"area_approx" => 0,
					"area_approx" => 1,
					"country_region" => $this->command['region'],
					"cursor" => $page,
					"has_a_picture" => 0,
					"logic_presence" => "",	
					"mode_display" => "",	 	
					...
					"sex_preference" => 1,
					"site_lang" => $this->command['site_lang'],
					"with_vocal_profil" => 0
				);
				...
				if($this->command['online'] == 1){
					$content = $this->getHTTPContent("http://www.swissfriends.ch/zf/embed/index.php/search/online", $this->searchRefererURL, $cookiePath, $param, $headers);
				}else{
					$content = $this->getHTTPContent("http://www.swissfriends.ch/zf/embed/index.php/search/members", $this->searchRefererURL, $cookiePath, $param, $headers);
				}
				
				if($page == 1){
					$allpage = substr($content, strpos($content, 'Seite'));
					$allpage = substr($allpage, 0, strpos($allpage, '</td>'));
					list($f,$l) = @explode("/",$allpage);
					$number = trim($l);
					$this->savelog("All Page: " . $number);
				}
				...
				$page++;
			}while(count($result)>=$this->searchResultsPerPage);
		}
		/*
		while($endloop == false){
	...
	private function getMembersFromSearchResult($content){
		...
		if($html = str_get_html($content)){
				...
				$nodes = $html->find("div.hprofile");

				foreach ($nodes as $node) {	
					
					$html2 = str_get_html($node->outertext);
					$nodes2 = $html2->find(".profiltopname");
					foreach ($nodes2 as $node2) {	
						$user = trim($node2->innertext);
					}

					$uid = substr($node->outertext, strpos($node->outertext, 'openPrivateChatPopUp('));
					$uid = substr($uid, 0, strpos($uid, ','));
					$uid = trim(str_replace('openPrivateChatPopUp(','',$uid));
	...
	private function work_visitProfile($username, $item, $cookiePath)
	{
		...
		$content = $this->getHTTPContent("http://www.swissfriends.ch/member/detail/profile.php?member_id=".$item["uid"], $this->searchIndex, $cookiePath);
		$this->sleep(5);
		return $content;
	}
		...
		if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
		{
			...
			{				
				...
				if(!$this->isAlreadySent($item["username"]) || $enableMoreThanOneMessage)
				{
					...
					$headers = array('Content-Type: application/x-www-form-urlencoded');
					$content = $this->getHTTPContent($this->messageURL."?recipient_id=".$item["uid"], $this->messageURL."?recipient_id=".$item["uid"], $cookiePath, $param, $headers);
					...
					if(strpos($content, '<div id="green_text">'))
					{	
						$this->newMessage=true;
