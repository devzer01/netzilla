...
class bot
{
	...
	public function returnUsers($forceNew = FALSE) {
		...
		if(count($result) > 0){		
			...
			$login_arr = array(
				"user_login" => $result[0]['username'],
				"pass_login" => $result[0]['password'],
				"redirect" => "member.php",
				"login_form" => 1,
				"login" => "Go",
				"autologin"=> 1
	...
	protected function isLoggedIn($username)
	{
		...
		$content = $this->getHTTPContent("http://www.websingles.at/pages/site/de/", $this->loginRefer, $cookiePath);
	...
	protected function checkIsLoggedIn($username)
	{
		$cookiePath = $this->getCookiePath($username);		
		$content = $this->getHTTPContent("http://www.websingles.at/pages/site/de/", $this->loginRefer, $cookiePath);
