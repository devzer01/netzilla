...
class meetme_de extends bot
{
	public function meetme_de($post)
	{
			...
			$this->command = array(
									"profiles" => array(
															array(
																	...
																	//"password" => "workably"
																	"username" => "staudinger_elke@hotmail.com",
																	"password" => "Wesselburen1"
																	)
														),
									"messages" => array(
															array(
									...
									"receive" => array(
															array(
																	"username" => "Nydia_24Adler@hotmail.com",
																	"password" => "TreesLike"
																)
														),
									"start_h" => 00,
		...
		{
			$this->messagesPart[$i] = DBConnect::row_retrieve_2D_conv_1D("SELECT message FROM messages_part WHERE part=".$i." and target='".$this->command['target']."'");
	...
	public function work()
	{
		...
		if($this->command['send_test'] == 1){
			...
			$content = $this->getHTTPContent("http://www.meetme.com/member/".$this->command['receive'][0]['userid'], $this->loginRefererURL, $cookiePath);
			$content = substr($content,strpos($content,'<form method="post" action="/?mysession=')+28);		
			$content = substr($content,0,strpos($content,'<input type="text" class="translate" value="Quick Name Search" name="s_name" />'));
