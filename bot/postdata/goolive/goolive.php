...
class Goolive extends bot
{
	public $sendmsg_total = 0;
	public function goolive($post)
	{
		...
		$this->databaseName = "goolive";
		$this->userhash = "";

		//Login
			$this->usernameField = "username";
			$this->indexURL = "http://www.goolive.de";
			$this->indexURLLoggedInKeyword = "/logout.html";
			$this->loginURL = "http://www.goolive.de/logout.html";
			$this->loginRefererURL = "http://www.goolive.de";
			$this->loginRetry = 3;
			$this->logoutURL = "http://www.goolive.de/logout.html";
		
		//Search
			$this->searchIndex = "http://www.goolive.de/benutzersuche";
	...
	{		
		...
		while($loop == true){
			...
			//http://www.goolive.de/index.php?ext=usersuche&action=results&adv=1&searchstr=minalter,18|maxalter,20|gender,w|company_cat,egal|online,1|land,de
			//http://www.goolive.de/index.php?ext=usersuche&action=results&adv=0&searchstr=gender,m|minalter,25|maxalter,34|company_cat,0|online,1
			$content = $this->getHTTPContent("http://www.goolive.de/index.php?ext=usersuche&action=results&adv=1&searchstr=online,".$this->command['online']."|land,".$this->command['land']."|gender,".$this->command['gender']."|company_cat,egal|minalter,".$this->command['ageStart']."|maxalter,".$this->command['ageEnd']."&seite=".$pages, "http://www.goolive.de/meingoolive", $cookiePath);			
			
			if(strpos($content, 'Schade, deine Suche hat keine passenden Ergebnisse gefunden') !== false)
			{
				...
				if($this->command['online'] == 1){
					$content = substr($content, strpos($content, '<div id="main_title">'));
					$content = substr($content, 0, strpos($content, '<div id="FXMSky"></div>'));
				}
	...
	private function work_visitProfile($username, $item, $cookiePath)
	{
		...
		$content = $this->getHTTPContent("http://www.goolive.de/user/".$item["username"], $this->searchRefererURL, $cookiePath);
		$this->sleep(5);
		return $content;
	}
			...
			if($this->command['msg_type'] == "pm")
			{
				...
				{				
					...
					{				
						...
						$content = $this->getHTTPContent("http://www.goolive.de/index.php", "http://www.goolive.de/messenger/nachricht-schreiben-".$user_id, $cookiePath, $param);
						$content = $this->getHTTPContent("http://www.goolive.de/index.php?ext=messenger&show=sendmsgconfirm&user_id=".$user_id, "http://www.goolive.de/messenger/nachricht-schreiben-".$user_id, $cookiePath);
				...
				{
					$param = array(
						"entry" => utf8_decode($message),
						"action" => "add"
					);

					$content = $this->getHTTPContent("http://www.goolive.de/user/".$item['username']."/gaestebuch-1", "http://www.goolive.de/user/".$item['username']."/gaestebuch-1", $cookiePath, $param);
							
					file_put_contents("sending/gb-".$username."-".$item['username'].".html",$content);
						
					if(strpos($content, 'Vielen Dank für Deinen Gästebucheintrag') !== false)
					{
						$this->newMessage=true;
	...
	private function getInboxMessages($username, $cookiePath)
	{
		...
		$count = 1;
		$endloop = false;
		$content = $this->getHTTPContent("http://www.goolive.de/messenger/posteingang-1", "http://www.goolive.de/messenger/posteingang-1", $cookiePath);

		if($html = str_get_html($content)){					
					
			$nodes = $html->find("a.msgread0");					
				
			
				foreach($nodes as $node){
					
					if(strpos($node->href, '/messenger/nachricht-lesen') == false)
					{
						array_push($list,
							array(
								"username" => trim(str_replace("/user/","",$node->href))
							)
						);
					}
				}
	...
	public function checkTargetProfile($profile = '') {
		...
		$cookiePath = $this->getCookiePath($username);
		if($profile != '') {
			$content = $this->getHTTPContent('http://www.goolive.de/user/'.$profile, $this->indexURL, $cookiePath);
			if(strpos($content,'Nachricht schreiben')) {
				return TRUE;
			} else {
				return FALSE;
			}
