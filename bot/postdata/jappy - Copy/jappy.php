...
class Jappy extends bot
{
	public function jappy($post)
	{
		...
		else
		{
			$this->command = array(
									"profiles" => array(
															array(
																	"username" => "Aprikose_89",
																	"password" => "filleboheme1"
																	)
														),
									"messages" => array(
															array(
	...
	public function work()
	{
		...
		{
			if(($this->workCount == 1) && ($age==$this->command['ageStart']))
			{
				...
				{	
					if($this->isLoggedIn($username))
					{
						...
						*/
						...
						$content = $this->getHTTPContent("http://www.jappy.de/search/standard/settings/alter", "http://www.jappy.de/search/standard/settings", $cookiePath, $search_arr, $headers);
						//$content2 = $this->getHTTPContent("http://www.jappy.de/search/standard", "	http://www.jappy.de/search/standard/settings", $cookiePath);	
						
echo $content;exit;
						$rs = json_decode($content2, true);					
						$numberOfUsers = $rs['numberOfUsers'];			
						
						if($useLocation == 0)
							$this->savelog("Search user by[City:".$city.", Age:".$age."]");
						else
							$this->savelog("Search user by Age:".$age);
						...
						if(count($rs['result']) > 0){
							...
							$pageResult = ceil($numberOfUsers / $resultPerPages);
							$this->savelog("Found All Member ".$numberOfUsers." member(s) ".$pageResult." Page(s)");
							...
							while($count < $pageResult){
								...
								if($count > 0){
									$start = $start + $resultPerPages;
									$search_arr2 = array("h" => $this->userhash,"numberOfResults" => $resultPerPages,"start" => $start);
									$content = $this->getHTTPContent($this->searchURL, $this->searchRefererURL, $cookiePath, $search_arr);
									$content2 = $this->getHTTPContent($this->searchQueryURL, $this->searchPageURL, $cookiePath, $search_arr2);
									$rs = json_decode($content2, true);								
								}
								...
								for($i=0; $i<count($rs['result']);$i++){
									...
									{
										$item = array();
										$item['username'] = trim($rs['result'][$i]['stamp']['nickname']);
										$item['recipientId'] = $rs['result'][$i]['stamp']['userId'];
										$item['h'] = $this->userhash;									
										
										if($item['username'] != ".jappy"){

											if($this->command['isOnline']==1){

												$cProfile = $this->getHTTPContent($this->profileURL.$item['username'], $this->searchRefererURL, $cookiePath);										
												$cHtml = str_get_html($cProfile);

												foreach($cHtml->find('span.no') as $eProfile){
													
													if(isset($eProfile->innertext)){					
										...
										if(is_array($inbox))
										{
											...
											{	
												foreach($inbox as $key => $v)
												{
													...
													{											
														$item = array();
														$item['username'] = trim($v['nickname']);
														$item['recipientId'] = $v['userid'];
														//$item['username'] = "GoHoch69";
														//$item['recipientId'] = "13282466";
														$item['h'] = $this->userhash;
														
														if($item['username'] != ".jappy"){
															if(!$this->work_sendMessage($username, $item, $cookiePath)) return false;
														}
													}
													else
													{
	...
	private function getMembersFromSearchResult($username, $page, $content){
		$list = array();
		$html = str_get_html($content);
		foreach($html->find('div#userMatching') as $e)
		{
			foreach($e->find('div.link') as $item)
			{		
				list($s,$l) = explode("</small>",$item->innertext);
				$profile = array("username" => trim($l));
