...
class funcs
{
	...
	{
		$cookie_path = self::getCookiePath($username);
		$postData = array(	"filter1" => "0,".$searchData['gender'],
							"filter2" => "2,".$searchData['age_from'].",".$searchData['age_to'],
							"filter3" => "5,31536000",
							"filter4" => "1,0",
							"filter5" => "35,0",
							"filter6" => "22,74",
							...
							"sa" => 1,
							"using_saved_search" => "",
							"low" => (($page-1)*$searchResultsPerPage)+1,
							"count" => $searchResultsPerPage,
							"ajax_load" => 1
							);
		$postData = http_build_query($postData);
	...
	static function getMembersFromSearchResult($username, $page, $content)
	{
		...
		if(isset($parser->document->div))
		{
			...
			{
				$profile =  array(	"username" => $row->div[0]->div[0]->h4[0]->a[0]->span[0]->tagData,
									"age" => substr($row->div[0]->div[0]->p[0]->tagData,0,2),
									"pic" => $row->div[0]->div[0]->a[0]->img[0]->tagAttrs['src'],
								);
	...
	static function saveMembers($list,$post)
	{
		...
		{
			DBConnect::execute_q("INSERT INTO okcupid_member (username, age, gender, pic, created_datetime) VALUES ('".$member['username']."', '".$member['age']."', '".$post['gender']."', '".$member['pic']."', NOW())");
		}
	}

	static function getMembers($post,$amount)
	{
		$sql = "SELECT username FROM okcupid_member WHERE gender='".$post['data']['User']['Gender']."' LIMIT ".$amount;
		return DBConnect::assoc_query_2D($sql);
	}

	static function getNextMember($post)
	{
		$sql = "SELECT username FROM okcupid_member WHERE gender='".$post['gender']."' AND age>='".$post["age_from"]."' AND age<='".$post["age_to"]."' AND username NOT IN (SELECT to_username FROM okcupid_sent_messages) AND ((id-1)%6)=(".BOT_ID."-1) ORDER BY DATE(created_datetime) DESC, id ASC LIMIT 1";
	...
	static function sendMessage($from, $toName, $subject, $message, $sendMessageURL, $sendMessageRefererURL)
	{
		...
		$sendMessageReferer = $sendMessageRefererURL.$toName;
		$authcode = urldecode(self::getAuthCode($from, $sendMessageRefererURL));
		$sendMessagePostData = array(	"ajax" => 1,
										"sendmsg" => 1,
										"r1" => $toName,
										"subject" => "",
										"body" => $message,
		...
		if(strpos($result, '"status" : 3')>-1)
		{
			funcs::savelog("Sent message to ".$toName);
			DBConnect::execute_q("INSERT INTO okcupid_sent_messages (to_username,from_username,subject,message,sent_datetime) VALUES ('".$toName."','".$from."','".addslashes($subject)."','".addslashes($message)."',NOW())");
			return true;
		}
		else
		{
			funcs::savelog("Sending message to ".$toName." failed");
			DBConnect::execute_q("DELETE FROM okcupid_member WHERE username='".$toName."'");
			return false;
		}
	}
