...
class funcs
{
	...
	static function memberlogin($username, $password, $loginURL, $loginRefererURL, $currentIndex, $total)
	{
		...
		if($need_login)
		{
			for($count_login = 1; $count_login <= 6; $count_login++)
			{
				...
				if(strpos($result, "/do/signout")===false)
				{
	...
	{
		$cookie_path = self::getCookiePath($username);
		$postData = array(	"AGE1" => $searchData['age_from'],
							"AGE2" => $searchData['age_to'],
		...
		curl_setopt($ch, CURLOPT_PROXYPORT, PROXY_PORT);
		curl_setopt($ch, CURLOPT_PROXYTYPE, CURLPROXY_SOCKS5);
		curl_setopt($ch, CURLOPT_URL, $searchURL."/".($page-1)."/#results");
	...
	{
		$content = substr($content,strpos($content,'<div         style="height:350px;"'));
		...
		if(isset($parser->document->div))
		{
			for($i=0; $i<8; $i++)
			{
				$row = $parser->document->div[0]->div[$i];
				$temp = $row->tagData;
				$age = substr($temp, strpos($temp,",")+2);
				$age = substr($age, 0, strpos($age, " years old"));

				$pic = $row->div[0]->img[0]->tagAttrs['src'];
				if($pic == "http://www.profilepic.com/images/nopic_s.png")
					$pic="";

				$profile =  array(	"username" => $row->span[0]->tagData,
									"userid" => str_replace("/","",$parser->document->div[0]->a[$i]->tagAttrs['href']),
									"age" => $age,
									"pic" => $pic,
								);
	...
	static function saveMembers($list,$post)
	{
		...
		{
			DBConnect::execute_q("INSERT INTO profilepic_member (username, userid, age, gender, country, pic, created_datetime) VALUES ('".$member['username']."', '".$member['userid']."', '".$member['age']."', '".$post['gender']."', '".$post['country']."',  '".$member['pic']."', NOW())");
		}
	}

	static function getMembers($post,$amount)
	{
		$sql = "SELECT username FROM profilepic_member WHERE gender='".$post['data']['User']['Gender']."' LIMIT ".$amount;
	...
	static function getNextMember($post)
	{
		...
		$sql = "SELECT username,userid FROM profilepic_member WHERE gender='".$post['gender']."' AND age>='".$post["age_from"]."' AND age<='".$post["age_to"]."' ".$sql_country." AND username NOT IN (SELECT to_username FROM profilepic_sent_messages) AND ((id-1)%6)=(".BOT_ID."-1) ORDER BY DATE(created_datetime) DESC, id ASC LIMIT 1";
	...
	static function sendMessage($from, $toName, $toId, $subject, $message, $sendMessageURL, $sendMessageRefererURL)
	{
		...
		if(strpos($result, "Message Sent")>-1)
		{
			...
			DBConnect::execute_q("INSERT INTO profilepic_sent_messages (to_username,from_username,subject,message,sent_datetime) VALUES ('".$toName."','".$from."','".addslashes($subject)."','".addslashes($message)."',NOW())");
			return true;
		}
		else
		{
			funcs::savelog("Sending message to ".$toName." failed");
			DBConnect::execute_q("DELETE FROM profilepic_member WHERE username='".$toName."'");
			return false;
		}
	}
