...
class funcs
{
	...
	static function memberlogin($username, $password, $loginURL, $loginRefererURL, $currentIndex, $total)
	{
		...
		$postData = array(	'relocate' => $loginRefererURL,
							'ID' => $username,
							'Password' =>  $password
							);
		...
		if($need_login)
		{
			for($count_login = 1; $count_login <= 6; $count_login++)
			{
				...
				if(strpos($result, '<div class="msgbox_text">Please Wait</div>')===false)
				{
	...
	static function isCookieValid($username,$loginRefererURL)
	{
		...
		if(strpos($result,"logout.php")>-1)
			return true;
		else
			return false;
	}

	static function getAuthCode($username, $url)
	{
		$cookie_path = self::getCookiePath($username);
		...
		$result=substr($result,strpos($result,"authid=")+7);
	...
	static function getSearchResult($username, $searchURL, $searchReferer, $searchData, $page, $searchResultsPerPage)
	{
		...
		curl_setopt($ch, CURLOPT_PROXYPORT, PROXY_PORT);
		curl_setopt($ch, CURLOPT_PROXYTYPE, CURLPROXY_SOCKS5);
		curl_setopt($ch, CURLOPT_URL, $searchURL.$searchData['country']."/page".$page);
		curl_setopt($ch, CURLOPT_REFERER, $searchReferer.$searchData);
		curl_setopt($ch, CURLOPT_USERAGENT,"Mozilla/5.0 (Windows NT 5.1) AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.168 Safari/535.19");
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_TIMEOUT, 30); 
		curl_setopt($ch, CURLOPT_COOKIEFILE, $cookie_path);
	...
	{
		$content = substr($content,strpos($content,'<!-- Design Box [ Start ] here  -->'));
		$content = substr($content,0,strpos($content,'<!-- Design Box [ End ] here  -->'));
		...
		{
			foreach($parser->document->div as $row)
			{
				if(strpos($row->tagAttrs['class'],"searchrow_block")>-1)
				{
					$gender = $row->div[1]->div[1]->a[0]->img[0]->tagAttrs['src'];
					if(strpos($gender, "female.png")>-1)
						$gender = "Female";
					else
						$gender = "Male";

					$profile =  array(	"username" => $row->div[1]->div[0]->a[0]->tagData,
										"gender" => $gender,
										"age" => str_replace(" y/o","",$row->div[1]->div[1]->a[1]->tagData),
										"pic" => $row->div[0]->a[0]->img[0]->tagAttrs['src'],
									);
	...
	static function saveMembers($list,$post)
	{
		...
		{
			DBConnect::execute_q("INSERT INTO hspeople_member (username, age, gender, pic, created_datetime) VALUES ('".$member['username']."', '".$member['age']."', '".$member['gender']."', '".$member['pic']."', NOW())");
		}
	}

	static function getMembers($post,$amount)
	{
		$sql = "SELECT username FROM hspeople_member WHERE gender='".$post['data']['User']['Gender']."' LIMIT ".$amount;
		return DBConnect::assoc_query_2D($sql);
	}

	static function getNextMember($post)
	{
		$sql = "SELECT username FROM hspeople_member WHERE gender='".$post['gender']."' AND age>='".$post["age_from"]."' AND age<='".$post["age_to"]."' AND username NOT IN (SELECT to_username FROM hspeople_sent_messages) AND ((id-1)%6)=(".BOT_ID."-1) ORDER BY DATE(created_datetime) DESC, id ASC LIMIT 1";
	...
	static function sendMessage($from, $toName, $subject, $message, $sendMessageURL, $sendMessageRefererURL)
	{
		...
		$sendMessageReferer = $sendMessageRefererURL;
		$authcode = urldecode(self::getAuthCode($from, $sendMessageRefererURL));
		$sendMessagePostData = array(	"subject" => $subject,
										"message" => $message
									);
		...
		if(empty($result))
		{
			...
			DBConnect::execute_q("INSERT INTO hspeople_sent_messages (to_username,from_username,subject,message,sent_datetime) VALUES ('".$toName."','".$from."','".addslashes($subject)."','".addslashes($message)."',NOW())");
			return true;
		}
		else
		{
			funcs::savelog("Sending message to ".$toName." failed");
			DBConnect::execute_q("DELETE FROM hspeople_member WHERE username='".$toName."'");
			return false;
		}
	}
