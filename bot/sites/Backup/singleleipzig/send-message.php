...
require_once('singlesleipzig.php');
require_once('config.php');

$bot = new SinglesLeipzig($_POST);

if($bot->getAction()=='check')
{
	$bot->savelog("Get all profiles to check if they are still usable.");
	$profiles = DBConnect::assoc_query_2D("SELECT id, username, password FROM user_profiles WHERE site_id =".$bot->getSiteID()." AND status!='false'");
