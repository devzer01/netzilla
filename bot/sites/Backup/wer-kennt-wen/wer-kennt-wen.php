...
class werKenntWen extends bot
{
	...
	public function werKenntWen($post)
	{
		...
		else
		{
			$this->command = array(
									"profiles" => array(
															array(
																	"username" => "mitschele.mechthild@rocketmail.com",
																	"password" => "6teeN4Lyf"
																	)
														),
									"messages" => array(
	...
	public function work()
	{
		...
		if(isset($this->command['online']) && ($this->command['online']=="1"))
		{
			...
			do
			{
				$offset = ($page-1)*$this->searchResultsPerPage;
				$this->savelog("Go to ONLINE page.");
				$content = $this->getHTTPContent($this->searchOnlinePageURL.$offset, $this->loginRefererURL, $cookiePath);
			...
			for($birthyear=(date("Y")-$this->command['age_from']); $birthyear>=(date("Y")-$this->command['age_to']); $birthyear--)
			{
				...
				foreach($cities as $city)
				{
					...
					do
					{
						...
						/***** search *****/
						/******************/
						//http://www.wer-kennt-wen.de/find/users/0/gender/2/city/Berlin/birthyear/0/city_select/other/page/3
						$search_arr = array(
						...
						$this->savelog("Search for gender: ".$this->command['gender'].", city: ".$city.", birthyear: ".$birthyear.", page: ".$page);

						$search_url = "";
						foreach($search_arr as $key=>$search_item)
						{
							$search_url .= $key."/".$search_item."/";
						}

						$content = $this->getHTTPContent($this->searchURL.$search_url, $this->searchRefererURL, $cookiePath);
						...
						if(is_array($list))
						{
							...
							if(count($list))
							{
								...
								foreach($list as $item)
								{
									...
									if($sleep_time==0)
									{
										$this->work_sendMessage($username, $item, $this->searchURL.$search_url, $cookiePath);
									}
									else
									{
										$this->savelog("Not in running time period.");
	...
	private function getMembersFromOnlineResult($username, $page, $content)
	{
		...
		$content = substr($content,strpos($content,'<div class="uiWkwList  user" >'));
		$content = substr($content,0,strpos($content,'<p class="center pagination">'));
	...
	private function work_sendMessage($username, $item, $referer, $cookiePath, $enableMoreThanOneMessage=false){
		$return = false;
		// If not already sent
		if(!$this->isAlreadySent($item['userid']) || $enableMoreThanOneMessage)
		{
			...
			if($this->reserveUser($item['username'], $item['userid']))
			{
				...
				$gender=substr($content, strpos($content, "/find/field/gender/search/")+26);
				$gender=substr($gender, 0, 1);

				if($gender==$this->command['gender'])
				{
					// If guestbook enabled
					if(strpos($content, "G&auml;stebuch")!==false)
					{
						$hash = $this->getInputValue("hash",$content);
						$hashKey = $this->getInputValue("hashKey",$content);
						/**************************/
						/***** Sign guestbook *****/
						/**************************/
						$shift = rand(0,1);
						...
						$message_arr = array(
												...
												"type_id" => 1,
												"decorator_id" => rand(1+($shift*30),15+($shift*30))
											);
						...
						if(!$this->isAlreadySent($item['userid']) || $enableMoreThanOneMessage)
						{
							$content = $this->getHTTPContent($this->signGuestbookURL.$item['userid'], $this->profileURL.$item['userid'], $cookiePath, $message_arr);
							file_put_contents("sending/gb-".$username."-".$item['username']."-1.html",$content);

							if(strpos($content, "Dein G&auml;stebucheintrag wurde gespeichert.")!==false)
							{
								$this->newMessage=true;
								$this->savelog("Signing guestbook completed.");
								$this->lastSentTime = time();
								DBConnect::execute_q("INSERT INTO werKenntWen_sent_messages (to_username, to_userid,from_username,subject,message,sent_datetime) VALUES ('".addslashes($item['username'])."','".$item['userid']."','".$username."','".addslashes($subject)."','".addslashes($message)."',NOW())");
								$return = true;
							}
							else
							{
				...
				{
					$this->newMessage=false;
					$this->savelog("This profile is not a ".($this->command['gender']==2?"man":"woman").", skipping.");
				}
				$this->cancelReservedUser($item['userid']);
			}
		}
