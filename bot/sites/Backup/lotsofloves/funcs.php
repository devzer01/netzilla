...
class funcs
{
	...
	static function memberlogin($username, $password, $loginURL, $loginRefererURL, $currentIndex, $total, $max_retry=6)
	{
		$cookie_path = self::getCookiePath($username);
		
		$postData = array(	'_method' => 'POST',
							'data' => array(
											'User' => array(
															'username' => $username,
															'password' =>  $password,
		...
		if($need_login)
		{
			for($count_login = 1; $count_login <= $max_retry; $count_login++)
			{
	...
	static function isCookieValid($username,$loginRefererURL)
	{
		...
		if(strpos($result,"/profile/logout")>-1)
			return true;
		else
			return false;
	}
	...
	static function getSearchResult($username, $searchURL, $searchReferer, $searchData, $page)
	{
		$cookie_path = self::getCookiePath($username);
		$postData = array( "_method" => "POST",
							"data[User][Country]" => $searchData['country'],
							"data[User][County]" => $searchData['county'],
							"data[User][AgeTo]2"=> $searchData['age_from'],
							"data[User][AgeFrom]2" => $searchData['age_to'],
							"data[User][Gender]" => $searchData['gender'],
							"data[User][Sexuality]" => 1,
							"data[Criteria1][Type]" => 0,
							"data[Criteria1][Option]" => 0,
							"data[Criteria1][Value]" => 0,
							"data[Criteria2][Type]" => 0,
							"data[Criteria2][Option]" => 0,
							"data[Criteria2][Value]" => 0,
							"data[Criteria3][Type]" => 0,
							"data[Criteria3][Option]" => 0,
							"data[Criteria3][Value]" => 0
							);
		$postData = http_build_query($postData);
		...
		curl_setopt($ch, CURLOPT_PROXYPORT, PROXY_PORT);
		curl_setopt($ch, CURLOPT_PROXYTYPE, CURLPROXY_SOCKS5);
		curl_setopt($ch, CURLOPT_URL, $searchURL.$searchPage);
		...
		$filename = $username."-".$searchData['country']."-".$searchData['county']."-".$page;
		file_put_contents("search/".$filename.".html",$result);
		return self::getMembersFromSearchResult($username, $filename, utf8_encode($result));
	}

	static function getMembersFromSearchResult($username, $filename, $content)
	{
		//$content=file_get_contents("sample-search-content.html");
		//Find NEXT url

		$content = substr($content,strpos($content,'<div id="content">'));
		$content = substr($content,0,strpos($content,'<div id="footer">'));
		$content = substr($content,strpos($content,'WideSectionContainer1'));
		$content = substr($content,0,strpos($content,'<span class="current">'));
		...
		file_put_contents("xml/".$filename.".xml",$xml);
		...
		if(isset($parser->document->div))
		{
			...
			{
				if($div->tagAttrs['id']=="WideSectionContainer1")
				{
					$user_id = str_replace("/profile/user/","",trim($div->div[0]->div[2]->table[0]->tr[0]->td[0]->table[0]->tr[0]->td[0]->div[0]->a[0]->tagAttrs['href']));
					$user_img = trim($div->div[0]->div[2]->table[0]->tr[0]->td[0]->table[0]->tr[0]->td[0]->div[0]->a[0]->img[0]->tagAttrs['src']);
					$user_name = trim($div->div[0]->div[2]->table[0]->tr[0]->td[0]->table[0]->tr[0]->td[1]->a[0]->tagData);
					$age = trim($div->div[0]->div[2]->table[0]->tr[0]->td[0]->table[0]->tr[0]->td[1]->strong[0]->tagData);

					if($user_img=="/img/photo/0M_m.jpg")
						$user_img = "";
					else
						$user_img = "http://www.lotsofloves.com".str_replace("_m","_l",$user_img);

					array_push($list,array('username' => $user_name, 'userid' => $user_id, 'pic' => $user_img, 'age' => $age));
				}
			}
		}
		return $list;
	...
	static function saveMembers($list,$post)
	{
		...
		{
			//echo "INSERT INTO lotsofloves_member (username, userid, gender, pic, created_datetime) VALUES ('".$member['username']."', '".$member['userid']."', '".$post['data']['User']['Gender']."', '".$member['pic']."', NOW())"."<br/>";
			DBConnect::execute_q("INSERT INTO lotsofloves_member (username, userid, age, gender, country, pic, created_datetime) VALUES ('".$member['username']."', '".$member['userid']."', '".$member['age']."', '".$post['gender']."', '".$post['country']."',  '".$member['pic']."', NOW())");
		}
	}

	static function getMembers($post,$amount)
	{
		$sql = "SELECT username FROM lotsofloves_member WHERE gender='".$post['data']['User']['Gender']."' LIMIT ".$amount;
		return DBConnect::assoc_query_2D($sql);
	}

	static function getNextMember($post)
	{
		$sql = "SELECT username,userid FROM lotsofloves_member WHERE gender='".$post['gender']."' AND age>='".$post["age_from"]."' AND age<='".$post["age_to"]."' AND country='".$post['country']."' AND username NOT IN (SELECT to_username FROM lotsofloves_sent_messages) AND ((id-1)%12)=(".BOT_ID."-1) ORDER BY DATE(created_datetime) DESC, id ASC LIMIT 1";
	...
	static function sendMessage($from, $toName, $toId, $subject, $message, $sendMessageURL, $sendMessageRefererURL)
	{
		...
		$sendMessagePostData = array(	"_method" => "POST",
										"data" => array(
														"Mail" => array(
																		"subject" =>  $subject,
																		"text" => $message
		...
		if(strpos($result, "Mail Sent.")>-1)
		{
			funcs::savelog("Sent message to ".$toName);
			DBConnect::execute_q("INSERT INTO lotsofloves_sent_messages (to_username,from_username,subject,message,sent_datetime) VALUES ('".$toName."','".$from."','".addslashes($subject)."','".addslashes($message)."',NOW())");
			return true;
		}
		else
		{
			funcs::savelog("Sending message to ".$toName." failed");
			DBConnect::execute_q("DELETE FROM lotsofloves_member WHERE username='".$toName."'");
			return false;
		}
	}
