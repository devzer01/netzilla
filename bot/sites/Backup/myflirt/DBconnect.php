...
class DBconnect
{
	...
	*/ 	
	...
	static function checkMySQLConnection()
	{
		global $bot;
		global $link;
		$link=@mysql_connect(MYSQL_SERVER,MYSQL_USERNAME, MYSQL_PASSWORD) or die("Database connection error."); //connect mysql
		...
		while(!@mysql_query("SET NAMES UTF8"))
		{
			...
			{
				$bot->savelog("MySQL connection lost, wait for 60s.");
				$bot->sleep(60);
			}
			else
			{
				funcs::savelog("MySQL connection lost, wait for 60s.");
				funcs::sleep(60);
			}
			@mysql_close($link);
