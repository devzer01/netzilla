...
class bot
{
	...
	protected $indexURL = "";
	protected $indexURLLoggedInKeyword = array();
	protected $loginURL = "";
	...
	public function login()
	{
		...
		if(!($this->isLoggedIn($username)))
		{
			...
			for($count_login=1; $count_login<=$this->loginRetry; $count_login++)
			{
				...
				{
					$this->savelog("Logged in with profile: ".$username. " [".$this->profileLanguage."]");
					return true;
				}
			}
		}
		...
		{
			$this->savelog("This profile: ".$username." has been logged in.". " [".$this->profileLanguage."]");
			return true;
		}
	}
	...
	protected function isLoggedIn($username)
	{
		...
		if(strpos($content, $this->indexURLLoggedInKeyword['EN'])!==false)//If logged in
		{
			$this->profileLanguage = "EN";
			$loggedin = true;
			$this->token = $this->getToken($content);
		}
		elseif(strpos($content, $this->indexURLLoggedInKeyword['DE'])!==false)//If logged in
		{
	...
	protected function getToken($content)
	{
		$token = substr($content, strpos($content, "SC.token.set")+14);
