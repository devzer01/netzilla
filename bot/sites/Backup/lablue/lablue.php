...
class Lablue extends bot
{
	public function lablue($post)
	{
		...
		else
		{
			$this->command = array(
									"profiles" => array(
															array(
																	"username" => "adalinda_87",
																	"password" => "6heuschrecke"
																	)
														),
									"messages" => array(
															array(
		...
		for($age=$this->command['ageStart']; $age<=$this->command['ageEnd']; $age++)
		{
			...
			foreach($loops as $key => $val){
				...
				$search_arr = array(
					...
					"rsn" => 0,
					"rsr" => 0,
					"sexf" => $this->command['sexf'],
					"sexm" => $this->command['sexm'],
				...
				if(count($members)>0){
					...
					foreach($members as $nodes => $member){
						...
						if($this->command['version']==1){							
							$this->savelog("Sending Message to: ".$member);
	...
	{
		$this->savelog("Go to profile page: ".$item);
		$content = $this->getHTTPContent($this->profileURL."/".$item.".html", $this->searchRefererURL, $cookiePath);
		$this->sleep(5);
		return $content;
	}
