...
class funcs
{
	...
	static function memberlogin($username, $password, $loginURL, $loginRefererURL, $headerOpt, $currentIndex, $total)
	{
		...
		{*/
			for($count_login = 1; $count_login <= 6; $count_login++)
			{
				...
				$result = curl_exec($ch);
				$result = str_replace("location.href='/member.php';","",$result);
				...
				if(strpos($result, "Welcome, <b>".$username."</b>")!==false)//if($cookie['sid']!="")//rosiebell!<br>Please
				{
	...
	static function isLoggedIn($username, $headerOpt)
	{
		...
		curl_setopt($ch, CURLOPT_PROXYPORT, PROXY_PORT);
		curl_setopt($ch, CURLOPT_PROXYTYPE, CURLPROXY_SOCKS5);
		curl_setopt($ch, CURLOPT_URL, "http://www.freedateworld.com/");
		curl_setopt($ch, CURLOPT_REFERER, "http://www.freedateworld.com/");
		curl_setopt($ch, CURLOPT_USERAGENT,"Mozilla/5.0 (Windows NT 5.1) AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.168 Safari/535.19");
		curl_setopt($ch, CURLOPT_HTTPHEADER, $headerOpt);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_TIMEOUT,30); 
	...
	static function memberLogOut($username)
	{
		...
		curl_setopt($ch, CURLOPT_PROXYPORT, PROXY_PORT);
		curl_setopt($ch, CURLOPT_PROXYTYPE, CURLPROXY_SOCKS5);
		curl_setopt($ch, CURLOPT_URL, "http://www.freedateworld.com/logout?logout=1");
		curl_setopt($ch, CURLOPT_REFERER, "http://www.freedateworld.com/");
		curl_setopt($ch, CURLOPT_USERAGENT,"Mozilla/5.0 (Windows NT 5.1) AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.168 Safari/535.19");
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_TIMEOUT,30); 
		...
		curl_setopt($ch, CURLOPT_PROXYPORT, PROXY_PORT);
		curl_setopt($ch, CURLOPT_PROXYTYPE, CURLPROXY_SOCKS5);
		curl_setopt($ch, CURLOPT_URL, "http://www.freedateworld.com/profilesearch/search_simple/");
		curl_setopt($ch, CURLOPT_REFERER, "http://www.freedateworld.com/");
		curl_setopt($ch, CURLOPT_USERAGENT,"Mozilla/5.0 (Windows NT 5.1) AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.168 Safari/535.19");
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_TIMEOUT,30); 
	...
	static function getSearchResult($username, $searchURL, $searchReferer, $searchData, $page)
	{
		...
		curl_setopt($ch, CURLOPT_FOLLOWLOCATION, TRUE);
		$result = curl_exec($ch); 
		$result = str_replace("location.href='/search.php';","",$result);
	...
	{
		//Find NEXT url
		$content = substr($content, strpos($content, '<!-- CONTENT (Search result) -->'));//'<div style="padding-bottom:18px; padding-top:18px;">'
		$content = substr($content, 0, strpos($content, '<!-- CONTENT (Search result) [END] -->'));//'<div style="clear:both; height:25px;">'
		...
		if(isset($parser->document->table[0]->tr[1]))
		{
			foreach($parser->document->table[0]->tr[1]->td[0]->table as $member)
			{
				if(isset($member->tr[0]->td[1]->h3))
				{
					$username = $member->tr[1]->td[0]->table[0]->tr[1]->td[1]->table[0]->tr[0]->td[0]->div[0]->tagData;
					$temp = $member->tr[1]->td[0]->table[0]->tr[1]->td[1]->table[0]->tr[1]->td[0]->tagData;
					$age = substr($temp,0,2);
					$location = $member->tr[1]->td[0]->table[0]->tr[1]->td[1]->table[0]->tr[1]->td[0]->a[0]->tagData;
					$location .= ",".$member->tr[1]->td[0]->table[0]->tr[1]->td[1]->table[0]->tr[1]->td[0]->a[1]->tagData;
					$pic = $member->tr[1]->td[0]->table[0]->tr[1]->td[0]->table[0]->tr[0]->td[0]->div[0]->a[0]->img[0]->tagAttrs['src'];

					array_push($list,array('username' => $username, 'age' => $age, 'location' => $location, 'pic' => $pic));
				}
			}
		}
	...
	static function saveMembers($list, $post)
	{
		...
		{
			//echo "INSERT INTO freedateworld_member (username, gender, age, pic, created_datetime) VALUES ('".$member['username']."', '".$post['LookingFor']."', '".$member['age']."', '".$member['pic']."', NOW())"."<br/>";
			DBConnect::execute_q("INSERT INTO freedateworld_member (username, gender, age, pic, created_datetime) VALUES ('".$member['username']."', '".$post['LookingFor']."', '".$member['age']."', '".$member['pic']."', NOW())");
		}
	}
	...
	{
		$sql = "SELECT username FROM freedateworld_member WHERE gender='".$post['LookingFor']."' LIMIT ".$amount;
		return DBConnect::assoc_query_2D($sql);
	}

	static function getNextMember($post)
	{
		$sql = "SELECT username FROM freedateworld_member WHERE username NOT IN (SELECT to_username FROM freedateworld_sent_messages) AND ((id-1)%6)=(".BOT_ID."-1) AND gender='".$post['LookingFor']."' AND age>='".$post['DateOfBirth_start']."' AND age<='".$post['DateOfBirth_end']."' ORDER BY id ASC LIMIT 1";
	...
	static function sendMessage($from, $username, $userid, $subject, $message, $sendMessageURL, $sendMessageReferer, $headerOpt)
	{
		...
		$sendMessageURL = $sendMessageURL.$username;
		...
		file_put_contents("sending/".$from."-".$username.".txt",$result);
		
		if(strpos($result, "Message has been successfully sent")>-1)
		{
			funcs::savelog("Sent message to ".$username);
			
			DBConnect::execute_q("INSERT INTO freedateworld_sent_messages (to_username,from_username,subject,message,sent_datetime) VALUES ('".$username."','".$from."','".addslashes($subject)."','".addslashes($message)."',NOW())");
			return true;
		}
		else
		{
			self::savelog("Sending message to ".$username." failed");
			DBConnect::execute_q("DELETE FROM freedateworld_member WHERE username='".$username."'");
	...
	static function parse_curl_cookie($cookie_file)
    {
		if(file_exists($cookie_file))
		{
			...
			{
				if(strpos($line,"www.freedateworld.com")>-1)
				{
	...
	{
		$sql = "SELECT COUNT(id) AS total FROM freedateworld_member";
		return DBConnect::assoc_query_1D($sql);
	}
