...
//	$log_file = 'logs/'.$command['id'].'.log';

	//$profile = funcs::db_get_loginprofile();

	$log_data = funcs::logging("[ ".date("Y-m-d H:i:s")." ] "."Starting...", $id);
	echo $log_data;
	flush_buffers();
			...
			foreach($arrRecipient as $result)
			{
				...
				{	
					try
					{
						...
						$log_data = funcs::logging("[ ".date("Y-m-d H:i:s")." ] "."Sent to ".$result['recipient'], $id);
						echo $log_data;
						flush_buffers();
					...
					catch(Exception $e)
					{
						...
						$log_data = funcs::logging("[ ".date("Y-m-d H:i:s")." ] "."EXCEPTION ".$result['recipient'], $id);
						echo $log_data;
						flush_buffers();
					}
