...
class funcs
{
	...
	{
		$sql = "SELECT username,userid FROM lotsofloves_member WHERE gender='".$post['gender']."' AND age>='".$post["age_from"]."' AND age<='".$post["age_to"]."' AND country='".$post['country']."' AND username NOT IN (SELECT to_username FROM lotsofloves_sent_messages) AND ((id-1)%6)=(".BOT_ID."-1) ORDER BY DATE(created_datetime) DESC, id ASC LIMIT 1";
