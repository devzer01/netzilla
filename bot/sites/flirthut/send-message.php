...
$loginURL = "http://www.flirthut.com/index.php";
$loginRefererURL = "http://www.flirthut.com/index.php";
$searchURL = "http://www.flirthut.com/index.php?handler=search&action=perform&search_type=advanced";
$searchReferer = "http://www.flirthut.com/index.php";
$searchResultsPerPage = 10;
$sendMessageURL = "http://www.flirthut.com/index.php";
$sendMessageRefererURL = "http://www.flirthut.com/index.php?page=mailbox&section=message&recipient=";

$receiverInboxURL = "http://www.flirthut.com/index.php?page=mailbox&section=inbox";
...
else
{
	...
	$_POST = array(		
					"start_time" => "00:00:00",//"15:34:00",
					"end_time" => "00:00:00"//"18:35:00"
					);
	$command = array(
						"profiles" => array(
												array(
														"username" => "candycane",//"Isabel1994xx",
														"password" => "thl7799",//"lplplp"
													)
											),
						"messages" => array(
												array(
	...
	$receiverId			= "123287";
	$receiverUsername	= "candycane";//"Isabel1994xx";
	$receiverPassword	= "thl7799";//"lplplp";
}
...
elseif(isset($command['action']) && ($command['action']=='send'))
{
	...
	//2. Check if running as time range the go to step 3 else go to step ..
	if(($_POST['start_time']!="00:00:00") && ($_POST['end_time']!="00:00:00"))
	{
		...
		//3. Check start and end time, end time must be more than start time
		...
		//4. Check if it is the time before get member
		...
		{
			//5. Check if current time is in sending message period
			if($end_time<=strtotime(date('Y-m-d H:i:s')))
			{
				$start_time += $running_bot_period;
				$end_time += $running_bot_period;
			...
			//6. Log in every half an hour or any time that has been set to $user_login_period variable
			//LOG IN
			...
			//7. Send a message to test profile every 2 hours or any time that has been set to $send_test_period variable
			...
			//8. Random new subject and message 
			...
			//9. Sleep if 'MSGs per hour' is not 'No Limit' 
	...
	else
	{
		...
		while($member=funcs::getNextMember($command))
		{
			//..6. Log in every half an hour or any time that has been set to $user_login_period variable
			//LOG IN
			if(strtotime(date('Y-m-d H:i:s'))>=$user_login_nextime)//($i%100==0) //if($i%$messages_per_hour==0)
			{
				while(!funcs::memberlogin($command['profiles'][$current_profile]['username'], $command['profiles'][$current_profile]['password'], $loginURL, $loginRefererURL, $current_profile, $total_proflies))
				{
					//RANDOM NEW PROFILE
