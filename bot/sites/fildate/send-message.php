...
if($_POST)
{
	...
	$loginURL = "http://www.fildate.com/prglogin.php";
	$loginRefererURL = "http://www.fildate.com";
	$searchURL = "http://www.fildate.com/prgsearch.php";
	$searchReferer = "http://www.fildate.com/search.php";
	$sendMessageURL = "http://www.fildate.com/prgsendmail.php";
	$sendMessageRefererURL = "http://www.fildate.com/sendmail.php";
...
//	$sendMessageURL = "http://www.single123.com/account/messages/compose/";
	...
	$receiverUserId = "69189";
	$receiverPassword = "R2021982b";
	$receiverInboxURL = "http://www.fildate.com/myemail.php";
}
else
{
	define("ID",1);
...
elseif(isset($command['action']) && ($command['action']=='search'))
{
	if(funcs::memberlogin($command['profiles'][$current_profile]['username'], $command['profiles'][$current_profile]['password'],$loginURL,$loginRefererURL))
	{
		...
		foreach($command['Country'] as $country)
		{
			$retries = $max_retries;
			$post['Country'] = $country;
			//Must start with page 1
			$page = 1;
			do
			{
				funcs::savelog("Search [country ".$post['Country']."] page: ".$page);
