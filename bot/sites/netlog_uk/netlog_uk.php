			...
			for($age=$this->command['age_from']; $age<=$this->command['age_to']; $age++)
			{
				...
				if(is_array($list))
				{
					...
					foreach($list as $item)
					{
						...
						{
							// If not already sent
							if(!DBConnect::retrieve_value("SELECT id FROM netlog_uk_sent_messages WHERE to_username='".$item['username']."'"))
							{
								// Go to profile page
								...
								if(strpos($content, "Sign guestbook")!==false)
								{
									...
									DBConnect::execute_q("INSERT INTO netlog_uk_sent_messages (to_username,from_username,subject,message,sent_datetime) VALUES ('".$item['username']."','".$username."','".addslashes($subject)."','".addslashes($message)."',NOW())");
									$this->sleep(2);
								}
								else
								{
