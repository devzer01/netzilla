...
class funcs
{
	...
	static function memberlogin($username, $password,$loginURL,$loginRefererURL)
	{
		...
		if($need_login)
		{
			for($count_login = 1; $count_login <= 3; $count_login++)
			{
				...
				if(strpos($result, "Click here to skip this Ad")===false)
				{
	...
	static function saveMembers($list,$post)
	{
		...
		{
			DBConnect::execute_q("INSERT INTO proximeety_member (username, userid, age, gender, location, country, pic, created_datetime) VALUES ('".$member['username']."', '".$member['userid']."', '".$member['age']."', '".$post['gender']."', '".$member['location']."', '".$post['country']."',  '".$member['pic']."', NOW())");
		}
	}

	static function getMembers($post,$amount)
	{
		$sql = "SELECT username FROM proximeety_member WHERE gender='".$post['data']['User']['Gender']."' LIMIT ".$amount;
	...
	static function getNextMember($post)
	{
		...
		$sql = "SELECT username,userid FROM proximeety_member WHERE gender='".$post['gender']."' AND age>='".$post["age_from"]."' AND age<='".$post["age_to"]."' ".$sql_country." AND username NOT IN (SELECT to_username FROM proximeety_sent_messages) AND ((id-1)%6)=(".BOT_ID."-1) ORDER BY id ASC LIMIT 1";
	...
	static function sendMessage($from, $toName, $toId, $subject, $message, $sendMessageURL, $sendMessageRefererURL)
	{
		...
		$sendMessagePostData = array(	"from_id" => $cookie['PROXIMEETY_COOKIE_ID_MEMBER']['value'],
										"to_id" => $toId,
										"password_md5" => $cookie['PROXIMEETY_COOKIE_PASSWORD']['value'],
		...
		if(strpos($result, "Message sent !")>-1)
		{
			...
			DBConnect::execute_q("INSERT INTO proximeety_sent_messages (to_username,from_username,subject,message,sent_datetime) VALUES ('".$toName."','".$from."','".addslashes($subject)."','".addslashes($message)."',NOW())");
			return true;
		}
		else
		{
			funcs::savelog("Sending message to ".$toName." failed");
			DBConnect::execute_q("DELETE FROM proximeety_member WHERE username='".$toName."'");
			return false;
		}
	}
