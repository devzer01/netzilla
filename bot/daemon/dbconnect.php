<?php
define('MYSQL_SERVER','localhost'); //config mysql host
define('MYSQL_USERNAME','bot'); //config mysql username
define('MYSQL_PASSWORD','bot'); //config mysql password
define('MYSQL_DATABASE','bot'); //config mysql database
...
class DBconnect
{
	...
	*/ 	
	...
	static function checkMySQLConnection()
	{
		global $bot;
		global $link;
		$link=@mysql_connect(MYSQL_SERVER,MYSQL_USERNAME, MYSQL_PASSWORD); //connect mysql
		@mysql_select_db(MYSQL_DATABASE); //connect database

		while(!mysql_query("SET NAMES UTF8"))
		{
			logging("MySQL connection lost, wait for 60s.");
			sleep(60);
			@mysql_close($link);
